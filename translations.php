<?php
	$GLOBALS['translations'] = array(
		'en' => array(
			...
			"elu_description" => "Exponential Linear Unit (ELU).<br>This function helps the model learn faster and prevents dead neurons. For negative values, it smooths the curve instead of cutting it off completely.<br>Equation: <span class='temml_me'>\\text{elu}\\left(x\\right) = \\left\\{\\begin{array}{ll} \\alpha \\cdot \\left(e^x - 1\\right) & \\text{for } x < 0 \\\\ x & \\text{for } x >= 0\\end{array}\\right.</span>",
			"leakyReLU_description" => "Leaky ReLU (Leaky Rectified Linear Unit).<br>This version allows small negative outputs instead of setting them to zero, which helps avoid inactive neurons.<br>Equation: <span class='temml_me'>\\text{leakyReLU}(x) = \\left\\{\\begin{array}{ll} \\alpha \\cdot x & \\text{for } x < 0 \\\\ x & \\text{for } x >= 0 \\end{array}\\right.</span>",
			"reLU_description" => "Rectified Linear Unit (ReLU).<br>This is one of the most common activation functions. It keeps all positive values and turns negatives into zero, making the network faster and simpler.<br><span class='temml_me'>\\mathrm{relu}\\left(x\\right) = \\mathrm{max}(0, x)</span>",
			"softmax_description" => "Softmax activation function.<br>It converts a list of numbers into probabilities that sum up to 1. This is commonly used in the output layer for classification tasks.<br><span class='temml_me'>\\mathrm{softmax}(x) = \\frac{e^{z_j}}{\\sum^K_{k=1} e^{z_k}}</span>",
			"thresholdedReLU_description" => "Thresholded ReLU.<br>This function works like ReLU, but it only activates when the input is greater than a certain threshold value θ.<br><span class='temml_me'>f(x) = \\left\\{\\begin{array}{ll} x & \\text{for } x > \\theta \\\\ 0 & \\text{otherwise}\\end{array}\\right.</span>",
			"snake_description" => "Snake activation.<br>This function transforms the input as follows, where <span class='temml_me'>\alpha</span> is a learnable parameter:<br><span class='temml_me'>f(x) = x + \\frac{\\sin^2(x)}{\\alpha}</span>",
		...
		'de' => array(
			...
			"elu_description" => "Exponential Linear Unit (ELU).<br>Diese Funktion hilft dem Modell, schneller zu lernen und verhindert, dass Neuronen dauerhaft inaktiv werden. Für negative Werte wird die Kurve weich abgesenkt, anstatt sie abrupt auf null zu setzen.<br>Gleichung: <span class='temml_me'>\\text{elu}\\left(x\\right) = \\left\\{\\begin{array}{ll} \\alpha \\cdot \\left(e^x - 1\\right) & \\text{für } x < 0 \\\\ x & \\text{für } x >= 0\\end{array}\\right.</span>",
			"leakyReLU_description" => "Leaky ReLU (Leaky Rectified Linear Unit).<br>Im Gegensatz zur normalen ReLU-Funktion werden negative Werte hier nicht komplett abgeschnitten, sondern leicht durchgelassen. Das verhindert, dass Neuronen 'einschlafen'.<br>Gleichung: <span class='temml_me'>\\text{leakyReLU}(x) = \\left\\{\\begin{array}{ll} \\alpha \\cdot x & \\text{für } x < 0 \\\\ x & \\text{für } x >= 0 \\end{array}\\right.</span>",
			"reLU_description" => "Rektifizierte lineare Einheit (ReLU).<br>Eine der am häufigsten genutzten Aktivierungsfunktionen. Positive Werte bleiben erhalten, negative werden zu null gesetzt. Dadurch lernt das Netzwerk einfacher und schneller.<br><span class='temml_me'>\\mathrm{relu}\\left(x\\right) = \\mathrm{max}(0, x)</span>",
			"softmax_description" => "Softmax-Aktivierungsfunktion.<br>Sie wandelt eine Liste von Zahlen in Wahrscheinlichkeiten um, die sich zu 1 aufsummieren. Wird häufig in der Ausgabeschicht für Klassifikationsaufgaben verwendet.<br><span class='temml_me'>\\mathrm{softmax}(x) = \\frac{e^{z_j}}{\\sum^K_{k=1} e^{z_k}}</span>",
			"thresholdedReLU_description" => "Thresholded ReLU (Schwellwert-ReLU).<br>Diese Funktion arbeitet wie ReLU, aktiviert sich jedoch nur, wenn der Eingabewert größer als ein bestimmter Schwellwert θ ist.<br><span class='temml_me'>f(x) = \\left\\{\\begin{array}{ll} x & \\text{für } x > \\theta \\\\ 0 & \\text{sonst}\\end{array}\\right.</span>",			'batchNormalization_description' => "Batch-Normalisierungsschicht (<a href='https://arxiv.org/abs/1502.03167' target='_blank'>Ioffe and Szegedy, 2014</a>).<br>Normalisieren Sie die Aktivierungen der vorherigen Schicht in jeder Batch, d.h. wendet eine Transformation an, die die mittlere Aktivierung nahe bei 0 und die Aktivierungsstandardabweichung nahe bei 1 hält.",
			"snake_description" => "Snake-Aktivierung.<br>Diese Funktion transformiert die Eingabe wie folgt, wobei <span class='temml_me'>\alpha</span> ein lernbarer Parameter ist:<br><span class='temml_me'>f(x) = x + \\frac{\\sin^2(x)}{\\alpha}</span>",
