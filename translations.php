<?php
	$GLOBALS['translations'] = array(
		'en' => array(
			...
			'elu_description' => 'Exponential Linear Unit (ELU).<br>It follows: $$\\text{elu}\\left(x\\right) = \\left\\{\\begin{array}{ll} \\alpha \\cdot \\left(e^x - 1\\right) & \\text{for } x < 0 \\n x & \\text{for } x >= 0\\end{array}\\right.$$',
			'leakyReLU_description' => 'Leaky version of a rectified linear unit.<br>It allows a small gradient when the unit is not active: $$ \\text{leakyReLU}(x) = \\left\\{\\begin{array}{ll} \\alpha \\cdot x & \\text{for } x < 0 \\n x & \\text{for } x >= 0 \\end{array}\\right.$$',
			'reLU_description' => 'Rectified Linear Unit activation function. $$\\mathrm{relu}\\left(x\\right) = \\mathrm{max}\\left(0, x\\right)$$',
			'softmax_description' => 'Softmax activation layer. $$\\mathrm{softmax}\\left(x\\right) = \\frac{e^{z_j}}{\\sum^K_{k=1} e^{z_k}}$$',
			'thresholdedReLU_description' => 'Thresholded Rectified Linear Unit. It follows: $$f(x) = \\left\\{\\begin{array}{ll} x & \\text{for } x > \\theta \\n 0 & \\text{otherwise}\\end{array}\\right.$$',
		...
		'de' => array(
			...
			'elu_description' => 'Exponential Linear Unit (ELU).<br>Es folgt: $$\\text{elu}\\left(x\\right) = \\left\\{\\begin{array}{ll} \\alpha \\cdot \\left(e^x - 1\\right) & \\text{für } x < 0 \\n x & \\text{für } x >= 0\\end{array}\\right.$$',
			'leakyReLU_description' => 'Leaky-Version einer rektifizierten linearen Einheit.<br>Sie erlaubt eine kleine Steigung, wenn die Einheit nicht aktiv ist: $$ \\text{leakyReLU}(x) = \\left\\{\\begin{array}{ll} \\alpha \\cdot x & \\text{für } x < 0 \\n x & \\text{für } x >= 0 \\end{array}\\right.$$',
			'reLU_description' => 'Aktivierungsfunktion der rektifizierten linearen Einheit. $$\\mathrm{relu}\\left(x\\right) = \\mathrm{max}\\left(0, x\\right)$$',
			'softmax_description' => 'Softmax-Aktivierungsschicht. $$\\mathrm{softmax}\\left(x\\right) = \\frac{e^{z_j}}{\\sum^K_{k=1} e^{z_k}}$$',
			'thresholdedReLU_description' => 'Thresholded Rectified Linear Unit. Es folgt: $$f(x) = \\left\\{\\begin{array}{ll} x & \\text{für } x > \\theta \\n 0 & \\text{sonst}\\end{array}\\right.$$',
