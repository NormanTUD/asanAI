...
#for i in $(ack "^function " *.js | grep -v tests.js | sed -e 's#.*function\s*##' | sed -e 's#\s*(.*##' | sort); do grep $i tests.js 2>&1 >/dev/null || NUM_OCC=$(ack "(?:(?:^\s*)|=|\()\s*(?:await)?\s*$i\s*\(" *.js | grep -v function | wc -l); echo "$NUM_OCC: $i is untested currently"; done | sort -nr
