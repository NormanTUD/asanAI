...
// ============================================================
// TOPOLOGICAL DATA ANALYSIS (TDA) MODULE v3.0
// Singleton, Interactive, Auto-Updating, Sliders, Explanations
// ============================================================
...
var TDAAnalysis = (function () {
	...
	var _state = {
		...
		// Slider values
		sliderMaxPoints: 300,
		sliderMinPersistence: 0.005,
		sliderTakensWindow: 8,
		sliderEpsilonSteps: 40
	};
	...
	var _SINGLETON_ID = "tda_singleton_container";
	...
	function _getOrCreateContainer(divOrId) {
		...
		if (existing) {
			...
			_state.containerId = _SINGLETON_ID;
			return existing;
		}
	...
	function _computeDistanceMatrix(points) {
		...
		for (var i = 0; i < n; i++) {
			for (var j = i + 1; j < n; j++) {
				var d = _euclideanDistance(points[i], points[j]);
				if (!isFinite(d)) d = 0;
				dist[i * n + j] = d;
				dist[j * n + i] = d;
	...
	function _pca2D(points) {
		...
		for (var i = 0; i < n; i++) {
			for (var j = 0; j < dim; j++) mean[j] += (points[i][j] || 0);
		}
		...
		for (var i = 0; i < n; i++) {
			var row = [];
			for (var j = 0; j < dim; j++) row.push((points[i][j] || 0) - mean[j]);
			centered.push(row);
		}
	...
	function _UnionFind(n) {
		...
		for (var i = 0; i < n; i++) { this.parent[i] = i; this.size[i] = 1; }
	}
	...
	function _computeH0Persistence(distInfo) {
		...
		var n = distInfo.n, dist = distInfo.matrix, edges = [];
		for (var i = 0; i < n; i++) {
			for (var j = i + 1; j < n; j++) {
				var d = dist[i * n + j];
				if (isFinite(d) && d > 0) edges.push({ i: i, j: j, d: d });
			}
		}
		if (edges.length === 0) return [];
		edges.sort(function (a, b) { return a.d - b.d; });
		if (edges.length > _config.maxEdgesForVR) edges = edges.slice(0, _config.maxEdgesForVR);

		var uf = new _UnionFind(n);
		var persistence = [];
		for (var e = 0; e < edges.length; e++) {
			var edge = edges[e];
			var ri = uf.find(edge.i), rj = uf.find(edge.j);
			if (ri !== rj) {
				var deathEps = edge.d;
				if (deathEps > _config.minPersistence) {
					persistence.push({ birth: 0, death: deathEps, persistence: deathEps, dimension: 0, representative: [edge.i, edge.j] });
				}
			...
			if (ri === rj) {
				...
				if (pathLen >= 2 && pathLen <= 10) {
					...
					if (!cyclesSeen[cycleKey] && pers > _config.minPersistence) {
						...
						persistence.push({ birth: birthEps, death: deathEps, persistence: pers, dimension: 1, representative: [edge.i, edge.j], cycleLength: pathLen + 1 });
					}
				}
			} else {
				uf.union(edge.i, edge.j);
	...
	function _bfsPathLength(adjacency, start, end, n) {
		...
		while (head < queue.length) {
			...
			for (var i = 0; i < neighbors.length; i++) {
				...
				if (!visited[next]) { visited[next] = 1; queue.push({ node: next, depth: current.depth + 1 }); }
			}
		}
		return Infinity;
	}
	...
	function _computeBettiCurve(distInfo, persistence, dimension) {
		...
		var steps = _state.sliderEpsilonSteps;
		var curve = [];
		for (var s = 0; s <= steps; s++) {
			...
			for (var p = 0; p < persistence.length; p++) {
				if (persistence[p].dimension === dimension) {
					if (persistence[p].birth <= eps && (persistence[p].death > eps || !isFinite(persistence[p].death))) count++;
				}
			}
	...
	function _computeTopologicalSummary(persistence) {
		if (!persistence || persistence.length === 0) {
			return { totalFeatures: 0, h0Count: 0, h1Count: 0, maxPersistence: 0, meanPersistence: 0, medianPersistence: 0, entropy: 0, wasserstein1: 0, landscape: [] };
		}
		...
		var maxPers = 0, sumPers = 0, persValues = [];
		...
		return { totalFeatures: persistence.length, h0Count: h0.length, h1Count: h1.length, maxPersistence: maxPers, meanPersistence: meanPers, medianPersistence: medianPers, entropy: entropy, wasserstein1: wasserstein1, landscape: landscape };
	}
	...
	function _subsampleArray(arr, maxPoints) {
		...
		var result = [];
		for (var i = 0; i < maxPoints; i++) result.push(arr[indices[i]]);
		return result;
	}
	...
	function _reshapeToPointCloud(data, shape) {
		...
		try {
			if (!shape || shape.length < 2) {
				var windowSize = Math.min(_state.sliderTakensWindow, Math.floor(data.length / 3));
		...
		return _subsampleArray(points, _state.sliderMaxPoints);
	}
	...
	function _extractRawTrainingData() {
		try {
			if (typeof xy_data_global !== "undefined" && xy_data_global && xy_data_global.x) {
				var xData;
				if (xy_data_global.x.dataSync) xData = Array.from(xy_data_global.x.dataSync());
				else if (Array.isArray(xy_data_global.x)) xData = xy_data_global.x.flat(Infinity);
				else return null;
				var shape = xy_data_global.x.shape || null;
	...
	function _extractActivationData() {
		try {
			...
			for (var i = 0; i < latestRecord.layers.length; i++) {
				...
				layers.push({ data: Array.from(layer.output), shape: layer.outputShape, name: layer.layerName || ("layer_" + i), type: layer.layerType || "unknown", index: layer.layerIndex || i });
			}
			...
			for (var i = 0; i < rawWeights.length; i++) {
				if (rawWeights[i] && !rawWeights[i].isDisposed) {
					...
					if (wData && wData.length >= 4) {
						layers.push({ data: Array.from(wData), shape: rawWeights[i].shape, name: (m.weights && m.weights[i]) ? m.weights[i].name : ("weight_" + i), type: "weight", index: i });
					}
				}
			}
			return layers.length > 0 ? layers : null;
	...
	function _analyzePointCloud(points, name, meta) {
		...
		var distInfo = _computeDistanceMatrix(points);
		...
		return { name: name, meta: meta || {}, numPoints: points.length, pointDim: points[0] ? points[0].length : 0, persistence: allPersistence, bettiCurve0: bettiCurve0, bettiCurve1: bettiCurve1, summary: summary, projected: projected, maxDist: distInfo.maxDist };
	}

	function analyze(mode) {
		if (!mode) mode = _state.mode;
		_config.minPersistence = _state.sliderMinPersistence;
		_config.maxPoints = _state.sliderMaxPoints;
		_config.takensWindowSize = _state.sliderTakensWindow;
		_config.epsilonSteps = _state.sliderEpsilonSteps;

		var results = [], error = null;
		try {
			if (mode === "raw_data") {
				var rawData = _extractRawTrainingData();
				if (!rawData) { error = "tda_no_raw_data"; }
				else { var pts = _reshapeToPointCloud(rawData.data, rawData.shape); var r = _analyzePointCloud(pts, rawData.name, { type: "raw_data" }); if (r) results.push(r); }
			} else if (mode === "activations") {
				var layers = _extractActivationData();
				if (!layers) { error = "tda_no_activation_data"; }
				else { for (var i = 0; i < layers.length; i++) { var pts = _reshapeToPointCloud(layers[i].data, layers[i].shape); var r = _analyzePointCloud(pts, layers[i].name, { type: layers[i].type, index: layers[i].index, source: "activation" }); if (r) results.push(r); } }
			} else if (mode === "weights") {
				var layers = _extractWeightData();
				if (!layers) { error = "tda_no_model"; }
				else { for (var i = 0; i < layers.length; i++) { var pts = _reshapeToPointCloud(layers[i].data, layers[i].shape); var r = _analyzePointCloud(pts, layers[i].name, { type: layers[i].type, index: layers[i].index, source: "weight", shape: layers[i].shape }); if (r) results.push(r); } }
			} else {
				var rawData = _extractRawTrainingData();
				if (rawData) { var pts = _reshapeToPointCloud(rawData.data, rawData.shape); var r = _analyzePointCloud(pts, rawData.name, { type: "raw_data", source: "raw_data" }); if (r) results.push(r); }
				var actLayers = _extractActivationData();
				if (actLayers) { for (var i = 0; i < actLayers.length; i++) { var pts = _reshapeToPointCloud(actLayers[i].data, actLayers[i].shape); var r = _analyzePointCloud(pts, actLayers[i].name, { type: actLayers[i].type, index: actLayers[i].index, source: "activation" }); if (r) results.push(r); } }
				var wLayers = _extractWeightData();
				if (wLayers) { for (var i = 0; i < wLayers.length; i++) { var pts = _reshapeToPointCloud(wLayers[i].data, wLayers[i].shape); var r = _analyzePointCloud(pts, wLayers[i].name, { type: wLayers[i].type, index: wLayers[i].index, source: "weight", shape: wLayers[i].shape }); if (r) results.push(r); } }
				if (results.length === 0) error = "tda_no_data_available";
			}
		} catch (e) { error = "tda_analysis_error"; }

		var output = { layers: results, timestamp: Date.now(), mode: mode, epoch: (typeof current_epoch !== "undefined") ? current_epoch : 0, error: error };
		_state.lastResult = output;
		if (results.length > 0) {
			_state.history.push({ timestamp: output.timestamp, epoch: output.epoch, summaries: results.map(function (r) { return { name: r.name, summary: r.summary }; }) });
			if (_state.history.length > _state.maxHistory) _state.history.shift();
		}
		return output;
	}
	...
	function _persistenceDiagramSVG(persistence, layerIdx) {
		...
		var width = 240, height = 240, margin = 32;
		var plotW = width - 2 * margin, plotH = height - 2 * margin;
		var maxVal = 0;
		for (var i = 0; i < persistence.length; i++) {
		...
		var svg = "<svg class='tda_svg_interactive' width='100%' viewBox='0 0 " + width + " " + height + "'>";
		for (var g = 0; g <= 4; g++) {
			var gx = margin + (g / 4) * plotW;
			var gy = height - margin - (g / 4) * plotH;
			svg += "<line x1='" + gx + "' y1='" + margin + "' x2='" + gx + "' y2='" + (height - margin) + "' stroke='currentColor' stroke-width='0.3' opacity='0.12'/>";
			svg += "<line x1='" + margin + "' y1='" + gy + "' x2='" + (width - margin) + "' y2='" + gy + "' stroke='currentColor' stroke-width='0.3' opacity='0.12'/>";
		}
		...
		for (var i = 0; i < persistence.length; i++) {
			...
			svg += "<circle class='tda_pd_point' cx='" + x.toFixed(1) + "' cy='" + y.toFixed(1) + "' r='" + radius.toFixed(1) + "' fill='" + color + "' opacity='" + opacity.toFixed(2) + "' data-dim='" + p.dimension + "' data-birth='" + p.birth.toFixed(4) + "' data-death='" + p.death.toFixed(4) + "' data-pers='" + p.persistence.toFixed(4) + "' style='cursor:pointer; transition: r 0.2s, opacity 0.2s;'>";
			svg += "<title>H" + p.dimension + " | birth=" + p.birth.toFixed(4) + " | death=" + p.death.toFixed(4) + " | pers=" + p.persistence.toFixed(4) + "</title>";
			svg += "</circle>";
		}

		svg += "<text x='" + (width / 2) + "' y='" + (height - 3) + "' text-anchor='middle' font-size='9' fill='currentColor' opacity='0.5'>Birth (ε)</text>";
		svg += "<text x='8' y='" + (height / 2) + "' text-anchor='middle' font-size='9' fill='currentColor' opacity='0.5' transform='rotate(-90, 8, " + (height / 2) + ")'>Death (ε)</text>";
	...
	function _barcodeSVG(persistence, layerIdx) {
		...
		var maxDeath = 0;
		for (var i = 0; i < finite.length; i++) { if (finite[i].death > maxDeath) maxDeath = finite[i].death; }
		...
		for (var i = 0; i < finite.length; i++) {
			...
			svg += "<rect class='tda_barcode_bar' x='" + x1.toFixed(1) + "' y='" + y.toFixed(1) + "' width='" + barW.toFixed(1) + "' height='" + barHeight + "' fill='" + color + "' opacity='0.75' rx='2' style='cursor:pointer; transition: opacity 0.2s;'>";
	...
	function _bettiCurveSVG(curve0, curve1, layerIdx) {
		...
		for (var c = 0; c < curves.length; c++) {
			...
			svg += "<path d='" + areaPath + "' fill='" + colors[c] + "' opacity='0.07'/>";
			var path = "M";
			for (var i = 0; i < curve.length; i++) {
				var x = margin.left + (curve[i].epsilon / maxEps) * plotW;
				var y = margin.top + plotH - (curve[i].betti / maxBetti) * plotH;
	...
	function _pointCloudSVG(projected, layerIdx) {
		if (!projected || projected.length < 3) return "<div class='tda_empty_plot'>\u2014</div>";
		var width = 240, height = 240, margin = 20;
		var plotW = width - 2 * margin, plotH = height - 2 * margin;
		var minX = Infinity, maxX = -Infinity, minY = Infinity, maxY = -Infinity;
		for (var i = 0; i < projected.length; i++) {
			var px = projected[i][0] || 0, py = projected[i][1] || 0;
		...
		for (var i = 0; i < projected.length; i++) {
			...
			svg += "<circle class='tda_cloud_point' cx='" + x.toFixed(1) + "' cy='" + y.toFixed(1) + "' r='2.5' fill='#8e44ad' opacity='0.5' style='cursor:crosshair; transition: r 0.15s;'>";
			svg += "<title>(" + (projected[i][0] || 0).toFixed(3) + ", " + (projected[i][1] || 0).toFixed(3) + ")</title>";
			svg += "</circle>";
		}
	...
	function _landscapeSVG(landscape, layerIdx) {
		...
		var plotH = height - margin.top - margin.bottom;
		var maxVal = 0;
		for (var i = 0; i < landscape.length; i++) { if (landscape[i] > maxVal) maxVal = landscape[i]; }
		if (maxVal === 0) maxVal = 1;
		var svg = "<svg class='tda_svg_interactive' width='100%' viewBox='0 0 " + width + " " + height + "'>";
		...
		for (var i = 0; i < landscape.length; i++) {
			...
			svg += "<rect x='" + x.toFixed(1) + "' y='" + y.toFixed(1) + "' width='" + barW.toFixed(1) + "' height='" + barH.toFixed(1) + "' fill='#9b59b6' opacity='" + (0.5 + (landscape[i] / maxVal) * 0.4).toFixed(2) + "' rx='1.5'>";
	...
	function _bindInteractiveEvents() {
		...
		for (var i = 0; i < pdPoints.length; i++) {
			...
			pdPoints[i].addEventListener("click", function () { _showPointDetail(this); });
		}

		var bars = _state.container.querySelectorAll(".tda_barcode_bar");
		for (var i = 0; i < bars.length; i++) {
			bars[i].addEventListener("mouseenter", function () { this.setAttribute("opacity", "1"); });
			bars[i].addEventListener("mouseleave", function () { this.setAttribute("opacity", "0.75"); });
		}

		var cloudPoints = _state.container.querySelectorAll(".tda_cloud_point");
		for (var i = 0; i < cloudPoints.length; i++) {
			cloudPoints[i].addEventListener("mouseenter", function () { this.setAttribute("r", "5"); this.setAttribute("opacity", "1"); });
			cloudPoints[i].addEventListener("mouseleave", function () { this.setAttribute("r", "2.5"); this.setAttribute("opacity", "0.5"); });
		}

		var cardHeaders = _state.container.querySelectorAll(".tda_card_header_toggle");
		for (var i = 0; i < cardHeaders.length; i++) {
			cardHeaders[i].addEventListener("click", function () {
				...
				if (body) {
					...
					var icon = this.querySelector(".tda_expand_icon");
		...
		// Sliders
		var sliders = _state.container.querySelectorAll(".tda_slider_input");
		for (var i = 0; i < sliders.length; i++) {
			sliders[i].addEventListener("input", function () {
				var key = this.dataset.sliderKey;
				var val = parseFloat(this.value);
				if (key && !isNaN(val)) {
					_state[key] = val;
					var display = this.parentElement.querySelector(".tda_slider_val");
	...
	function _showPointDetail(el) {
		var existing = _state.container.querySelector(".tda_point_detail");
		...
		detail.innerHTML = "<div class='tda_detail_content'>" +
			...
			"<div class='tda_detail_close' onclick='this.parentElement.parentElement.remove()'>\u2715</div>" +
			"</div>";
	...
	// ============================================================
	// RENDER: SLIDERS PANEL
	// ============================================================

	function _renderSlidersPanel() {
		var html = "<div class='tda_sliders_panel'>";
		html += "<div class='tda_sliders_title'><span class='TRANSLATEME_tda_interactive_controls'></span></div>";
		html += "<div class='tda_sliders_grid'>";

		html += _renderSlider("sliderMaxPoints", "tda_slider_max_points", 10, 1000, _state.sliderMaxPoints, 10);
		html += _renderSlider("sliderMinPersistence", "tda_slider_min_persistence", 0, 0.5, _state.sliderMinPersistence, 0.005);
		html += _renderSlider("sliderTakensWindow", "tda_slider_takens_window", 2, 20, _state.sliderTakensWindow, 1);
		html += _renderSlider("sliderEpsilonSteps", "tda_slider_epsilon_steps", 10, 100, _state.sliderEpsilonSteps, 5);
	...
	function _renderSlider(key, labelKey, min, max, value, step) {
		return "<div class='tda_slider_item'>" +
			"<label class='tda_slider_label'><span class='TRANSLATEME_" + labelKey + "'></span>: <span class='tda_slider_val'>" + value + "</span></label>" +
			"<input type='range' class='tda_slider_input' data-slider-key='" + key + "' min='" + min + "' max='" + max + "' step='" + step + "' value='" + value + "'/>" +
			"</div>";
	}
	...
	function _renderPlotExplanation(key) {
		return "<div class='tda_plot_explanation'><span class='TRANSLATEME_" + key + "'></span></div>";
	}
	...
	function _renderLayerCard(layer, mode, layerIdx) {
		...
		// Header
		html += "<div class='tda_card_header_toggle'>";
		html += "<span class='tda_expand_icon'>" + (isExpanded ? "\u25BC" : "\u25B6") + "</span> ";
		html += "<strong>" + _escapeHtml(layer.name || "") + "</strong>";
		html += " <span class='tda_badge'>" + layer.numPoints + " pts, dim=" + layer.pointDim + "</span>";
		html += "</div>";
		...
		if (_state.filterDimension >= 0) {
			filteredPersistence = layer.persistence.filter(function (p) { return p.dimension === _state.filterDimension; });
		}
		...
		html += _renderPlotExplanation("tda_explain_persistence_diagram");
		...
		// Barcode
		html += "<div class='tda_plot_box'>";
		html += _renderPlotExplanation("tda_explain_barcode");
		...
		// Betti Curves
		html += "<div class='tda_plot_box'>";
		html += _renderPlotExplanation("tda_explain_betti_curves");
		...
		// Point Cloud
		html += "<div class='tda_plot_box'>";
		html += _renderPlotExplanation("tda_explain_point_cloud");
		html += "<div class='tda_plot_label'><span class='TRANSLATEME_tda_point_cloud'></span> (PCA 2D)</div>";
		html += _pointCloudSVG(layer.projected, layerIdx);
		html += "</div>";

		// Landscape
		html += "<div class='tda_plot_box'>";
		html += _renderPlotExplanation("tda_explain_landscape");
	...
	function _update() {
		if (_state.container) render(_state.container, _state.mode);
	}
	...
	function render(divOrId, mode) {
		...
		if (result.error) {
			...
			html += "<div class='tda_error'><span class='TRANSLATEME_" + result.error + "'></span></div>";
			html += "<div class='tda_error_nav'>";
			html += "<span class='TRANSLATEME_tda_switch_to'></span>: ";
			...
			for (var m = 0; m < modes.length; m++) {
				if (modes[m] !== _state.mode) {
					html += "<button class='tda_tab' onclick='TDAAnalysis.setMode(\"" + modes[m] + "\")'><span class='TRANSLATEME_" + modeKeys[m] + "'></span></button> ";
				}
			}
			html += "</div></div>";
		...
		html += "<button class='tda_btn' onclick='TDAAnalysis.refresh()' title='Refresh'>\u27F3</button>";
		html += "<button class='tda_btn " + (_state.autoUpdateTimer ? "tda_btn_active" : "") + "' onclick='TDAAnalysis.toggleAutoUpdate()'>" + (_state.autoUpdateTimer ? "\u23F8" : "\u25B6") + "</button>";
		html += "</div></div>";

		html += "<p class='tda_subtitle'><span class='TRANSLATEME_tda_subtitle'></span> | " + new Date(result.timestamp).toLocaleTimeString() + "</p>";
		...
		var modeKeys = ["tda_tab_activations", "tda_tab_weights", "tda_tab_raw_data", "tda_tab_auto"];
		for (var m = 0; m < modes.length; m++) {
			html += "<div class='tda_tab " + (_state.mode === modes[m] ? "tda_tab_active" : "") + "' onclick='TDAAnalysis.setMode(\"" + modes[m] + "\")'><span class='TRANSLATEME_" + modeKeys[m] + "'></span></div>";
		}
		html += "</div>";

		// Filter row
		html += "<div class='tda_filter_row'>";
		html += "<span class='TRANSLATEME_tda_filter'></span>: ";
		html += "<button class='tda_filter_btn " + (_state.filterDimension === -1 ? "tda_filter_active" : "") + "' onclick='TDAAnalysis.setFilter(-1)'><span class='TRANSLATEME_tda_all'></span></button>";
		html += "<button class='tda_filter_btn " + (_state.filterDimension === 0 ? "tda_filter_active" : "") + "' onclick='TDAAnalysis.setFilter(0)'>H\u2080</button>";
		html += "<button class='tda_filter_btn " + (_state.filterDimension === 1 ? "tda_filter_active" : "") + "' onclick='TDAAnalysis.setFilter(1)'>H\u2081</button>";
		html += "</div>";
		...
		if (!result.layers || result.layers.length === 0) {
			...
			for (var i = 0; i < result.layers.length; i++) {
				html += _renderLayerCard(result.layers[i], _state.mode, i);
			}
		}
	...
	function _injectStyles() {
		if (document.getElementById("tda_styles_v3")) return;
		var style = document.createElement("style");
		style.id =  "tda_styles_v3";
		style.textContent = [
			...
			".tda_card_header_toggle { display: flex; align-items: center; gap: 8px; cursor: pointer; user-select: none; padding: 4px 0; flex-wrap: wrap; }",
			".tda_expand_icon { font-size: 0.7em; opacity: 0.5; }",
			".tda_badge { font-size: 0.72em; opacity: 0.55; background: rgba(128,128,128,0.08); padding: 2px 8px; border-radius: 4px; margin-left: 8px; }",
			...
			".tda_plot_explanation { font-size: 0.72em; opacity: 0.55; margin-bottom: 6px; line-height: 1.4; padding: 5px 8px; background: rgba(52,152,219,0.04); border-radius: 4px; border-left: 3px solid rgba(52,152,219,0.3); }",
			...
			".tda_error_nav { text-align: center; margin-top: 12px; display: flex; gap: 6px; justify-content: center; flex-wrap: wrap; align-items: center; }",
			".tda_no_data { padding: 30px; text-align: center; opacity: 0.4; font-size: 0.95em; }",
			".tda_sliders_panel { border: 1px solid rgba(128,128,128,0.15); border-radius: 8px; padding: 12px; margin-bottom: 14px; background: rgba(128,128,128,0.02); }",
			".tda_sliders_title { font-size: 0.82em; font-weight: 600; margin-bottom: 8px; opacity: 0.7; }",
			".tda_sliders_grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; }",
			".tda_slider_item { display: flex; flex-direction: column; gap: 3px; }",
			".tda_slider_label { font-size: 0.72em; opacity: 0.6; }",
			".tda_slider_val { font-weight: bold; }",
			".tda_slider_input { width: 100%; cursor: pointer; accent-color: #3498db; }",
	...
	return {
		...
		setConfig: function (cfg) {
			if (cfg && typeof cfg === "object") {
				for (var k in cfg) {
					if (_config.hasOwnProperty(k)) _config[k] = cfg[k];
				}
			}
		}
	};
