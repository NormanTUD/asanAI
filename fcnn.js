...
function _point_in_region(px, py, region) {
    try {
        if (region.shape === "circle") {
            var dx = px - region.x;
            var dy = px - region.x; // fix below
            dx = px - region.x;
            var dyy = py - region.y;
            return (dx * dx + dyy * dyy) <= (region.radius * region.radius);
        } else {
            // rectangle
...
function _compute_stats(arr) {
    ...
    return { min, max, avg, std, count, sum };
}

function _make_mini_canvas_data_url(data2d, width, height, maxDisplaySize) {
    // data2d: 2D array [rows][cols], returns a data URL of a small image
    try {
        ...
        // Compute min/max
        var flat = data2d.flat ? data2d.flat() : [].concat(...data2d);
        var mn = Math.min(...flat.filter(v => isFinite(v)));
        var mx = Math.max(...flat.filter(v => isFinite(v)));
        if (mn === mx) mx = mn + 1;

        for (var r = 0; r < rows; r++) {
            for (var col = 0; col < cols; col++) {
                var v = data2d[r][col];
                var norm = Math.floor(((v - mn) / (mx - mn)) * 255);
                norm = Math.max(0, Math.min(255, norm));
                cx.fillStyle = `rgb(${norm},${norm},${norm})`;
        ...
        for (var r = 0; r < rows; r++)
            for (var col = 0; col < cols; col++)
                for (var ch2 = 0; ch2 < 3; ch2++)
                    if (data3d[r] && data3d[r][col]) allVals.push(data3d[r][col][ch2]);
        var mn = Math.min(...allVals.filter(v => isFinite(v)));
        var mx = Math.max(...allVals.filter(v => isFinite(v)));
        if (mn === mx) mx = mn + 1;
        var sc = 255 / (mx - mn);
...
function _build_connection_tooltip_html(region) {
    var parts = [];
    parts.push(`<div style="font-weight:bold;font-size:13px;margin-bottom:4px;">🔗 Connections</div>`);
    parts.push(`<table style="border-collapse:collapse;width:100%;">`);
    ...
    parts.push(row("Position (x, y)", `(${Math.round(region.x)}, ${Math.round(region.y)})`));
    parts.push(row("Area (w × h)", `${region.w} × ${region.h}`));
...
function _bind_fcnn_canvas_mouse_events() {
    ...
    canvas.addEventListener("mousemove", function (e) {
        try {
            ...
            var hit = null;
            // Search in reverse so top-drawn items are found first
            ...
            if (hit) {
                var html = "";
                switch (hit.type) {
                    case "neuron": html = _build_neuron_tooltip_html(hit); break;
                    case "conv2d": html = _build_conv2d_tooltip_html(hit); break;
                    case "flatten": html = _build_flatten_tooltip_html(hit); break;
                    case "layernorm": html = _build_layernorm_tooltip_html(hit); break;
                    case "connection": html = _build_connection_tooltip_html(hit); break;
                    case "input_image": html = _build_input_image_tooltip_html(hit); break;
                    default: html = `<div>Unknown element: ${hit.type}</div>`;
                }
	...
	try {
		...
		if (convLike) {
			...
			_register_fcnn_hit_region({
				...
				weight_stats: weightInfo ? _compute_stats(Array.from(weightInfo.data)) : null
			});
			return;
		}
...
function _make_mini_canvas_data_url(data2d, width, height, maxDisplaySize) {
	// data2d: 2D array [rows][cols], returns a data URL of a small grayscale image
	try {
		if (!data2d || !data2d.length) return null;
		var rows = data2d.length;
		var cols = Array.isArray(data2d[0]) ? data2d[0].length : 0;
...
function _bind_fcnn_canvas_mouse_events() {
	...
	canvas.addEventListener("mousemove", function (e) {
		try {
			...
			// Avoid redundant tooltip rebuilds for same region
	...
	window.addEventListener("resize", function () {
		_hide_fcnn_tooltip();
		_last_hit_idx = -1;
		// Reset binding flag so it rebinds after canvas resize
