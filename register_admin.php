    ...
    if(!admin_exists()) {
        if(array_key_exists("email", $_GET) && ($_GET["email"] != "")) {
            ...
            if(array_key_exists("admin_name", $_GET) && ($_GET["admin_name"] != "")) {
                ...
                if(array_key_exists("password", $_GET) && (strlen($_GET["pw"]) > 7)) {
                    $password = $_GET["pw"];
                    $salt = generateRandomString();
                    $query = "insert into tfd_db.login (username, email, pw, salt, role_id) values (".esc($name).", ".esc($email).", ".esc(hash("sha256", $password.$salt)).", ".$salt.", 1)";
                    if(run_query($query)) {
