    ...
    try {
        ...
        try {
            ...
            try {
                ...
                console.log("track.applyConstraints() result:", applyConstraintsResult);
            } catch (applyErr) {
        ...
        if (!stream_width || !stream_height || stream_width < 2 || stream_height < 2) {
            console.log("Fallback path: create visible small tempVideo and wait for rendered frames (Safari-safe).");
            ...
            usedTempVideo = true;
            console.log("Appended tempVideo to DOM (visible small element).");

            // ensure playback started
            try {
                await tempVideo.play();
                console.log("tempVideo.play() awaited.");
            } catch (playErr) {
            ...
            console.log("After metadata, videoWidth,videoHeight:", tempVideo.videoWidth, tempVideo.videoHeight);
            ...
            console.log("Waiting up to", deadlineMs, "ms for a real frame...");

            if (typeof tempVideo.requestVideoFrameCallback === "function") {
                console.log("Using requestVideoFrameCallback to detect composited frames (preferred).");
                await new Promise(function (resolve) {
                    let resolved = false;
                ...
                console.log("requestVideoFrameCallback not available — using RAF loop + fromPixelsAsync.");
            ...
            if (!capturedTensor) {
                console.log("Trying cam.capture() as last-resort after tempVideo attempts.");
                try {
                    let maybeTensor = await cam.capture();
                    console.log("cam.capture() returned shape (if tensor):", maybeTensor && maybeTensor.shape);
                    if (maybeTensor && maybeTensor.shape && maybeTensor.shape.length === 3 && maybeTensor.shape[0] > 1 && maybeTensor.shape[1] > 1) {
                        ...
                        console.log("cam.capture() produced a useful tensor.");
                    } else {
            ...
            console.log("Direct settings path: trying cam.capture() immediately.");
            try {
                let directTensor = await cam.capture();
                console.log("cam.capture() direct returned shape:", directTensor && directTensor.shape);
        ...
        try {
            ...
            console.log("floatTensor dtype/shape:", floatTensor && floatTensor.dtype, floatTensor && floatTensor.shape);

            arrayResult = array_sync(floatTensor);
            console.log("arrayResult length:", arrayResult && arrayResult.length);

            cam_image = arrayResult[0];
            console.log("cam_image dims (h,w):", cam_image && cam_image.length, cam_image && cam_image[0] && cam_image[0].length);
        } catch (procErr) {
        ...
        if (usedTempVideo && tempVideo && tempVideo.parentNode) {
            try { tempVideo.pause(); tempVideo.srcObject = null; tempVideo.parentNode.removeChild(tempVideo); console.log("Removed tempVideo from DOM."); } catch (remErr) { console.warn("Failed removing tempVideo:", remErr); }
        }
