...
function add_new_category() {
	...
	if (imgDiv.length == 0 || imgDiv.length <= n) {
		...
		$(
			...
			'<button data-rotated="1" style="' + webcam_button_style + '" class="webcam_data_button hide_in_cosmo_mode" onclick="take_image_from_webcam(this)">&#128248; Take image from webcam</button>' +
			...
			'<div id="' + uuid + '"></div>' +
			`<button id='save_button_${uuid}' class='large_button' onclick="add_image_to_category($('#${uuid}_sketcher')[0].toDataURL(), ${label_nr});event.preventDefault();atrament_data['${uuid}_sketcher']['atrament'].clear();">&#128190;</button>`
