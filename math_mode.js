...
function get_loss_equations() {
	return {
		...
		"categoricalCrossentropy": "\\text{Categorical Crossentropy:} -\\frac{1}{n} \\sum_{i=1}^n \\sum_{j=1}^m y_{i,j} \\log\\left(\\hat{y}_{i,j}\\right)",
		"binaryCrossentropy": "\\text{Binary Crossentropy:} -\\frac{1}{n} \\sum_{i=1}^n \\left(y_i \\log\\left(\\hat{y}_i\\right) + (1 - y_i) \\log\\left(1 - \\hat{y}_i\\right)\\right)",
		"meanSquaredLogarithmicError": "\\text{Mean Squared Logarithmic Error:} \\frac{1}{n} \\sum_{i=1}^n \\left(\\log\\left(y_i + 1\\right)- \\log\\left(\\hat{y}_i + 1\\right)\\right)^2",
		"poisson": "\\text{Poisson:} \\frac{1}{n} \\sum_{i=1}^n \\left(\\hat{y}_i - y_i \\log\\left(\\hat{y}_i\\right)\\right)",
		"squaredHinge": "\\text{Squared Hinge:} \\frac{1}{n} \\sum_{i=1}^n \\left(\\max\\left(0, 1 - y_i \\cdot \\hat{y}_i\\right)\\right)^2",
		"logcosh": "\\text{logcosh:} \\frac{1}{n} \\sum_{i=1}^n \\log\\left(\\cosh\\left(\\hat{y}_i - y_i\\right)\\right)",
		"meanAbsolutePercentageError": "\\text{MAPE} = \\frac{1}{n} \\sum_{i=1}^{n} \\left|\\frac{y_i - \\hat{y}_i}{y_i}\\right|",
 ...
 */
function _register_skip_kernel_editables_2d(kernel, gui_layer_idx, skip_proj_layer, decimals, max_rows, max_cols) {
	var all_editables = [];
	...
	return all_editables;
}

function _register_skip_kernel_editables_1d(kernel, gui_layer_idx, skip_proj_layer, decimals, max_rows) {
	var all_editables = [];
...
function _build_skip_kernel_latex(kernel, gui_layer_idx, max_rows, max_cols, decimals, is_2d) {
...
function _render_skip_projection_editable(container, skip_proj_layer, layer_idx, gui_layer_idx, input_layer, layer_data, colors) {
	...
	var all_editables = is_2d
		? _register_skip_kernel_editables_2d(kernel, gui_layer_idx, skip_proj_layer, decimals, max_rows, max_cols)
		: _register_skip_kernel_editables_1d(kernel, gui_layer_idx, skip_proj_layer, decimals, max_rows);

	var skip_input_latex = _get_skip_input_full_latex(layer_idx, input_layer, layer_data, colors);
	var skip_kernel_latex = _build_skip_kernel_latex(kernel, gui_layer_idx, max_rows, max_cols, decimals, is_2d);
...
function wrap_with_activation_function (layer_idx, layer_str) {
	...
	if(activation_name != "linear") {
		return "\\mathrm{\\underbrace{" + activation_name + "}_{\\mathrm{Activation}}}\\left(" + layer_str + "\\right)";
	}
...
/**
 * Render a single complete LaTeX string, then post-process to make
