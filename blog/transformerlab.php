...
The results are a list of Tensors containing a list of matrices which place the neural network somewhere in the Embedding space, they are just appended. You can think of it like this. The Tensor that's passend to the next Layer, the **Feed Forward Neural Network**, called $W_\text{FFN}$, is a concatinated list of Tensors like $\left[A_\text{Word Endings}, B_\text{Temporal Tense}, C_\text{Male or Female}, \dots\right]$, each, $A, B$ and $C$ being the result of a trained attention head. This, similiar to the idea of the Tug-of-War that places the vector depending on the context, Transformers do it for whole sentences. What comes out of this *Neural Network* is a new vector with as many dimensions as the *Embedding vector space* has.

(TODO This is then layer-normalized to not be massively larger, maybe also? before)

The vector coming from the $W_\text{FFN}$ is then compared in angle to every other word in the Embedding space. As similiarity is defined by angle, the vector that comes out, where every token is identified by position as a **One-Hot-Encoded** vector thas shows, for each word, how similiar it is. This is then passend through **SoftMax** to convert it into a **Probability Distribution**.
