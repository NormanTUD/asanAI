...
// ═══════════════════════════════════════════════════════════════════
// FFN Manifold Visualization System
// Computes and renders the deformation field, activation regions,
// and Jacobian analysis of the FFN at each layer.
// Lazily rendered via IntersectionObserver, updated on training.
// ═══════════════════════════════════════════════════════════════════

window.ffnManifold = (function () {
    "use strict";

    // ── State ──
    let _mode = 'field';          // 'field' | 'regions' | 'jacobian'
    ...
    function init() {
        const container = document.getElementById('ffn-manifold-plot');
        if (container) {
            _observer.observe(container);
        }
    }

    // ── Public: schedule a lazy update (called from training loop, config changes, etc.) ──
    ...
    // ── Public: switch visualization mode ──
    function setMode(mode) {
        _mode = mode;
        const modes = ['field', 'regions', 'jacobian'];
        modes.forEach(m => {
            const btn = document.getElementById(`ffn-manifold-mode-${m}`);
            if (btn) {
                btn.style.background = (m === mode) ? '#6366f1' : 'white';
                btn.style.color = (m === mode) ? 'white' : '#6366f1';
            }
        });
        scheduleUpdate();
    }
    ...
    function _syncLayerSelector() {
        const sel = document.getElementById('ffn-manifold-layer-select');
        if (!sel || !window.currentWeights) return;
        const nLayers = window.currentWeights.length;
        const currentVal = parseInt(sel.value) || 0;

        if (sel.options.length !== nLayers) {
            sel.innerHTML = '';
        ...
        sel.value = Math.min(currentVal, nLayers - 1);
    }

    // ── Core: compute FFN output for a single point ──
    function _ffnForward(point, layerWeights) {
        const d_model = point.length;
        const gamma2 = layerWeights.gamma2 || new Array(d_model).fill(1.0);
        const beta2 = layerWeights.beta2 || new Array(d_model).fill(0.0);

        // Pre-LN (single vector version)
        const mean = point.reduce((a, b) => a + b, 0) / d_model;
        const variance = point.reduce((a, b) => a + (b - mean) ** 2, 0) / d_model;
        const normed = point.map((v, i) => {
            const std = Math.sqrt(variance + epsilon);
            return ((v - mean) / std) * gamma2[i] + beta2[i];
        });

        // Layer 1: normed * W1 + b1, then ReLU
        const W1 = layerWeights.W1;
        const b1 = layerWeights.b1;
        const d_ff = b1.length;
        const hidden = new Array(d_ff);
        const activationMask = new Array(d_ff);
        ...
        for (let j = 0; j < d_ff; j++) {
            ...
            activationMask[j] = sum > 0 ? 1 : 0;
        }

        // Layer 2: hidden * W2 + b2
        const W2 = layerWeights.W2;
        const b2 = layerWeights.b2;
        const output = new Array(d_model);
        ...
        // Residual: h2 = point + FFN(normed(point))
        const h2 = point.map((v, i) => v + output[i]);

        return { h2, activationMask, hidden, normed, output };
    }

    // ── Compute Jacobian singular values at a point ──
    function _computeJacobianSVD(point, layerWeights) {
        const d_model = point.length;
        const eps = 1e-5;
        const jacobian = [];
        ...
        for (let i = 0; i < d_model; i++) {
            ...
            for (let j = 0; j < d_model; j++) {
                ...
                pPlus[j] += eps;
                pMinus[j] -= eps;
                const fPlus = _ffnForward(pPlus, layerWeights).h2;
                const fMinus = _ffnForward(pMinus, layerWeights).h2;
                row.push((fPlus[i] - fMinus[i]) / (2 * eps));
            }
            jacobian.push(row);
        }

        // Compute singular values via J^T * J eigenvalues (for small matrices)
        // For d_model <= 16 this is fine
        const JtJ = [];
        for (let i = 0; i < d_model; i++) {
            JtJ.push([]);
            for (let j = 0; j < d_model; j++) {
        ...
        // Power iteration for top singular value (fast approximation)
        let v = new Array(d_model).fill(1 / Math.sqrt(d_model));
        for (let iter = 0; iter < 20; iter++) {
            const Av = new Array(d_model).fill(0);
            for (let i = 0; i < d_model; i++) {
                for (let j = 0; j < d_model; j++) {
                    Av[i] += JtJ[i][j] * v[j];
                }
        ...
        // Rayleigh quotient for top eigenvalue of J^T*J
        const Av = new Array(d_model).fill(0);
        for (let i = 0; i < d_model; i++) {
            for (let j = 0; j < d_model; j++) {
                Av[i] += JtJ[i][j] * v[j];
        ...
        const topEigenvalue = v.reduce((s, vi, i) => s + vi * Av[i], 0);
        const topSingularValue = Math.sqrt(Math.max(0, topEigenvalue));

        // Trace gives sum of squared singular values
        let trace = 0;
        for (let i = 0; i < d_model; i++) trace += JtJ[i][i];
        const frobeniusNorm = Math.sqrt(Math.max(0, trace));

        // Determinant approximation via diagonal product (rough)
        let detApprox = 1;
        for (let i = 0; i < d_model; i++) {
            detApprox *= Math.max(1e-12, JtJ[i][i]);
        }
        detApprox = Math.pow(Math.abs(detApprox), 0.5 / d_model); // geometric mean of singular values

        return {
            topSingularValue,
            frobeniusNorm,
            conditionApprox: topSingularValue / Math.max(1e-12, detApprox),
            jacobian
        };
    }

    // ── Convert activation mask to a hash string for region coloring ──
    function _maskToHash(mask) {
        // Pack bits into a string
        let hash = 0;
        for (let i = 0; i < mask.length; i++) {
            hash = ((hash << 1) | mask[i]) >>> 0;
            if (i % 30 === 29) hash = (hash * 2654435761) >>> 0; // mix to avoid overflow
        }
        return hash;
    }

    function _hashToColor(hash) {
    ...
    // ── Generate sample grid around token positions ──
    function _generateGrid(tokenPositions, d_model, resolution, scale) {
        if (tokenPositions.length === 0) return [];

        // Compute bounding box of token positions
        const mins = new Array(d_model).fill(Infinity);
        const maxs = new Array(d_model).fill(-Infinity);
        ...
        const center = mins.map((mn, d) => (mn + maxs[d]) / 2);
        const halfSpan = mins.map((mn, d) => Math.max(1.0, (maxs[d] - mn) / 2) * scale);
        ...
        if (d_model === 1) {
            for (let i = 0; i < resolution; i++) {
                ...
                points.push([center[0] + halfSpan[0] * (2 * t - 1)]);
            }
        } else if (d_model === 2) {
            for (let i = 0; i < resolution; i++) {
                for (let j = 0; j < resolution; j++) {
                    const ti = i / (resolution - 1);
                    const tj = j / (resolution - 1);
                    points.push([
                        center[0] + halfSpan[0] * (2 * ti - 1),
                        center[1] + halfSpan[1] * (2 * tj - 1)
                    ]);
                }
            }
        } else if (d_model === 3) {
                ...
                for (let j = 0; j < resolution; j++) {
                    for (let k = 0; k < resolution; k++) {
                        ...
                        points.push([
                            center[0] + halfSpan[0] * (2 * ti - 1),
                            center[1] + halfSpan[1] * (2 * tj - 1),
                            center[2] + halfSpan[2] * (2 * tk - 1)
                        ]);
                    }
                }
            }
        } else {
            // High-dim: sample random points in the bounding box
            const nSamples = Math.min(resolution * resolution, 200);
            for (let s = 0; s < nSamples; s++) {
                const pt = [];
                for (let d = 0; d < d_model; d++) {
                    pt.push(center[d] + halfSpan[d] * (2 * Math.random() - 1));
                }
                points.push(pt);
            }
        }
    ...
    // ── Get current token h1 positions (input to FFN) ──
    function _getTokenH1Positions(layerIdx) {
        const config = getTransformerConfig();
        const { d_model, n_heads } = config;
        const weights = window.currentWeights;
        if (!weights || !weights[layerIdx]) return [];

        const vizMode = window.tlabVisualizationMode || 'train';
        const trainingInput = document.getElementById('transformer-training-data');
        const masterInput = document.getElementById('transformer-master-token-input');
        const sourceValue = (vizMode === 'inference') ? masterInput.value : trainingInput.value;
        const tokens = transformer_tokenize_render(sourceValue, null);
        const vocabulary = [...new Set(transformer_tokenize_render(trainingInput.value, null))];
        const knownTokens = tokens.filter(t => vocabulary.includes(t));

        if (knownTokens.length === 0) return [];

        // Run forward pass up to the target layer to get h1
        let h = embedTokensWithPE(knownTokens, d_model);
        for (let l = 0; l <= layerIdx; l++) {
            const normH = calculateLayerNorm(h, weights[l].gamma, weights[l].beta);
            const headData = causalMultiHeadAttention(normH, weights[l].attention, d_model, n_heads);
            const concat = h.map((_, tIdx) =>
                [].concat(...headData.map(hd => hd.context[tIdx]))
            );
            const projected = matMul(concat, weights[l].attention.output);
            h = matAdd(h, projected);

            if (l < layerIdx) {
                // Full layer: also apply FFN
                h = run_ffn_block(h, weights[l], true, l);
            }
            // At target layer, h is now h1 (after attention, before FFN)
        }

        return { positions: h, tokenLabels: knownTokens };
    }

    // ── Compute a hash of current state to avoid redundant renders ──
    function _computeRenderHash() {
        const sel = document.getElementById('ffn-manifold-layer-select');
        const res = document.getElementById('ffn-manifold-resolution');
        const scl = document.getElementById('ffn-manifold-scale');
        const layerIdx = sel ? parseInt(sel.value) || 0 : 0;
        const resolution = res ? parseInt(res.value) || 8 : 8;
        const scale = scl ? parseFloat(scl.value) || 2.0 : 2.0;

        const weights = window.currentWeights;
        if (!weights || !weights[layerIdx]) return null;

        // Quick hash of FFN weights
        const lw = weights[layerIdx];
        let wHash = 0;
        if (lw.W1 && lw.W1[0]) {
            for (let i = 0; i < Math.min(lw.W1.length, 4); i++) {
                for (let j = 0; j < Math.min(lw.W1[0].length, 4); j++) {
                    wHash = (wHash * 31 + (lw.W1[i][j] * 10000 | 0)) | 0;
                }
            }
        }

        return `${_mode}-${layerIdx}-${resolution}-${scale}-${wHash}-${window.last_d_model}`;
    }

    // ══════════════════════════════════════════════════════
    // RENDER: Vector Field Mode
    // ══════════════════════════════════════════════════════
    function _renderVectorField(gridPoints, layerWeights, tokenData, d_model) {
        const traces = [];
        const displacements = [];

        // Compute FFN displacement for each grid point
        gridPoints.forEach(pt => {
            const result = _ffnForward(pt, layerWeights);
            const displacement = result.h2.map((v, i) => v - pt[i]);
            const magnitude = Math.sqrt(displacement.reduce((s, v) => s + v * v, 0));
            displacements.push({ pt, displacement, magnitude, h2: result.h2 });
        });

        const maxMag = Math.max(...displacements.map(d => d.magnitude), 1e-6);

        if (d_model === 3) {
            // 3D cone plot for vector field
            const x = [], y = [], z = [], u = [], v = [], w = [];
            const colors = [];
            ...
            displacements.forEach(d => {
                ...
                y.push(d.pt[1]);
                z.push(d.pt[2]);
                u.push(d.displacement[0]);
                v.push(d.displacement[1]);
                w.push(d.displacement[2]);
                colors.push(d.magnitude / maxMag);
            });
            ...
            traces.push({
                ...
                anchor: 'tail',
                colorscale: 'Viridis',
                colorbar: { title: '|Δ|', thickness: 15, len: 0.6 },
                opacity: 0.7,
                name: 'FFN Displacement',
                hovertemplate: 'Δ = (%{u:.3f}, %{v:.3f}, %{w:.3f})<br>|Δ| = %{customdata:.4f}<extra></extra>',
                customdata: displacements.map(d => d.magnitude)
            });

            // Token markers: before FFN
            if (tokenData.positions) {
                traces.push({
                    type: 'scatter3d',
                    x: tokenData.positions.map(p => p[0]),
                    y: tokenData.positions.map(p => p[1]),
                    z: tokenData.positions.map(p => p[2]),
                    mode: 'markers+text',
                    text: tokenData.tokenLabels,
                    textposition: 'top center',
                    marker: { size: 6, color: '#ef4444', symbol: 'circle', line: { width: 1, color: '#000' } },
                    name: 'Tokens (h₁)',
                    hovertemplate: '<b>%{text}</b> (before FFN)<extra></extra>'
                });

                // Token markers: after FFN
                const afterPositions = tokenData.positions.map(p => _ffnForward(p, layerWeights).h2);
                traces.push({
                    type: 'scatter3d',
                    x: afterPositions.map(p => p[0]),
                    y: afterPositions.map(p => p[1]),
                    z: afterPositions.map(p => p[2]),
                    mode: 'markers+text',
                    text: tokenData.tokenLabels,
                    textposition: 'bottom center',
                    marker: { size: 6, color: '#22c55e', symbol: 'diamond', line: { width: 1, color: '#000' } },
                    name: 'Tokens (h₂)',
                    hovertemplate: '<b>%{text}</b> (after FFN)<extra></extra>'
                });

                // Lines connecting before → after
                tokenData.positions.forEach((p, i) => {
                    const a = afterPositions[i];
                    traces.push({
                        type: 'scatter3d',
                        x: [p[0], a[0]], y: [p[1], a[1]], z: [p[2], a[2]],
                        mode: 'lines',
                        line: { color: getPositionColor(i, tokenData.positions.length), width: 4 },
                        showlegend: false,
                        hoverinfo: 'skip'
                    });
                });
            ...
            // 2D quiver-style arrows
            displacements.forEach((d, idx) => {
                const normMag = d.magnitude / maxMag;
                const r = Math.round(68 + normMag * 187);
                const g = Math.round(130 - normMag * 80);
                const b = Math.round(246 - normMag * 200);
                const color = `rgb(${r},${g},${b})`;

                traces.push({
                    type: 'scatter',
                    x: [d.pt[0], d.pt[0] + d.displacement[0]],
                    y: [d.pt[1], d.pt[1] + d.displacement[1]],
                    mode: 'lines+markers',
                    line: { width: 1.5, color },
                    marker: { size: [0, 6], symbol: 'arrow', angleref: 'previous', color },
                    showlegend: false,
                    hovertemplate: `Δ = (${d.displacement[0].toFixed(3)}, ${d.displacement[1].toFixed(3)})<br>|Δ| = ${d.magnitude.toFixed(4)}<extra></extra>`
                });
            });
            ...
            if (tokenData.positions) {
                traces.push({
                    ...
                    text: tokenData.tokenLabels,
                    textposition: 'top center',
                    marker: { size: 10, color: '#ef4444', symbol: 'circle', line: { width: 2, color: '#000' } },
                    name: 'Tokens (h₁)'
                });
                ...
                traces.push({
                    ...
                    text: tokenData.tokenLabels,
                    textposition: 'bottom center',
                    marker: { size: 10, color: '#22c55e', symbol: 'diamond', line: { width: 2, color: '#000' } },
                    name: 'Tokens (h₂)'
                });
            }
        } else {
            // High-dim: show displacement magnitudes as a bar chart per sample
            const mags = displacements.map((d, i) => ({ idx: i, mag: d.magnitude }));
            mags.sort((a, b) => b.mag - a.mag);
            const topN = mags.slice(0, 50);

            traces.push({
                type: 'bar',
                x: topN.map(m => m.idx),
                y: topN.map(m => m.mag),
                marker: { color: topN.map(m => m.mag), colorscale: 'Viridis' },
    ...
    // ══════════════════════════════════════════════════════
    // RENDER: Activation Regions Mode
    // ══════════════════════════════════════════════════════
    function _renderActivationRegions(gridPoints, layerWeights, tokenData, d_model) {
        const traces = [];
        const regionMap = new Map(); // hash → count

        const results = gridPoints.map(pt => {
            const r = _ffnForward(pt, layerWeights);
            const hash = _maskToHash(r.activationMask);
            regionMap.set(hash, (regionMap.get(hash) || 0) + 1);
            return { pt, hash, activationMask: r.activationMask, h2: r.h2 };
        });

        const uniqueRegions = regionMap.size;

        if (d_model === 3) {
            // 3D: color each grid point by its activation region
            const x = [], y = [], z = [], colors = [], hoverTexts = [];

            results.forEach(r => {
                x.push(r.pt[0]);
                y.push(r.pt[1]);
                z.push(r.pt[2]);
                colors.push(_hashToColor(r.hash));
                const activeCount = r.activationMask.reduce((a, b) => a + b, 0);
                hoverTexts.push(`Region hash: ${r.hash}<br>Active neurons: ${activeCount}/${r.activationMask.length}`);
            });
            ...
            traces.push({
                ...
                hoverinfo: 'text',
                text: hoverTexts,
                name: `Activation Regions (${uniqueRegions} unique)`,
                showlegend: true
            });
            ...
            if (tokenData.positions) {
                traces.push({
                    ...
                    marker: { size: 7, color: '#ef4444', symbol: 'circle', line: { width: 2, color: '#000' } },
                    name: 'Tokens (h₁)',
                    hovertemplate: '<b>%{text}</b><extra></extra>'
                });
            }

            // Draw region boundaries: find pairs of adjacent grid points with different hashes
            // and draw thin lines between them to show the "folds"
            const boundaryX = [], boundaryY = [], boundaryZ = [];
            for (let i = 0; i < results.length; i++) {
                for (let j = i + 1; j < results.length; j++) {
                    if (results[i].hash !== results[j].hash) {
                        const dist = Math.sqrt(
                            results[i].pt.reduce((s, v, d) => s + (v - results[j].pt[d]) ** 2, 0)
                        );
                        // Only connect nearby points (adjacent in grid)
                        const resolution = parseInt(document.getElementById('ffn-manifold-resolution')?.value) || 8;
                        const scale = parseFloat(document.getElementById('ffn-manifold-scale')?.value) || 2.0;
                        const gridSpacing = (scale * 4) / (resolution - 1);
                        if (dist < gridSpacing * 1.8) {
                            boundaryX.push(results[i].pt[0], results[j].pt[0], null);
                            boundaryY.push(results[i].pt[1], results[j].pt[1], null);
                            boundaryZ.push(results[i].pt[2], results[j].pt[2], null);
                        }
                    }
                }
            }

            if (boundaryX.length > 0) {
                traces.push({
                    type: 'scatter3d',
                    x: boundaryX, y: boundaryY, z: boundaryZ,
                    mode: 'lines',
                    line: { color: 'rgba(239, 68, 68, 0.4)', width: 2 },
                    name: 'Region Boundaries (ReLU folds)',
                    hoverinfo: 'skip'
                });
            }

        } else if (d_model === 2) {
            // 2D: scatter with region colors
            const x = [], y = [], colors = [], hoverTexts = [];
            ...
            results.forEach(r => {
                ...
                colors.push(_hashToColor(r.hash));
                const activeCount = r.activationMask.reduce((a, b) => a + b, 0);
                hoverTexts.push(`Region: ${r.hash}<br>Active: ${activeCount}/${r.activationMask.length}`);
            });
            ...
            traces.push({
                ...
                x, y,
                mode: 'markers',
                marker: { size: 8, color: colors, opacity: 0.6 },
                text: hoverTexts,
                hoverinfo: 'text',
                name: `Activation Regions (${uniqueRegions} unique)`
            });
            ...
            if (tokenData.positions) {
                traces.push({
                    ...
                    text: tokenData.tokenLabels,
                    textposition: 'top center',
                    marker: { size: 12, color: '#ef4444', symbol: 'circle', line: { width: 2, color: '#000' } },
                    name: 'Tokens (h₁)'
                });
            }
        } else {
            // High-dim: bar chart of region population
            ...
            traces.push({
                type: 'bar',
                x: regionCounts.map((_, i) => `Region ${i + 1}`),
                y: regionCounts.map(r => r[1]),
                marker: { color: regionCounts.map(r => _hashToColor(r[0])) },
                name: 'Points per Region'
            });
        }
    ...
    // ══════════════════════════════════════════════════════
    // RENDER: Jacobian Stretch Mode
    // ══════════════════════════════════════════════════════
    function _renderJacobianStretch(gridPoints, layerWeights, tokenData, d_model) {
        const traces = [];

        // Subsample for Jacobian (expensive)
        const maxJacobianPoints = Math.min(gridPoints.length, 150);
        const step = Math.max(1, Math.floor(gridPoints.length / maxJacobianPoints));
        const sampledPoints = [];
        for (let i = 0; i < gridPoints.length; i += step) {
            sampledPoints.push(gridPoints[i]);
        }

        const jacobianResults = sampledPoints.map(pt => {
            const svd = _computeJacobianSVD(pt, layerWeights);
            return { pt, ...svd };
        });

        const maxSV = Math.max(...jacobianResults.map(r => r.topSingularValue), 1e-6);

        if (d_model === 3) {
            const x = [], y = [], z = [], colors = [], sizes = [], hoverTexts = [];

            jacobianResults.forEach(r => {
                x.push(r.pt[0]);
                y.push(r.pt[1]);
                z.push(r.pt[2]);
                const normSV = r.topSingularValue / maxSV;
                colors.push(normSV);
                sizes.push(3 + normSV * 8);
                hoverTexts.push(
                    `σ₁ = ${r.topSingularValue.toFixed(4)}<br>` +
                    `‖J‖_F = ${r.frobeniusNorm.toFixed(4)}<br>` +
                    `Condition ≈ ${r.conditionApprox.toFixed(2)}`
                );
            });
            ...
            traces.push({
                ...
                marker: {
                    ...
                    colorscale: 'RdYlGn',
                    reversescale: true,
                    colorbar: { title: 'σ₁ (stretch)', thickness: 15, len: 0.6 },
                    opacity: 0.8,
                    line: { width: 0.5, color: '#333' }
                },
                text: hoverTexts,
                hoverinfo: 'text',
                name: 'Jacobian Top Singular Value'
            });

            // Token markers
            if (tokenData.positions) {
                // Compute Jacobian at token positions too
                const tokenJacobians = tokenData.positions.map(p => _computeJacobianSVD(p, layerWeights));
                ...
                traces.push({
                    ...
                    z: tokenData.positions.map(p => p[2]),
                    mode: 'markers+text',
                    text: tokenData.tokenLabels.map((label, i) =>
                        `${label} (σ₁=${tokenJacobians[i].topSingularValue.toFixed(3)})`
                    ),
                    textposition: 'top center',
                    marker: {
                        size: 8,
                    ...
                    name: 'Tokens (with Jacobian)',
                    hovertemplate: '<b>%{text}</b><extra></extra>'
                });
            }

        } else if (d_model === 2) {
            const x = [], y = [], colors = [], sizes = [], hoverTexts = [];

            jacobianResults.forEach(r => {
                x.push(r.pt[0]);
                y.push(r.pt[1]);
                const normSV = r.topSingularValue / maxSV;
                colors.push(r.topSingularValue);
                sizes.push(6 + normSV * 14);
                hoverTexts.push(
                    `σ₁ = ${r.topSingularValue.toFixed(4)}<br>` +
                    `‖J‖_F = ${r.frobeniusNorm.toFixed(4)}`
                );
            });
            ...
            if (tokenData.positions) {
                traces.push({
                    ...
                    text: tokenData.tokenLabels,
                    textposition: 'top center',
                    marker: { size: 12, color: '#ef4444', symbol: 'diamond', line: { width: 2, color: '#000' } },
                    name: 'Tokens (h₁)'
                });
            }
        } else {
            // High-dim: histogram of singular values
            const svs = jacobianResults.map(r => r.topSingularValue);
            traces.push({
                type: 'histogram',
                x: svs,
                nbinsx: 30,
                marker: { color: '#6366f1' },
                name: 'Distribution of σ₁'
            });
        }
    ...
    // ══════════════════════════════════════════════════════
    function _executeRender() {
        const plotDiv = document.getElementById('ffn-manifold-plot');
        const statsDiv = document.getElementById('ffn-manifold-stats');
        if (!plotDiv) return;
        ...
        // Check for redundant render
        const hash = _computeRenderHash();
        if (hash === _lastRenderHash) return;
        _lastRenderHash = hash;

        const weights = window.currentWeights;
        if (!weights || !weights[layerIdx]) {
            plotDiv.innerHTML = '<div style="padding:40px; text-align:center; color:#94a3b8;">No weights available. Initialize or train the model first.</div>';
            return;
        }
        ...
        // Get token positions (h1 = after attention, before FFN)
        let tokenData = { positions: null, tokenLabels: [] };
        try {
            const td = _getTokenH1Positions(layerIdx);
            if (td && td.positions && td.positions.length > 0) {
                tokenData = td;
            }
        } catch (e) {
            console.warn('FFN Manifold: Could not get token positions:', e);
        }

        // Generate grid
        const defaultPositions = tokenData.positions || [new Array(d_model).fill(0)];
        const gridPoints = _generateGrid(defaultPositions, d_model, resolution, scale);

        if (gridPoints.length === 0) {
            plotDiv.innerHTML = '<div style="padding:40px; text-align:center; color:#94a3b8;">No grid points generated.</div>';
            return;
        }
        ...
        switch (_mode) {
            case 'field':
                traces = _renderVectorField(gridPoints, layerWeights, tokenData, d_model);
                break;
            case 'regions':
                traces = _renderActivationRegions(gridPoints, layerWeights, tokenData, d_model);
                break;
            case 'jacobian':
                traces = _renderJacobianStretch(gridPoints, layerWeights, tokenData, d_model);
                break;
        }
        ...
        const layout = {
            title: {
                text: `FFN Manifold — Layer ${layerIdx + 1} (${_mode})`,
        ...
        if (d_model === 3) {
            ...
            layout.xaxis = { title: 'Dim 0' };
            layout.yaxis = { title: d_model >= 2 ? 'Dim 1' : 'Value' };
        }

        Plotly.react('ffn-manifold-plot', traces, layout, { responsive: true });

        // Stats
        if (statsDiv) {
            const d_ff = layerWeights.b1 ? layerWeights.b1.length : d_model * 4;

            // Compute stats at token positions
            let tokenStats = '';
            if (tokenData.positions) {
                const tokenAnalysis = tokenData.positions.map((pos, i) => {
                    const result = _ffnForward(pos, layerWeights);
                    const activeCount = result.activationMask.reduce((a, b) => a + b, 0);
                    const displacement = result.h2.map((v, d) => v - pos[d]);
                    const magnitude = Math.sqrt(displacement.reduce((s, v) => s + v * v, 0));
                    return {
                        label: tokenData.tokenLabels[i],
                ...
                tokenStats = tokenAnalysis.map(t =>
                    `<b>${t.label}</b>: ${t.activeNeurons}/${t.totalNeurons} neurons active, |Δ|=${t.displacement.toFixed(4)}`
                ).join(' &nbsp;|&nbsp; ');
            }

            statsDiv.innerHTML = `
                <b>Mode:</b> ${_mode} &nbsp;|&nbsp;
                <b>Layer:</b> ${layerIdx + 1} &nbsp;|&nbsp;
                <b>d_model:</b> ${d_model} &nbsp;|&nbsp;
                <b>d_ff:</b> ${d_ff} &nbsp;|&nbsp;
                <b>Grid points:</b> ${gridPoints.length} &nbsp;|&nbsp;
                <b>Render time:</b> ${renderTime.toFixed(0)}ms
                ${tokenStats ? '<br><b>Per-token:</b> ' + tokenStats : ''}
            `;
        }
    }
    ...
    return {
        ...
        scheduleUpdate,
        setMode,
        _executeRender  // exposed for debugging
    };
