...
// ============================================================
// MODULE ENTRY — lazy-mount the wave canvas when it scrolls into view
// ============================================================
async function loadInterferencePatternModule() {
    if (typeof updateLoadingStatus === 'function') updateLoadingStatus("Loading section about the Interference Pattern of Layers...");
    (function () {
        var reg = [], obs = null;
        function regEl(id, fn) { var el = document.getElementById(id); if (!el) return; reg.push({ el: el, fn: fn, done: false }); }
        function startObs() {
            if (obs) return;
            var rm = (typeof rootMargin !== 'undefined') ? rootMargin : '0px';
            obs = new IntersectionObserver(function (entries) {
                entries.forEach(function (entry) {
                    if (!entry.isIntersecting) return;
                    var m = reg.find(function (r) { return r.el === entry.target; });
                    if (m && !m.done) { m.done = true; obs.unobserve(m.el); m.fn(); }
                });
            }, { rootMargin: rm });
            reg.forEach(function (r) { if (!r.done) obs.observe(r.el); });
        }
        regEl('canvas-layer-wave', function () { initLayerWave(); });
        startObs();
        if (window.__MN_DARK) {
            window.__MN_DARK.onChange(function () { reg.forEach(function (r) { if (r.done) { try { r.fn(); } catch (e) {} } }); });
        }
    })();
