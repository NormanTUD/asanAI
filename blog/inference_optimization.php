...
* **GPTQ** (Frantar et al., 2022): post-training quantization using second-order information. The de facto standard.
* **AWQ** (Lin et al., 2023): activation-aware weight quantization. Identifies salient weight channels and preserves them at higher precision.
* **SmoothQuant** (Xiao et al., 2022): migrates quantization difficulty from activations to weights.
* **BitsAndBytes** (Dettmers et al., 2022): k-bit quantization for PyTorch (popular for QLoRA).
* **GGUF** (llama.cpp): many quantization schemes in a single file (Q2_K, Q3_K, Q4_K_M, Q5_K_M, Q6_K, Q8_0).
* **BitNet** (Ma et al., 2024): 1.58-bit ternary quantization. Surprisingly competitive at scale.
* **QuIP#** (Chee et al., 2024): lattice-based 2-bit quantization with incoherence processing.
...
* **Grouped-Query Attention (GQA)** (Ainslie et al., 2023): middle ground — 8 KV heads for 64 query heads gives 8× reduction with quality close to MHA. Used by Llama 2/3, Mistral, Qwen.
* **Sliding Window Attention** (Mistral, Beltagy et al., 2020): only attend to the last $w$ tokens. KV memory is $O(w)$ instead of $O(n)$. Combine with a few "global" attention layers to preserve long-range context.
...
* **Medusa** (Cai et al., 2024): parallel draft heads attached to the main model.
* **EAGLE** (Li et al., 2024): draft at the feature level, not the token level. Higher acceptance.
