...
// Tracking, was gerade im Sichtfeld ist
const visibleBlocks = { lin: true, deep: true }; 
...
function updateVisuals(id, force = false) {
	// Nur rendern wenn sichtbar ODER wenn force=true (beim Laden)
	if (!visibleBlocks[id] && !force) return;
	...
	if(chartEl) {
		const layout = {
			...
			xaxis: { title: 'Epoche', autorange: true },
			yaxis: { title: 'Loss', autorange: true, type: 'log' },
			autosize: true,
			uirevision: 'true'
		};
		const epochsX = c.loss.map((_, index) => index);
		Plotly.react(id+'-loss-chart', [{
			x: epochsX, y: c.loss, type: 'scatter',
			line: {color: '#ef4444', width: 2}, name: 'MSE'
...
async function toggleTraining(id) {
	...
	const epEl = document.getElementById(id+'-epochs');
	const epochs = epEl ? parseInt(epEl.value) : 100;
...
function plotLinData(force = false) {
	if(!visibleBlocks.lin && !force) return;
	const c = configs.lin;
	...
	Plotly.react('lin-data-chart', [
		...
		{x:tx, y:Array.from(py), mode:'lines', name:'Pred'}
	], {margin:{t:30,b:30,l:30,r:10}, title: 'Regression', autosize:true}, {responsive:true});
}

function plotDeepData(force = false) {
	if(!visibleBlocks.deep && !force) return;
	const c = configs.deep;
	const chartEl = document.getElementById('deep-data-chart');
	if(!chartEl) return;

	if(c.inputs.length !== 2) {
		chartEl.innerHTML = "<br><center>Visualisierung nur für 2 Inputs verfügbar</center>";
		return;
	}
	...
	tf.tidy(() => {
		...
		Plotly.react('deep-data-chart', [
			...
			{ x: x1, y: x2, mode: 'markers+text', text: y_true, textposition: 'top center', marker: { size: 12, color: y_true, colorscale: [[0, 'red'], [1, 'blue']], line: {color: 'black', width: 2} } }
...
// Observer Setup: Erkennt die Sichtbarkeit der Charts
function setupObservers() {
	const observer = new IntersectionObserver((entries) => {
		entries.forEach(entry => {
			// Wir prüfen, welcher Chart gerade sichtbar wird
			const isLin = entry.target.id.includes('lin');
			visibleBlocks[isLin ? 'lin' : 'deep'] = entry.isIntersecting;
			
			// Wenn er sichtbar wird, sofort ein Update erzwingen
			if(entry.isIntersecting) {
				updateVisuals(isLin ? 'lin' : 'deep');
			}
		});
	}, { threshold: 0.1 });

	// Beobachte die Chart-Container (diese existieren sicher im HTML)
