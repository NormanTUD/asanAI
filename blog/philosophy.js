...
const SheafViz = (() => {
	...
	function init() {
		...
		if (!gl) {
			log('WebGL not available — skipping 3D visualization');
			el.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;min-height:400px;background:#f8f9fa;border-radius:12px;border:2px dashed #ccc;">' +
				'<p style="text-align:center;padding:2em;color:#999;font-size:1.1em;">' +
				'🌐 3D visualization requires WebGL<br><span style="font-size:0.85em;">Your browser or environment does not support WebGL rendering.</span></p></div>';
			return;
		}
