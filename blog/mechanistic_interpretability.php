...
The "aha-moment": the FFN doesn't "compute" in the traditional sense — it **retrieves**. Each FFN layer is a soft associative memory with $d_{\text{ff}}$ slots (typically $4 \times d_{\text{model}}$, so ~3,072 to ~16,384 slots per layer). Across 96 layers, a large Transformer has access to roughly 300,000 to 1.5 million memory slots. When the model "knows" that Paris is the capital of France, that fact is stored as a key-value pair in one or more FFN layers: the key activates when the input pattern matches "capital of France," and the value pushes the residual stream toward the "Paris" direction. This is why knowledge editing (changing a single fact in a trained model) is possible: you just need to find and modify the relevant key-value pair in the FFN.
