...
When $N\le d$ it saturates at $2^N$: *every* dichotomy is separable (a $d$-weight classifier has VC dimension $d$; a bias adds one, up to $d+1$ points). Once $N$ outnumbers $d$, only a fraction $C(N,d)/2^N$ is separable — but for fixed $N$ that fraction **rises with $d$**, tending to $1$. Cover's point: a complex problem, cast *nonlinearly* in a high-dimensional, not-too-dense space, is more likely to be linearly separable than in a low-dimensional one. A layer wider than the data opens exactly the dimensions that push $d$ past the entanglement threshold.
...
for a universal positive constant $c$ — the curse of dimensionality in its benign form. A fold opened along an unoccupied direction is, almost surely, independent of every existing data direction. The unused dimensions are not merely empty but *orthogonal* space, so the fold genuinely separates rather than shuffles. That is why the anvil has room to fold.
...
The $\log N$ bound is **tight**. Read it both ways: (i) most high dimensions can be dropped while the geometry survives — which is why an embedding can be compact; (ii) the *distance/angle pattern*, not the coordinates, is the invariant. This is the quantitative case for *neighbourhood structure being the geometry, not the coordinates*. Cover and JL are two faces of one fact: the relational structure is the load-bearing content, and enough dimensions put any finite set into a well-separated general-position configuration.

**And the reverse — what if you *remove* dimensions, say from 100d down to 10d?** Two sharp facts. First, the source dimension does not appear in the bound at all: going from $100$, or from $10^{6}$, dimensions down to $10$ is governed only by how many points $N$ you have and the tolerance $\varepsilon$. Ten dims preserve *all* pairwise distances whenever $10\ge c\,\varepsilon^{-2}\log N$ — that is, for $N\lesssim\exp(10\,\varepsilon^{2}/c)$ points. You may throw away $999{,}999$ of a million dimensions and keep *every* pairwise distance among those $N$ points. Second, that floor is **tight**: Larsen and Nelson proved a matching lower bound, so no *linear* map can keep the relational structure of $N$ arbitrary points in fewer than $\approx\varepsilon^{-2}\log N$ dimensions \cite[Larsen & Nelson, 2014]{larsen2014jloptimal}. The catch that keeps it honest: JL preserves distances *among the $N$ points*, not the whole ambient space, and the surviving coordinates are random — meaningless on their own. Only the *relational* structure is invariant. (When the data in fact lives on a low-dimensional manifold, structured methods such as PCA beat $\log N$ — at the price of a worst-case guarantee.)

**4. Neural collapse — where the folding ends** \cite[Papyan, Han & Donoho, 2020]{papyan2020neuralcollapse}.
A well-trained classifier settles into a maximally symmetric geometry. In the terminal phase of training, each class's features collapse onto its class mean, and the $K$ class means become the vertices of a **simplex equiangular tight frame** (ETF): all pairwise angles equal, every point as far from the others as the space allows. So the "one flat cut" does not land on some arbitrary separating arrangement — it lands on the most separable one the space admits.

**5. Folding is topological untangling** \cite[Naitzat, Zhitnikov & Lim, 2020]{naitzat2020topology}.
The "egg" is literally a topological object: the inner class is a *hole* in the outer class's distribution. Topological data analysis counts such holes with **Betti numbers** ($b_0$ components, $b_1$ loops, $b_2$ voids). Layer by layer a trained net drives them to the minimum per class, and ReLU does it *faster* than $\tanh$ — ReLU is a non-homeomorphism that *changes* topology, $\tanh$ a homeomorphism that preserves it. Deep ReLU nets are *exponentially* more efficient than shallow ones at this simplification \cite[Ergen & Grillo, 2024]{ergengrillo2024topological}. Stated topologically: creases close holes and glue components.
