...
When $N\le d$ it saturates at $2^N$: *every* dichotomy is separable (a $d$-weight classifier has VC dimension $d$; a bias adds one, up to $d+1$ points). Once $N>d$, only a fraction $C(N,d)/2^N$ is separable — but for fixed $N$ that fraction **rises with $d$**, tending to $1$. Cover's point: a complex problem, cast *nonlinearly* in a high-dimensional, not-too-dense space, is more likely to be linearly separable than in a low-dimensional one. A layer wider than the data opens exactly the dimensions that push $d$ past the entanglement threshold.
