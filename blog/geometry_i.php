...
Section VIII ended with a proof of Poincaré's conjecture; the century between Poincaré's question (1895) and Perelman's answer (2003) is itself a history of geometry, and it is the shortest way to see what "space" came to mean. In the 20th century topology stopped being the study of rubber surfaces and became the mathematics of *relations*; and in the hands of **\citeauthorlastnameand{grothendieck_sheaf}** (1928–2014) the very notion of a space was rebuilt as the mathematics of *what a space can mean* \cite{grothendieck_sheaf}. This history never appears in a forward pass of a machine, and that is precisely the point: it is where "space" stopped meaning "the room you live in" and came to mean "any world of points whose relationships can be written down" — which is exactly what an embedding space is.

### Brouwer and Noether: numbers become groups

Poincaré's *Analysis Situs* counted holes as *numbers*, the Betti numbers $b_{k}$ \cite{poincareanalysissitus}. Two people turned counting into structure. **\citeauthorlastnameand{brouwer1911fixed}** (1881–1966) found the first genuinely *algebraic* invariants of continuous maps — the **degree** of a map (how many sheets it lays over its image, with sign) and his celebrated **fixed-point theorem** of 1911: every continuous map of a closed ball to itself leaves some point alone, i.e. $f(x)=x$ has a solution \cite{brouwer1911fixed}. The strange power of the theorem is that it is *topological*: the map may be folded or stretched any way you like, yet it cannot push every point away from itself. It also carried a quiet philosophy that runs through the whole century: to distinguish maps from one another, attach to each an *algebraic* number and compare equations, not diagrams.

The decisive step came when **Emmy Noether** — visiting Brouwer's home in Blaricum in December 1925 — pointed out that the Betti numbers were not numbers at all but the *ranks of groups*. A "hole of dimension $k$" is not a count; it is the group $H_{k}$, and two spaces differ when their hole-groups differ \cite{noether1926homology}. This one remark ("Betti numbers are the ranks of homology groups") turned topology into **algebraic topology**: shape, from then on, was to be studied by the algebra that summarises it — the same move the [Embeddings](embeddinglab) chapter makes when it replaces a word by a vector and studies the *numbers attached to it*.

### Hopf and de Rham: two grammars for the hole (1931)

The two theorems of 1931 teach how topology and analysis became one language. **\citeauthorlastnameand{hopf1931fibration}** discovered the **Hopf fibration** $S^{3}\to S^{2}$ — a continuous map from the 3-sphere onto the ordinary sphere that is *essential*: it cannot be deformed to a constant map, even though every loop in $S^{3}$ contracts ($\pi_{1}(S^{3})=0$, yet $\pi_{3}(S^{2})\cong\mathbb{Z}$) \cite{hopf1931fibration}. Its construction is pure algebra: write a point of $S^{3}$ as a pair of complex numbers $(z_{1}, z_{2})$ with $\lvert z_{1}\rvert^{2}+\lvert z_{2}\rvert^{2}=1$, and send it to the ratio $z_{1}/z_{2}$ on the Riemann sphere; the fibre over each ratio is a full circle, and the whole ball collapses to those interlocking circles. Hopf discovered it in 1931 while asking, in effect, a question that would have seemed mad to the 19th century — "can a higher-dimensional ball be wound around a lower-dimensional one in a way that is impossible to undo?" — and its message, that attached to every continuous map is a *counting number* that can be nonzero, is the seed of the homotopy groups that structure the modern theory of data.

That same year **\citeauthorlastnameand{derham1931}** proved the complementary statement, which reads like a dictionary between two dialects: the topology of *chains* (Section VIII's way of counting holes) and the analysis of *differential forms* declare the same answer, $H_{\mathrm{dR}}^{k}(M)\cong H^{k}(M)$ \cite{derham1931}. What de Rham showed is that a hole can be detected either by drawing loops or by checking whether certain integrals vanish on every closed form — calculus and topology measure the same shape.

### Chern and Whitney: measuring the twist (mid-century)

One more kind of invariant completed the picture. Curvature measures how a surface bends; **characteristic classes** measure how a *family of spaces attached along a base* — a fibre bundle — is twisted. **\citeauthorlastnameand{chern1946}** defined the **Chern classes** $c_{1}, c_{2}, \ldots$ of a complex vector bundle, with $c_{k}\in H^{2k}(M)$, whose nonzero components record the genuine obstructions to straightening the family \cite{chern1946}. The lineage runs straight from Gauss: the classical **Gauss–Bonnet theorem** — $\iint_{M}K\,dA=2\pi\chi$, the integral of curvature of Section VII is a whole number fixed by the topology — is itself the statement that "the top characteristic class evaluates to the Euler characteristic" \cite{docarmo} \cite{chern1946}. Chern's 1944 paper gave an intrinsic proof of the general Gauss–Bonnet theorem (found independently by Allendoerfer and Weil in 1943) so natural that it became the standard; it is one of the few times the "why does this integer exist?" question has a genuinely topological answer.
...
**\citeauthorlastnameand{smale1961hccobordism}** then proved, in 1961, the **h-cobordism theorem** in dimensions $\ge 5$: two simply-connected and sufficiently *flat-between* manifolds are diffeomorphic, and — as a corollary — the Poincaré conjecture holds in all dimensions $n\ge 5$ \cite{smale1961hccobordism}. In 1963 **\citeauthorlastnameand{atiyahsinger1963}** proved the **index theorem**: for an elliptic operator $D$ the difference between the dimension of its kernel and of its cokernel — an analytic quantity — equals a purely topological one, $\mathrm{ind}(D)=\mathrm{ch}(\sigma_{D})\,\mathrm{Td}(M)[M]$ \cite{atiyahsinger1963}. This is the theorem that makes "number of solutions minus number of constraints" computable from the shape of the space — the same pattern the kernel trick of Section IX exploits in reverse, when it computes with a function without ever writing its high-dimensional coordinates.
...
No-one bent the word "geometry" further and to greater effect than **\citeauthor{grothendieck_sheaf}** (1928–2014), and a short personal history is worth it: born in Berlin to a Russian anarchist father (Sascha Schapiro) and a German journalist mother, he grew up as a refugee: internment camps in southern France, then, self-taught, he walked into French mathematics as an adult and within two decades rebuilt most of its foundations \cite{grothendieck_sheaf}. His ruling idea: a space is not its points, but the *functions* admitted to live on it. A **scheme** is a space whose "points" are not locations but *solutions of polynomial equations*; a **topos** is a space so general that it may have no points at all — it is recorded by the sheaves of functions that can be planted in it, and the space itself is defined *by those functions* \cite{grothendieck_sheaf}.

The consequences sound like pure philosophy and are pure mathematics. The prime numbers become the points of a *line*: the scheme $\mathrm{Spec}\,\mathbb{Z}$ records exactly one closed point for each prime, and number theory becomes the geometry of a curve — the dream, first voiced by Jacques Tits, of treating $\mathrm{Spec}\,\mathbb{Z}$ as a curve over the "field with one element", whose Riemann hypothesis would then be a theorem of geometry \cite{grothendieck_sheaf} \cite{deligne1974weil}. This is the same prioritisation the manifold hypothesis of Section IX makes: the *functions* (the measurable, computable quantities) come first, and the "space" is the shadow they cast. Grothendieck's organisational genius was of the same kind: at the IHÉS (1959–1970), where he directed the famous Séminaire, he wrote (with Dieudonné) the monumental *Éléments de géométrie algébrique*, which rewrote algebraic geometry as the study of objects — then left in 1970 in protest over the institute's military funding, refusing in 1988 the prestigious Crafoord Prize, and spending his last decades in retreat in the Pyrenean village of Lasserre, invisible to the discipline he had rebuilt \cite{grothendieck_sheaf}. "The introduction of the digit 0 and the decimal mark", he once wrote, "were also essential steps" — a remark that ties the whole [History of AI](history) chapter's story of numerals to this one's story of spaces, and to *this* section's claim that geometry's history is the history of what *can* be written down.

### Deligne: the Riemann hypothesis over finite fields (1974)

The proof that this "space without points" was not an indulgence came in 1974, when **\citeauthorlastnameand{deligne1974weil}** proved the last and deepest of the **Weil conjectures** \cite{weilconjectures_wiki}. Counting the solutions of polynomial equations over finite fields defines, for a variety $X$, the zeta function $\zeta_{X}(s)$; Weil (1949) predicted that it satisfies — like Riemann's $\zeta(s)$ — a rationality, a functional equation, and a "Riemann hypothesis": the zeros lie on a symmetry line, equivalently the eigenvalues satisfy $\lvert\alpha_{i}\rvert = q^{i/2}$ \cite{deligne1974weil}. Grothendieck had restructured the very counting as cohomology (the famous $l$-adic sheaves), and Deligne's proof — completed in the pages of one IHÉS volume — settled it. Where Gauss, Riemann and Weil climbed, the Grothendieck school built a *language* in which the climb was guaranteed \cite{deligne1974weil} \cite{grothendieck_sheaf}. That is, in miniature, the whole lesson of this chapter for the machine: by the 1970s, "geometry" meant "the language in which the world's relations could be written", and every later application — embeddings, manifolds, kernels, geometric deep learning — is the same language put to work on the space of ideas.
</div>
...
* **1911 / 1925** — Brouwer's fixed-point theorem (algebraic topology's first tool); Noether at Blaricum turns Betti numbers into *groups* \cite{brouwer1911fixed} \cite{noether1926homology}.
...
* **1970 / 1974** — Grothendieck leaves the IHÉS over military funding; Deligne proves the Weil conjectures (the "Riemann hypothesis over finite fields") \cite{grothendieck_sheaf} \cite{deligne1974weil}.
