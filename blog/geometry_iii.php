...
<div class="md">
## II. Chains, boundaries, and the equation $\partial^{2}=0$
...
Before going further, we need the *other* pillar algebraic topology stands on: not what shapes *are* (homology), but what they can be *turned into* (homotopy). Two continuous maps $f,g: X \to Y$ are **homotopic** if you can slide one into the other without at any moment breaking it — if there is a continuous family $f_{t}$ with $f_{0}=f$, $f_{1}=g$. Two spaces have the **same homotopy type** if there are maps both ways whose compositions are homotopic to the identity: you can continuously *inflate* and *deflate* one into the other.
...
A spectral sequence is a *successive-approximations machine* for exact sequences: instead of one long exact sequence, you get a *book of pages* where page $E_{2}$ summarizes the input data, a differential $d_{2}$ acts, page $E_{3}$ summarizes the result, and so on until the sequence converges. In short: **where an exact sequence computes in a single ledger, a spectral sequence computes by moving amount after amount, page after page, until nothing moves anymore.** Leray used it on the cohomology of maps; a decade of computations followed.
...
* **1736 / 1752** — Euler: Königsberg bridges; $V-E+F=2$ \cite{eulerbridges} \cite{eulersolids}.
...
* **1931** — Hopf: $\pi_{3}(S^{2})=\mathbb{Z}$; the fibration that cracked homotopy open \cite{hopf1931fibration}.
