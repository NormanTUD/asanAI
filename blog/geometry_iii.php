...
Euler's formula is, in hindsight, the first homological statement ever made, the first Betti number of a simple shape, computed a century and a half before the name existed. Generalize it and the pattern becomes a hole counter. Take the alternating sum of the counts of the $k$-dimensional pieces of a shape; for a polyhedron that is exactly $V - E + F$, a number $\chi$ (read *chi*) that is always $2$ for a sphere-like solid, always $0$ for a doughnut, and always $-2$ for a double-doughnut. This $\chi$ is the **Euler characteristic**. **Enrico Betti** (1871) generalized the pattern to spaces of any dimension, counting the holes of each dimension and naming the counts after himself: the **Betti numbers**. The word *homology*, and the claim that you count holes by counting *cycles modulo boundaries*, came from the founder of the whole subject:
...
i.e. the hole-counting characteristic is the *alternating sum* of the **ranks** of the homology groups. (For a finitely generated abelian group the *rank* is the number of its independent, or free, generators; for the homology groups this is exactly the Betti number.) The result depends only on how many holes of each dimension a shape has, not on how it was cut up. This is why the same $\chi$ keeps appearing across geometry: in the Gauss–Bonnet theorem, $\oint_{M} K\,dA = 2\pi\chi$ (the integral signs are introduced in [Math III](math_iii)), it reads as *curvature*, and in the de Rham and Chern–Weil theorems it reads as *integral geometry* \cite{derham1931}.
...
**This is where the chapter meets the machine.** The [Embeddings](embeddinglab) chapter already computes, live, the persistent homology of token-vector clouds, the "Swiss cheese" of meaning: embedding space is full of genuine loops and cavities that persist across scale, which is what makes words cluster into *concepts* that can *wrap* around other concepts \cite{edelsbrunner2002persistent}. And in the [Transformer](transformer) chapter, the course runs a live topological analysis of the network's internal states, computing the Betti numbers of the cloud of activations at every layer and drawing the persistence barcode on screen. See those two chapters for the interactive visualizations: the barcode built there is the exact object this chapter has been about, computed on the shape of a machine's thought.
