...
3. **Apply the boundary operator.** The operator is written $\partial$ — the *curly d* you also meet as the **partial derivative** $\partial f/\partial x$ in calculus, a sign introduced by the Marquis de Condorcet in 1768 and later nicknamed *Jacobi’s delta*. Read it out loud as *“partial”* or *“the boundary of,”* and think of it as **“take the edge of.”** That a calculus sign is doing topological work is no accident: a boundary *is* a kind of derivative — it records how a piece *ends* — and, as the next line will show, both square to something harmless. To compute it, send each simplex to the signed sum of its lower-dimensional faces (an edge to its two endpoints, one $+$ and one $-$; a triangle to its three edges) and extend to all of $C_{k}$ by linearity. Formally, $\underbrace{\partial_{k}}_{\substack{\text{the boundary}\\\text{operator}}} : \underbrace{C_{k}}_{\substack{\text{formal integer}\\\text{sums of }k\text{-simplices}}} \to \underbrace{C_{k-1}}_{\substack{\text{sums of the }\\(k{-}1)\text{-dim pieces}}} $. The one equation that makes the whole subject work is
...
4. **Define the homology group.** Three more signs do the actual work here, so let us name them before we use them. **$\ker$** (short for *kernel*, the German *Kern*, “core”) of a map is the pile of inputs the map **sends to zero** — the information it forgets. **$\operatorname{im}$** (for *image*, the German *Bild*, “picture”) is the set of outputs the map **actually reaches**. And the slash in $A/B$ is a **quotient**: collapse the sub-object $B$ down to a single point, so that two things differing by an element of $B$ count as the same — read it *“mod”* or *“modulo.”* Say them aloud as *“kernel,”* *“image,”* *“mod.”* With those in hand, the $k$-cycles that are *not* boundaries — the genuine $k$-dimensional holes — are simply the closed cycles *modulo* the ones that already bound a solid piece. So declare
...
The rank of $H_{k}$ is the $k$-th **Betti number** $b_{k}$ — the number of independent holes of dimension $k$. For the torus (coffee cup/doughnut), a direct computation gives the whole story. (The answers are written in **$\mathbb{Z}$** — the *blackboard-bold* letter, from the German *Zahlen*, “numbers,” standing for the **integers** $\dots,-1,0,1,2,\dots$; the bold face is simply the house convention for naming famous number systems, so you will also meet $\mathbb{R}$ for the reals, $\mathbb{Q}$ for the rationals, and $\mathbb{C}$ for the complex. And $\mathbb{Z}^{2}$ means *pairs* of integers added component-wise — two independent “integer’s worth” of something.) It says:
...
The first and most famous exact sequence is the one that answers a practical question: *if I know the homology of two overlapping halves of a space, and of their overlap, do I know the homology of the whole?* Yes — with a long exact sequence. Stating it takes three signs, each doing a distinct job, and they are worth learning once. **$\cup$** (read *“union”*; an open cup that holds both pieces inside) means **put the two spaces together**: $X = A \cup B$ is everything that lies in $A$ *or* in $B$. **$\cap$** (read *“cap”*; the lens where the two overlap) is the **overlap** itself: $A \cap B$ is the part the two pieces share. Peano gave both their glyphs in 1895, in the same project that gave set theory its $\in$; the shapes were chosen to be self-evident — one opens up to *contain*, the other pinches to the *common* middle. The third is **$\oplus$** (read *“direct sum”* or, loosely, *“plus”*): it glues two algebraic objects **side by side as independent pieces**, so an element of $G \oplus H$ is just a pair $(g,h)$ and you add them component-wise. Picture the plane as $\mathbb{R} \oplus \mathbb{R}$ — the $x$- and $y$-axes welded at the origin, every point splitting *uniquely* into an $x$-part plus a $y$-part; the circle around the plus is what sets it apart from the ordinary product, marking these as *additive, independent* pieces that meet only at zero. In the sequence below, $H_k(A) \oplus H_k(B)$ simply means “line up every $k$-hole of $A$ next to every $k$-hole of $B$ and count them as one combined group.” For a space $\underbrace{X \;=\; A \;\cup\; B}_{\substack{\text{the whole space, written}\\\text{as two overlapping parts}\\\text{whose common piece is }A\cap B}}$ with overlap $A\cap B$:
...
Mayer–Vietoris computes *homology* (abelian, well-behaved). The **Seifert–van Kampen theorem** does the same glory job for the *fundamental group*, which is non-abelian and much less well-behaved — and so needs a different gluing sign. Where homology glued with $\oplus$ (“independent coordinates”), the loop group glues with $*$ (read *“star”*), the **free product**: take the loops of $A$ and the loops of $B$ and allow any word alternating between them, $a_1 b_1 a_2 b_2 \dots$, with *no rule at all* for how an $A$-loop interacts with a $B$-loop. “Free” means exactly that — the two families of generators stay independent, so the result is usually far bigger and non-commutative. Now refine it: because the two halves share the overlap $A\cap B$, a loop living in that overlap should be counted **once, not twice**. The **amalgamated free product** $*_{F}$ does precisely this: it is the free product $G * H$ *with one extra instruction* — the copy of the subgroup $F$ sitting inside $G$ and the copy sitting inside $H$ are declared to be the very same loops. In the formula below, $F = \pi_1(A\cap B)$, so “generators from both halves, but the shared loops identified.” If $\underbrace{X \;=\; A \;\cup\; B}_{\substack{\text{again, one space built from}\\\text{two overlapping open pieces}}}$ with open, path-connected $A,B$ whose intersection is also path-connected, then the fundamental group of $X$ is the **amalgamated free product**
