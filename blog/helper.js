...
const GLOSSARY = {
	...
	'morphism': 'In category theory, an arrow between two objects: a structure-preserving map $f : A \\to B$. The basic building block of a category.',
	'morphisms': 'In category theory, arrows between objects: structure-preserving maps $f : A \\to B$. The building blocks of a category; in a groupoid every one is invertible.',
	'endomorphism': 'A morphism from an object to itself, $f : A \\to A$. The self-maps of an object; the morphisms of a group are exactly the endomorphisms of its single object.',
	'endomorphisms': 'Morphisms from an object to itself, $f : A \\to A$ — the self-maps. In a group (a one-object category) every morphism is an endomorphism.',
	'automorphism': 'An invertible endomorphism, a self-map $f : A \\to A$ with an inverse. The symmetries of an object; a group is exactly the automorphisms of a single object.',
	'isomorphism': 'A morphism $f : A \\to B$ with an inverse $g : B \\to A$ ($g \\circ f = \\mathrm{id}_A$ and $f \\circ g = \\mathrm{id}_B$) — a reversible arrow. A groupoid is a category in which every morphism is an isomorphism.',
	'isomorphisms': 'Morphisms that have inverses, i.e. that are reversible in both directions. A groupoid is a category in which every morphism is an isomorphism.',
	'monomorphism': 'A left-cancellative morphism: $f \\circ h_1 = f \\circ h_2 \\Rightarrow h_1 = h_2$. The categorical generalisation of an injective function.',
	'epimorphism': 'A right-cancellative morphism: $h_1 \\circ f = h_2 \\circ f \\Rightarrow h_1 = h_2$. The categorical generalisation of a surjective function.',
	'n-cell': 'In an $n$-category or $\\infty$-groupoid, an $n$-dimensional morphism: a $0$-cell is an object, a $1$-cell a morphism, a $2$-cell a morphism between morphisms, and so on. Also called a $k$-cell.',
	'0-cells': 'The objects of a category or $\\infty$-groupoid — the $0$-dimensional cells (the points of the corresponding space).',
	'1-cells': 'The morphisms (arrows) of a category or $\\infty$-groupoid — the $1$-dimensional cells (the paths between points).',
	'2-cells': 'Morphisms between parallel $1$-cells in an $\\infty$-groupoid — homotopies, i.e. paths between paths (the $2$-dimensional cells).',
	'3-cells': 'Morphisms between parallel $2$-cells in an $\\infty$-groupoid — homotopies between homotopies (the $3$-dimensional cells).',
	...
	'smooth manifold': 'A manifold with the extra condition that the transition maps between overlapping coordinate charts are smooth (infinitely differentiable). This is what lets you do calculus globally: derivatives computed in one chart agree with derivatives computed in another. Examples: the sphere $S^n$, the torus, spacetime in general relativity.',
	'smooth function': 'A function of class $C^\\infty$: it has derivatives of all orders, and all those derivatives are continuous. No corners, no kinks, at any level of scrutiny. On a manifold, $f$ is smooth at $p$ if $f \\circ \\varphi^{-1}$ is smooth in any (equivalently, every) coordinate chart around $p$.',
	'holomorphic': 'Complex-differentiable at every point of an open set. Much stronger than real differentiability: a holomorphic function is automatically smooth and analytic (its Taylor series converges to the function). Holomorphic functions are rigid: they are determined by their values on any open subset.',
	...
	'barr\'s theorem': 'Every Grothendieck topos admits a surjective geometric morphism from a topos satisfying the axiom of choice (equivalently, a Boolean topos). Since presheaf topoi satisfy AC, this makes the easy theory of presheaf topoi dense among all Grothendieck topoi: many results proved first for presheaf topoi transfer to all topoi. (Barr 1974.)',
	...
	'homomorphism': 'A structure-preserving map between algebraic structures (groups, rings, etc.) that need not be invertible. In category theory, simply a morphism. An isomorphism is the special case of a homomorphism that is invertible.',
	'equivalence class': 'Given a set $S$ and an equivalence relation $\\sim$, the equivalence class of $x$ is $[x]_{\\sim} = \\{y \\in S \\mid x \\sim y\\}$: all elements related to $x$. The quotient set $S/\\sim$ is the collection of all such classes. In the groupoid setting, isomorphism classes play the role of equivalence classes.',
	...
	// ──────────────────────────────────────────────────────────────────
	// Coherent World Models: covers, sites, descent
	// ──────────────────────────────────────────────────────────────────
	'cover': 'In a site: a family of morphisms {cᵢ → c} that the chosen Grothendieck topology accepts as "good enough" to determine c. Intuition from topology: an open cover {Uᵢ} of an open set U is a family whose union contains U. In coherent modeling, a cover is a family of sub-views that together capture the whole context — with no relevant feature escaping every patch.',
	'covering': 'A family of morphisms {cᵢ → c} accepted by a Grothendieck topology as covering c. Two axioms: stable under pullback (pulling back a cover gives a cover) and transitive (covers of covers are covers).',
	'admissible cover': 'A cover whose morphisms all belong to the designated class T of admissible transitions — i.e., a cover that respects the constraints of the model. The sheaf condition is then tested only on these restricted covers, so coherence becomes a meaningful question instead of a vacuous one.',
	'admissible transition': 'A morphism that belongs to the designated class T of transformations considered legitimate in a given modelling context — sensor calibration, coordinate change, physical law, proof-preserving translation, validated decoder, documented archival transmission. The opposite of "any conceivable map"; without the restriction, coherence becomes vacuous.',
	'descent': 'The principle that data on a cover, agreeing on overlaps, can be "descended" to a unique piece of data on the covered object. Sheaf ≅ descent-data for sets, stack ≅ descent-data for groupoids, ∞-stack ≅ descent-data for ∞-groupoids. The flip side of gluing.',
	'refinement': 'In a category with a notion of cover: a cover {dⱼ → c} refines {cᵢ → c} when every dⱼ maps into some cᵢ commuting with the structure maps. Refinement is the categorical form of "zooming in" on a cover.',
	'poset': 'A partially ordered set — a set equipped with a reflexive, transitive, antisymmetric relation ≤. The poset of open subsets of a topological space (ordered by inclusion) is the canonical example.',
	'subcategory': 'A category whose objects and morphisms are taken from a larger one, with the same composition and identities. Full = all hom-sets are included; essentially small = up to equivalence of objects.',
	'cosheaf': 'The colimit dual of a sheaf: a presheaf that turns colimits of open sets into colimits of sections. Where a sheaf glues local → global, a cosheaf propagates global → local.',
	'fibration': 'A morphism p: E → B in which objects over B can be pulled back along every map in B. Grothendieck fibrations are the natural home of indexed families and dependent type theory.',
	'stack': 'A sheaf of groupoids: like a sheaf, but instead of just data on each patch one also tracks isomorphisms on overlaps (and coherence on triple overlaps). A 2-truncated ∞-stack.',
	'infinity-stack': 'A sheaf-like object valued in ∞-groupoids: like a stack but with higher coherence data tracked at every level. The ∞-categorical upgrade of the sheaf condition.',
	'infinity-sheaf': 'A sheaf-like object valued in ∞-groupoids instead of sets: on overlaps one requires coherent equivalence rather than equality, with the coherence tracked at every level. Handles situations where local data are not literally equal but only "equal up to homotopy up to homotopy up to…".',
	'infinity-1-category': 'A weak ∞-category in which all k-morphisms for k ≥ 2 are invertible (only the 1-morphisms need not be). The natural home of homotopy theory and of HoTT; sometimes written (∞,1)-category.',
	'2-morphism': 'In an (∞,1)-category: a morphism between two parallel 1-morphisms. In HoTT these are paths between paths; in ordinary category theory they appear when composition is only associative up to specified isomorphism.',
	'3-morphism': 'In a higher category: a morphism between 2-morphisms, one level up. n-morphisms live at level n.',
	'n-morphism': 'In an n-category: a cell of dimension n. 0-morphisms = objects, 1-morphisms = arrows, 2-morphisms = arrows between arrows, and so on.',
	'coalgebra': 'In category theory: an object X with a map X → F(X) for some endofunctor F, formalising "unfold into parts". A stream of bits is a coalgebra for the "delay by one step" functor; a residual stream is coalgebra-like because it exposes an old-state component next to a new-state contribution.',
	'cospan': 'A diagram shaped like ⊐ — two morphisms X → Z and Y → Z into a common apex Z. The pullback X ×_Z Y is the limit of this cospan.',
	'span': 'A diagram shaped like ⊏ — two morphisms Z → X and Z → Y out of a common apex Z. The pushout X ⊔_Z Y is the colimit of this span.',
	'cech nerve': 'The simplicial object built from a cover {cᵢ → c}: in degree 0 the disjoint union of the cᵢ, in degree 1 the disjoint union of pairwise overlaps cᵢ ×_c cⱼ, in degree 2 the triple overlaps, etc. The Čech nerve is the fundamental computational object behind ∞-descent — testing the sheaf condition on a single cover means testing equality on this whole simplicial diagram.',
	'simplicial object': 'A presheaf on the simplex category Δ: a sequence of objects X₀, X₁, X₂, … with face maps (going down) and degeneracy maps (going up), encoding the combinatorial structure of a simplicial complex. The Čech nerve of a cover is the prototypical simplicial object.',
	'geometric theory': 'A logical theory whose axioms use only ∧, ∨, ∃, =, ⊤, ⊥ — not ⇒, ∀, ¬. Geometric axioms are exactly those preserved by inverse-image functors of geometric morphisms, so geometric theories have a natural semantics in every Grothendieck topos.',
	'geodesic': 'In a metric space: a path between two points whose length equals the distance between them. A straight line in Euclidean space; a great circle arc on a sphere. Coherent gluing prefers geodesic representatives because they expose the structure cleanly.',
	'limit': 'A universal construction that builds the "most general" object receiving maps from a given diagram. Products, equalisers, and pullbacks are all limits.',
	'colimit': 'The dual of a limit: the "most specific" object that all objects in a diagram map into. Coproducts and pushouts are colimits.',
	'coalgebra of': 'A coalgebra for an endofunctor F is an object X together with a structure map X → F(X). In programming, streams and lazy lists are coalgebras for the "delay" functor; in ML, residual streams look coalgebra-like because each layer exposes an old-state component next to a new-state contribution.',
	'infinity-1-topos': 'A higher categorical upgrade of a Grothendieck topos: an (∞,1)-category with the topos-like structure (limits, exponentials, a subobject classifier, etc.) but with homotopy-coherent equality instead of strict equality. The natural setting for ∞-sheaves.',
	'geometric morphism': 'The right notion of "map between topoi". A geometric morphism f: F → E is an adjoint pair f* ⊣ f* of functors with f* (the inverse image) left adjoint and finite-limit-preserving. Continuous maps X → Y between spaces induce geometric morphisms Sh(X) → Sh(Y).',
	'grothendieck': 'Alexander Grothendieck (1928–2014), the algebraic geometer who, in the early 1960s, introduced the notion of a topology on an arbitrary category (Grothendieck topology) and used it to define sheaves and topoi without reference to an underlying point-set topology. The whole framework of this chapter — site, sheaf, topos, descent — is his.',
	'diaconescu': 'Diaconescu\'s theorem: in any topos, the axiom of choice implies the law of excluded middle. Once you leave Set, the two principles are no longer equivalent.',
	'comma category': 'For functors F: A → C and G: B → C, the comma category (F ↓ G) has as objects triples (A, B, f) with f: F(A) → G(B). Morphisms are pairs of arrows making the obvious square commute. The slice C/X is (id_C ↓ X).',
	'yoneda embedding': 'The functor Y: C → [C^op, Set] sending an object X to the representable functor C(-, X). The Yoneda lemma guarantees Y is full and faithful: every small category embeds into its presheaf topos.',
	'subobject classifier': 'An object Ω in a category together with a monomorphism true: 1 → Ω such that every monomorphism m: Y ↪ X arises uniquely as a pullback of true along a characteristic map χ_m: X → Ω. In Set, Ω = {0, 1}; in Sh(X), Ω(U) is the set of open subsets of U. The presence of Ω is what makes a category "set-like" and gives it an internal logic.',
	'site': 'A small category equipped with a Grothendieck topology: a collection of "covering families" of morphisms, telling you which families of objects cover a given object. The category of sheaves on a site is a Grothendieck topos.',
	'grothendieck topology': 'A coverage on a category: a collection of "covering families" of morphisms, telling you which families of objects cover a given object, satisfying stability under pullback and transitivity. A category equipped with a Grothendieck topology is a site; the sheaves on it form a Grothendieck topos.',
	'presheaf': 'A map from the category of open sets of a space (or more generally the opposite of any small category) to a target category, with restriction maps but not necessarily the gluing condition. Every sheaf is a presheaf; not every presheaf is a sheaf.',
	'gluing': 'The process of assembling local data into global data, made unique by the sheaf condition. The sheaf condition says: compatible local data glues into exactly one global section. Dual to descent.',
	'sheaf': 'A mathematical object that assigns data to every open set of a topological space (or more generally to every object of a site), consistently across overlaps. The local-to-global principle formalised: local data glues into global data iff it agrees on overlaps.',
	'sheaf condition': 'The requirement that local sections of a sheaf on a cover agree on overlaps iff they glue to a unique global section. What separates a sheaf from a mere presheaf.',
	'homotopy': 'A continuous deformation between two paths (or more generally between two morphisms). When a homotopy exists between two paths they are considered equal in HoTT.',
	'category': 'A collection of objects and morphisms between them, with associative composition and identity morphisms. The language of modern structural mathematics.',
	'functor': 'A structure-preserving map between categories: it sends objects to objects and morphisms to morphisms, respecting composition and identities.',
	'morphism': 'The primitive notion of "a way of getting from one object to another" in a category. In Set, morphisms are functions; in Top, continuous maps; in a sheaf category, natural transformations that commute with restriction.',
	'pullback': 'In category theory: the limit of a cospan (a diagram shaped like a corner with two arrows X → Z and Y → Z into a common apex). The pullback X ×_Z Y is the "most general" object receiving compatible maps from X and Y. The categorical generalisation of the fibre product.',
	'equalizer': 'In a category: the limit of a parallel pair of morphisms f, g: A → B. The equalizer E comes with e: E → A such that f ∘ e = g ∘ e, and e is universal with this property. In Set it is the set {a ∈ A : f(a) = g(a)}.',
	'identity type': 'In HoTT, the type of proofs that two terms are equal. An inhabitant of Id_A(a, b) is a path from a to b in the space A.',
	'path': 'In HoTT, a proof of equality between two terms — equivalently, a continuous path between two points in the corresponding space. Paths compose; paths-between-paths (homotopies) live one level up.',
	'iso': 'See "isomorphism".',
	'equivalence': 'In HoTT / ∞-category theory: a morphism that has a (homotopy) inverse. Equivalences compose, and the univalence axiom says that equivalent types are equal: (A ≃ B) ≃ (A = B).',
	'isomorphism': 'A morphism f: X → Y in a category that has a two-sided inverse g: Y → X with g ∘ f = id_X and f ∘ g = id_Y. The categorical notion of "bijection".',
	'topos': 'A "universe of mathematics": a category that behaves, in enough respects, like the category of ordinary sets, with its own internal logic. Sheaves on a space form a topos; in this topos you can do all of set-theoretic mathematics, but the internal logic may not be classically Boolean. Introduced by Alexander Grothendieck in the early 1960s and axiomatised by Lawvere and Tierney.',
	'presheaf topos': 'For a small category C, the functor category [C^op, Set]. This is always a Grothendieck topos. The Yoneda embedding sends C into it, so C can be recovered from its presheaf topos.',
	'sheaf topos': 'For a topological space X (or, more generally, a site), the category Sh(X) of set-valued sheaves on X. This is the prototypical Grothendieck topos. Truth values in Sh(X) are open sets, so statements can be "true on U, false on V, neither on the rest".',
	'terminal object': 'The final object 1 in a category: an object with exactly one morphism into it from every other object. In Set it is any singleton.',
	'initial object': 'The empty object 0 in a category: an object with exactly one morphism out of it into every other object. In Set it is the empty set.',
	'product': 'In a category: an object A × B with projections to A and B such that every pair of morphisms into A and B from a common object C factors uniquely through it. In Set, the product is the ordinary Cartesian product.',
	'coproduct': 'In a category: the colimit of a diagram of two objects. For A, B, the coproduct A + B comes with inclusions A → A + B and B → A + B such that every pair of morphisms out of A and B into some C factors uniquely through them. In Set, coproduct is the disjoint union.',
	'pushout': 'In a category: the colimit of a span (a diagram shaped like a corner with two arrows Z → X and Z → Y out of a common apex). The pushout of f: Z → X and g: Z → Y is the "most specific" object receiving compatible maps from X and Y. In Set, pushout is the quotient of the disjoint union X + Y by the relation identifying f(z) and g(z).',
	'universal property': 'A definition of a mathematical object by the maps into (or out of) it — the object that makes every compatible diagram commute in the "best" way. Products, sums, free constructions all have one.',
	'natural transformation': 'A family of morphisms, one for each object of a category, that commutes with every morphism of that category. The "morphism between functors".',
	'adjoint functors': 'A pair of functors going in opposite directions that are "best approximations" of each other — one is the most general, the other the most specific. Ubiquitous in mathematics and computer science.',
	'left adjoint': 'In an adjoint pair F ⊣ G: the functor F that is "the most general" of the two. Left adjoints preserve colimits.',
	'right adjoint': 'In an adjoint pair F ⊣ G: the functor G that is "the most specific" of the two. Right adjoints preserve limits.',
	'HoTT': 'Homotopy Type Theory — a foundational language for mathematics where types are spaces, terms are points, and equality is a path. The "oid" in ∞-groupoid means "group-like, but with many objects".',
	'univalence axiom': 'The axiom that equivalent types are equal: (A ≃ B) ≃ (A = B). Equality of types is exactly equivalence of types.',
	'open cover': 'A collection of open sets whose union is the whole space. Sheaves are tested on covers: local data on each open set must glue to global data on the whole.',
	'topology': 'Similar to geometry, but distance is abstracted away — of two points you can only say d(x, y) = 0 if they are the same point and d(x, y) ≠ 0 if they are not.',
	'monomorphism': 'A morphism m: X → Y that is left-cancellable: if f ∘ m = g ∘ m then f = g. The categorical generalisation of "injective function".',
	'epimorphism': 'A morphism e: X → Y that is right-cancellable: if e ∘ f = e ∘ g then f = g. The categorical generalisation of "surjective function".',
	'kernel pair': 'In a category: the pullback of a morphism f: X → Y along itself. The categorical analogue of the equivalence relation {(x₁, x₂) : f(x₁) = f(x₂)}.',
	'sieve': 'A collection S of morphisms into an object c of a category, closed under precomposition: if f: d → c is in S and g: e → d is any morphism, then f ∘ g is in S. In a presheaf topos [C^op, Set], the subobject classifier Ω assigns to each object c the set of sieves on c.',
	'elementary topos': 'A category with finite limits, exponentials (so it is Cartesian closed), and a subobject classifier Ω. Every Grothendieck topos is an elementary topos; some elementary topoi are not Grothendieck (e.g. the effective topos, FinSet).',
	'grothendieck topos': 'A topos that is "the right size": equivalent to the category of sheaves on a small site. Characterised by Giraud\'s axioms: small set of generators, all small colimits, sums are disjoint, equivalence relations are effective.',
	'heyting algebra': 'The intuitionistic generalization of a Boolean algebra: a lattice with ∧, ∨, ⇒, ⊥, ⊤ satisfying all the usual axioms except possibly the law of excluded middle p ∨ ¬p = ⊤. The subobject classifier of a topos is always a Heyting algebra internally.',
	'classifying topos': 'For a geometric theory T, the classifying topos Set[T] has the universal property that geometric morphisms F → Set[T] correspond to models of T inside F. The theory is recovered from the topos.',
	'point of a topos': 'A geometric morphism p: Set → E, i.e. a way of recovering a "classical point" inside the topos. Many topoi have no points.',
	'internal language': 'The typed higher-order intuitionistic logic obtained by interpreting a formal language inside a topos: types as objects, terms as generalized elements, propositions as morphisms 1 → Ω. Any theorem provable in higher-order intuitionistic logic holds internally in every topos.',
	'homotopy hypothesis': 'The conjecture (due to Grothendieck) that ∞-groupoids are equivalent to homotopy types: spaces up to homotopy. Conjecturally, this identifies higher category theory with algebraic topology. Still open in full generality.',
	'reflection': 'In category theory, a reflector is the left adjoint to the inclusion of a reflective subcategory. Example: the sheafification functor is the reflector from presheaves to sheaves.',
	'reflective subcategory': 'A full subcategory A ↪ C such that the inclusion has a left adjoint (the reflector). Examples: abelian groups inside all groups; sheaves inside presheaves.',
	'sheafification': 'The left adjoint to the inclusion Sh(X) ↪ [Open(X)^op, Set]. Sheafification takes any presheaf and returns the "closest sheaf to it", by forcing the gluing condition to hold.',
	'simplex category': 'The category Δ whose objects are the finite ordered sets [n] = {0, 1, …, n} and whose morphisms are order-preserving maps. Presheaves on Δ^op are simplicial sets, the combinatorial model for spaces.',
	'comonad': 'A comonad on a category C is a comonoid in the monoidal category of endofunctors: an endofunctor W together with unit ε: W → id and comultiplication δ: W → W ∘ W satisfying coassociativity and counit laws. The categorical dual of a monad. Residual streams can be modelled as coalgebras for a comonad.',
	'monad': 'A monad on a category C is a monoid in the monoidal category of endofunctors: an endofunctor T with unit η: id → T and multiplication μ: T² → T satisfying associativity and unit laws. Examples: the list monad, the probability monad, the reader/writer monads. The categorical counterpart of a "computational effect".',
	'coalgebra for': 'A coalgebra for an endofunctor F is an object X together with a structure map X → F(X) satisfying coaction axioms. Streams, residual streams, and unfolding data structures are coalgebras; the structure says "how to take this thing apart".',
	'cartesian closed': 'A category C in which for every pair A, B there is an exponential object B^A and the functor - × A has a right adjoint. Set and every (elementary) topos are Cartesian closed.',
	'exponential object': 'In a Cartesian closed category: an object B^A with an evaluation map ev: B^A × A → B such that for every C and every f: C × A → B there is a unique curry(f): C → B^A with f = ev ∘ (curry(f) × id_A). Categorical generalisation of the function space.',
	'power object': 'In a topos: the exponential Ω^X, which serves as the internal "power set of X". Subobjects of X correspond bijectively to morphisms X → Ω, hence to elements of Ω^X.',
	'Grothendieck': 'See "grothendieck". (Capitalised alias.)',
	'sheaves': 'The plural of sheaf — used informally to refer to the category of sheaves on a site.',
