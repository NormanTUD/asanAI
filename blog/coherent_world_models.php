...
8. **Residuals — the honest part.** Running the audit, the framework *finds residuals in itself*, and records them:
   ...
   - **Some concrete claims are over-licensed.** A historical reading (a fragment of a pre-formal philosopher read as an instance of the formalism) is *looser* than its presentation suggests; a structural analogy (a network "as a sheaf") is *not* a theorem in the borrowed category. Residual: *a few contact points fail, or are under-licensed, relative to the strength at which they are stated.*
...
A useful analogy is not a theorem, and the chapter now *marks its analogies as analogies* rather than letting them masquerade as biconditionals. Two corrections, both the same move (locate the difference at the right level, refuse to upgrade a weaker sameness into a stronger one): code execution and formal verification are *live observation arrows* an earlier draft underweighted (the training corpus, meanwhile, is a *frozen, mediated* one — the LLM has no *direct, real-time* arrow at inference, but it is not "without observation"); and the *dominant* hallucination is a *coherent-but-ungrounded* output (a correspondence failure), with the *self-contradicting* non-glueable case as a distinct, less common type.
