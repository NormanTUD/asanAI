...
What we receive is *almost always already mediated* — this is the framework's default commitment, not a universal claim. The redness of red, the pain of a headache, the taste of coffee are, on the framework's account, the final output of a long pipeline (photoreceptors → retinal processing → lateral geniculate nucleus → visual cortex → attentional and mnemonic modulation → …) applied to the world; they *feel* immediate only because we do not consciously witness the pipeline that produced them; the redness is the brain's construction, not the photon's revelation.

But there is a rare and uncomfortable limit case: **wordless introspection**. When we discuss qualia and *experience the very same qualia* while discussing them, the trace and the topic collapse: the experience we are tracking is the experience we are having. This is the closest the framework admits to an unmediated datum, and it is precisely where direct realism reasserts itself. Wordless introspection — the immediate, pre-conceptual awareness of one's own experience, before it is named, categorised, or compared — is the limit where trace and topic are most nearly identical.

In this limit, the framework's machinery changes shape. The access function $O$ is no longer external-to-the-trace but intrinsic to it; indexicality becomes trivial (the trace IS of itself); underdetermination fails (there is nothing to recover, since trace and source coincide); the properties *transformed* and *mediated* weaken, since there is no external-to-internal pipeline to traverse. The whole apparatus — admissible transitions, covers, sheaf conditions — was built for mediated traces and becomes vacuous when trace and topic collapse. Wordless introspection marks a *boundary of the framework*: inside it, productive machinery for working with mediated reports; outside it, experience without representation, where the question "is the model true?" does not arise.

Even here, the collapse is only partial and not stable. As soon as the experience enters *language*, it is re-mediated by conceptual scheme, by contrast-class, by the public vocabulary of colour ("red" already presupposes a contrast with green, yellow, blue). The moment we *describe* the wordless introspection, it becomes languaged, and the mediation reasserts itself. So the limit case exists only as long as we do not speak about it — a fragile fact about introspection that any philosopher of mind has had to wrestle with.

A philosopher who holds qualia to be primary — for whom the redness *is* the world's redness, not a brain construction, and for whom wordless introspection is the *primary* mode of experience — will say the framework has mis-described the boundary. They will read the present chapter as a useful tool for *communicated* experience but a poor account of *lived* experience, and they will refuse to grant that the mediated case is "default" rather than the limit. The framework takes the mediated, communicable case as its default, openly: it is the case where epistemology, science, and language work, and where its machinery is productive. The dispute with qualia-primary views is *foundational* — acknowledged at the door, not settled inside.

Inside the framework's default, there is no Archimedean point from which the world shows itself untransformed. Kant's name for what is forever on the other side of this mediation is the *thing in itself* (*Ding an sich*) — first introduced in the *Critique of Pure Reason* at KrV A26/B42 (§3 of Space, in the Transcendental Aesthetic) and developed systematically in the chapter "Of Phenomena and Noumena" at A235/B294 \cite{kant_critique_pure_reason}; the technical term here is **the subject matter as it would be independent of any access**. We never get there. The right question is therefore not "how do we reach it?" but "what can we honestly do with the traces we have?"
