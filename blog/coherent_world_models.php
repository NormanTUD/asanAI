...
- **The cover $\{c_i\}_{i \in I}$** is the *training corpus*: each $c_i$ is a document, a web page, a code repository, a conversation, a transcript. Each is a "patch" of partial evidence about $W$. The patches overlap — the same fact appears in many documents, the same code idiom in many repositories — and the union of their images is supposed to cover everything the model is meant to be able to speak about. The cover is not clean. Patches *disagree about the same facts* — different sources contradict each other, sometimes within the same document, sometimes across millions of pages — and some are noisy, outdated, or biased toward what was popular, accessible, and digitised at training time. Descent on an imperfect, partially contradictory cover produces a $G$ that *averages over contradictions* rather than resolving them: the global section inherits the cover's disagreements, weighted by how often each version appears. In the chapter's vocabulary, this is the self-consistent fantasy starting at the data — the model is internally consistent and inherits the corpus's errors at the same time.
  ...
  In the **strict** sheaf regime this would be fatal: local sections are supposed to be *equal* on overlaps, and the corpus is full of disagreements. But the chapter's *Higher coherence* section is built for exactly this case. The right target is $\infty\text{-Gpd}$ rather than $\mathbf{Set}$: local sections are *coherently equivalent* rather than equal, with the equivalences themselves being data — a 2-morphism between "Paris is the capital" and "the capital is Paris", a translation path between "Paris" and "la capitale", a correction path between "Berlin" and "Paris". The $\infty$-sheaf condition asks not for agreement but for *coherent agreement*: the patches' disagreements must themselves agree on triple overlaps, so corrections compose, translations chain consistently, lies cancel out in the limit. The chain of transformations from $W$ to the text in the corpus is, as the chapter's "observer" section already noted, $W \to \text{signal} \to \text{internal rep.} \to \text{language} \to \text{text} \to \text{corpus}$ — misremembering, badly-written prose, paraphrase, lies, mental problems, all the way down. Each arrow is a homotopy, and the cumulative effect is that the corpus's patches are not equal on overlaps but *related by homotopies whose homotopies may or may not compose coherently*. The training process is, in effect, an attempt to compute descent in the homotopical regime; a substantial fraction of why the corpus's noise is not fatal is that the homotopical regime absorbs it. What survives in $G$ is the *coherence class* of the cover — the structure common to all patches, with the patch-by-patch disagreements left as residuals — not any single one of them.
