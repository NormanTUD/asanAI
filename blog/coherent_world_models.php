...
This is the precise statement of what it means to say that a coherent world model is *true* — read as an *ideal the model converges to*, since the correspondence leg is a limit rather than a state it attains. Not true as a slogan: it is the conjunction of two conditions, one structural (checkable on the current cover) and one empirical (approached as the cover is widened); the operational, fallible criterion — *justified acceptance* — is drawn in the closing synthesis. The two are independent *precisely because* they run in different directions: coherence is checked *within* a fixed admissible cover, while correspondence — as the preceding subsection fixed it — is checked *across* the sequence of covers, as that cover is widened. One is a spatial condition on the patches; the other is a structural condition on the *sequence* of patches. That independence is what keeps the test from being circular, and it is also why neither alone suffices.
...
This is the positive form of what the *factbook* pathology — the second of the five — is the absence of: a heap of individually true, individually well-calibrated claims that no morphism connects. Every part of a genuine world model must, at least in theory, be *reducible to* every other part by some chain of licensed transitions. The morphisms are the connective tissue that turns "a collection of claims" into "one model of one subject matter"; a model whose parts are only co-present, never interconnectable, is not a model at all but a list.
...
| Out-of-cover query | Fluent, confident answer to a question outside the training cover (long-tail / out-of-distribution) | Coverage — no contact point exists to check | Detect non-coverage: coverage calibration, "I don't know", retrieval allowed to return nothing |
...
7. **Does descent hold?** Do the parts cohere? Mostly yes — *with recorded seams.* The internal tensions (the object-versus-element of $G$, the cover-versus-chain structure, the two-type hallucination split whose boundary is not always observable from the output) are *known* and tracked in the companion corrections; a global section that *knows its own seams* is still a section, but it is not a *seamless* one.
