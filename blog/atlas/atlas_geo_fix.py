...
def should_apply(query, lat, lng):
    """Return (apply?, why) for a geocoded (lat,lng) of a loc string."""
...
def entity_loc(e):
    """Loc string for a raw entity, or None if it has none (nested-only rows)."""
    loc = e.get("loc")
...
def resolve(cache, loc):
    """Return (lat, lng) to use for a loc, or None if we keep the original."""
    if not loc:
        return None
    rec = cache.get(norm_query(loc))
    if not rec or rec.get("lat") is None or rec.get("lng") is None:
        return None
    ok, why = should_apply(loc, rec["lat"], rec["lng"])
    if not ok:
        return None
    return (rec["lat"], rec["lng"], why)
        ...
        if file_changed:
            ...
            if not dry:
                ...
                with open(tmp, "w", encoding="utf-8") as f:
                    ...
                    f.write("\n")
                os.replace(tmp, p)
    print("raw files: %d | changed: %d | entities re-geocoded: %d" %
          (len(files), touched, changed))
...
def cmd_report():
    ...
    for e in ents:
        ...
        if res is None:
            if loc:
                _ok, why = should_apply(loc,
                                        cache.get(norm_query(loc), {}).get("lat"),
                                        cache.get(norm_query(loc), {}).get("lng"))
                kept[why.split("(")[0]] += 1
            continue
        lat, lng, why = res
        dlat = (e["lat"] or 0) - lat
        ...
        if abs(dlat) > 0.05 or abs(dlng) > 0.05:
            moved += 1
            if dist > 120:
                big.append((round(dist), e["name"], e["loc"],
                            (e["lat"], e["lng"]), (lat, lng)))
    print("entities whose coords will change: %d" % moved)
    print("keep-original reasons: %s" % dict(kept))
    print("moving >120 km: %d" % len(big))
    big.sort(reverse=True)
    for dist, name, loc, old, new in big[:60]:
        print("  ~%5d km  %-28s %-26s (%.2f,%.2f) -> (%.2f,%.2f)" %
              (dist, name[:28], loc[:26], old[0], old[1], new[0], new[1]))
