...
<div class="md">
Artificial Intelligence is often described as “Applied Statistics”: at its core, every decision an AI makes is a calculation of likelihoods. The frameworks below come from diverse origins — games of chance, mapping the heavens — and now enable machines to learn from data.
</div>
...
<figure class="md">
    ...
    <figcaption><span class="md">\citetitle{flammarion}: A symbol of the drive to venture beyond the limits of current understanding</span></figcaption>
</figure>
...
The goal of studying distributions is not to memorize them, but to understand how large datasets behave. Real-world data rarely matches a theoretical distribution perfectly, but studying the distributions changes how you see the data: not as isolated points, but as a cohesive whole.

In practice, many real-world datasets approximate a theoretical distribution closely enough for the model to be useful, and the statistical methods applied to them remain reliable even when the match is imperfect.
...
### How knowing about distributions helps in developing AI systems

Recognizing these distributions in real-world data — from telescope photons to pixel intensities in medical imaging — lets machines model uncertainty. Most natural phenomena are not random noise but follow mathematical patterns; by recognizing a Gaussian distribution, an AI can separate meaningful “signal” from background noise. For example, in autonomous driving, a sensor must decide whether a blurred shape is a pedestrian or a lens flare. Knowing the distribution of typical sensor errors lets the machine make a calculated “guess” rather than stalling on an exact match: abstracting the data into a probability curve is the “extra level of indirection” that, as David Wheeler is credited with saying, can solve any problem.

These distributions also underpin the “scaling laws” that drive modern large language models. As described in \citetitle{sutton2019bitter} by Rich Sutton, progress in AI comes from “general methods that leverage computation” rather than hand-coded human knowledge. When an AI is trained on vast datasets, it is essentially learning to map the distribution of human language; applications like predictive text and weather forecasting rely on the fact that the next word or the next storm front follows a predictable frequency distribution. Knowing these patterns lets developers make choices — from initializing neural networks to selecting training data — that match the variation actually present in the data.
</div>
...
<div class="statlab-section">
    <div class="md" data-mathlevel="50" data-optionaltitle="The Bernoulli Distribution: The Atom of Probability">
        ...
        The “Expected Value” is $E[X] = p$: for a biased coin, $p$ is the fraction of flips that land heads in the long run.
    </div>
...
$$P(X=k) = \underbrace{\binom{n}{k}}_{\text{number of sequences}} p^k (1-p)^{n-k}$$

$$\binom{n}{k} = \frac{n!}{k!(n-k)!}$$

**The “Problem of Points” Example:**
If two players are in a game where the first to 3 points wins, but the game is interrupted when the score is 2 to 1, Pascal and Fermat used these coefficients to determine fair prize splits. By looking at the triangle, they could count how many future “paths” (combinations of wins/losses) led to each player winning the overall pot.
</div>

<div class="optional md" data-headline="The History of the Binomial Distribution">
##### The Medieval Insight (c. 1250)
The earliest known mention of the differing frequencies of dice sums appears in a Latin poem called \citetitle{devetula} (p. 32), whose author is unknown. The poem correctly notes, for the first time, that some totals with three dice occur more often than others — the earliest recognition that dice sums are not equally likely.
...
Pascal wrote to **Pierre de Fermat**, and their exchange of letters is considered the founding moment of probability theory. They moved beyond mere counting and used the **Binomial Coefficients** (Pascal's Triangle) to count the possible continuations of an interrupted game (\citetitle{oevresdeformat}, p. 288 ff.).
</div>
...
On January 1, 1801, the dwarf planet **Ceres** was spotted by \citeauthor{ceresdiscovery}. The find was short-lived: Ceres soon vanished into the Sun's glare, leaving astronomers with only 41 days of “noisy” and uncertain data. To find it again, the world turned to **Carl Friedrich Gauß**, who solved the problem in 1801 by the method he later published in \citetitle{gauss1809} (\citeyear{gauss1809}, §§ 175–179, pp. 208–213): he considered all the observations together, since the true position must lie among them.
</div>
...
<div class="md">
To find Ceres, Gauß worked from the **errors** of the measurements. He assumed that while the planet follows a perfect path (green line), the telescope readings (blue dots) follow a **Bell Curve** centered on that path.
</div>
...
<div class="statlab-section">
    <div class="md">
        ...
        The theoretical framework underlying much of supervised learning was formalized by \citeauthor{vapnik} in the 1960s–1990s. His **Statistical Learning Theory** asks a fundamental question: given a finite set of training examples, how can we bound the error on unseen data?
...
$$\Phi(z) = \underbrace{\frac{1}{\sqrt{2\pi}}}_{\text{scaling factor}} \int_{-\infty}^{z} e^{-\frac{t^2}{2}} \, dt$$
...
- The $e^{-\frac{t^2}{2}}$ part is the **shape of the bell curve** itself; it is what makes it tall in the middle and flat at the edges.
...
Unlike the Binomial formula, where you get an exact answer with multiplication, this integral has **no neat algebraic shortcut**: it cannot be simplified into a clean fraction or product. But it can still be calculated — by **approximating** it numerically.
...
We can't start at $-\infty$, but the curve is essentially zero for $z < -4$, so we start there. Let's use a step size of $\Delta t = 0.5$ (coarse, for illustration):
</div>
...
<table class="stat-phi-table" style="width: 100%; max-width: 700px; margin: 20px auto; border-collapse: collapse; font-size: 0.95em;">
    ...
    <tbody>
        ...
        <tr style="border-bottom: 1px solid #e2e8f0;">
            ...
            <td style="padding: 6px 12px; text-align: center; font-family: monospace;">$0.4003$</td>
        </tr>
        <tr style="border-bottom: 1px solid #e2e8f0; background: #f8fafc;">
            ...
            <td style="padding: 6px 12px; text-align: center; font-family: monospace; font-weight: 700;">$0.5998$</td>
        </tr>
        <tr style="border-bottom: 1px solid #e2e8f0; background: #fefce8;">
            <td style="padding: 6px 12px; text-align: center; font-family: monospace; font-weight: 700; color: #b45309;">$0.5$</td>
            <td style="padding: 6px 12px; text-align: center; font-family: monospace; font-weight: 700; color: #b45309;">$0.3521$</td>
            <td style="padding: 6px 12px; text-align: center; font-family: monospace; font-weight: 700; color: #b45309;">$0.1760$</td>
            <td style="padding: 6px 12px; text-align: center; font-family: monospace; font-weight: 700; color: #b45309;">$\approx 0.78$</td>
        </tr>
        <tr>
            <td style="padding: 6px 12px; text-align: center; font-family: monospace;">$1.0$</td>
            <td style="padding: 6px 12px; text-align: center; font-family: monospace;">$0.2420$</td>
            <td style="padding: 6px 12px; text-align: center; font-family: monospace;">$0.1210$</td>
            <td style="padding: 6px 12px; text-align: center; font-family: monospace;">$0.8968$</td>
        </tr>
    </tbody>
</table>
...
That's it. The $\int$ symbol in the original equation is the limit of this sum as $\Delta t$ shrinks to zero: **the integral is a sum of rectangles, taken to the limit.**
...
For this, he developed the **Method of Least Squares**: instead of a line passing through every noisy data point, he sought the most probable path — the one that minimizes the sum of the squares of the errors.
</div>
...
The origins of the CLT lie in the 18th-century struggle for precision in the physical sciences, where **\citeauthor{laplace1810clt}** formalized the theorem in \citeyear{laplace1810clt} to solve the “Problem of Errors.”

Astronomers of the era faced a dilemma: every measurement taken via telescope or pendulum was slightly “noisy” due to atmospheric disturbances or human imperfection. Laplace proved that the **average** of these independent errors follows a bell curve, regardless of the shape of the individual error distribution. By aggregating many imprecise data points, one can mathematically “filter” the noise to recover the true position of a celestial body — a method that remains the foundation for how machines learn from “imperfect” real-world data.
...
When we have “noisy” data points, we find the best-fitting line by minimizing the sum of the squares of the distances between the data and the line.
...
<div class="statlab-section">
    <div class="md" data-mathlevel="65" data-optionaltitle="The Gumbel Distribution: The Math of Disasters">
        ...
        It is **asymmetrical** (skewed): the right tail, representing extreme “worst-case” values, decays much more slowly than the left tail. The PDF is:
        ...
        - **$\beta$ (Scale):** How spread out the extreme values are (the width of the disaster tail).
    </div>
    <div class="statlab-interactive-zone">
...
<div class="statlab-section">
    <div class="md" data-mathlevel="55" data-optionaltitle="The Poisson Distribution: The Law of Rare Events">
        ...
        It became known as the **“Law of Small Numbers”** after \citeauthor{gesetzderkleinenzahlen} applied it to the number of Prussian soldiers killed by horse kicks (p. 23f, §12) — events that are rare but occur at a constant average rate $\lambda$.
...
* **The Shared Signal**: He found a correlation of **$r \approx 0.5$** for height: a strong link, but not a 1:1 “perfect” copy.
...
Zipf's law is observed in many seemingly unrelated domains: city populations, income, web traffic, and more. The mathematical reason is debated; competing theories include preferential attachment (“the rich get richer”), information-theoretic optimization, and self-organized criticality.
...
The **Dirichlet distribution** (next chapter) is the natural probability distribution for Zipfian categorical data.
</div>
