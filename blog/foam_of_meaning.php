...
<style>
/* reuse the ps- design system; add a spoiler lock */
.ps-card { background: var(--mn-surface); border: 1px solid var(--mn-border); border-radius: var(--mn-radius-md); padding: 1rem 1.15rem; margin: 1.3rem 0; box-shadow: var(--mn-shadow-md); }
.ps-card-title { font-weight: 600; color: var(--mn-accent); margin-bottom: .7rem; display: flex; align-items: center; gap: .5rem; font-family: var(--mn-font-heading); font-size: 1.02rem; }
.ps-card-title .dot { width: 8px; height: 8px; border-radius: 50%; background: var(--mn-accent); box-shadow: 0 0 10px var(--mn-accent); flex: 0 0 auto; }
.ps-lead { color: var(--mn-text-secondary); }
...
.ps-callout.a { border-color: var(--mn-emerald); background: var(--mn-emerald-light); }
.ps-callout.q { border-color: var(--mn-accent4); }
.ps-example { background: var(--mn-bg-warm); border-left: 3px solid var(--mn-coral); padding: .7rem 1rem; border-radius: 0 8px 8px 0; margin: .6rem 0; }
.ps-btn { background: var(--mn-accent); color:#fff; border:0; padding:.55rem 1rem; border-radius:8px; font-weight:600; cursor:pointer; font-size:.85rem; }
.ps-btn.lock { background: var(--mn-coral); }
.ps-btn:hover { filter: brightness(1.12); }
.ps-spoiler { display:none; }
</style>

<div class="md">
# Homology by hand
## Level 2 — topological invariants you can actually compute on paper

The most computable invariant in the whole programme is the sequence of **Betti numbers** $b_0,b_1,b_2,\dots$ counting clusters, loops, and voids. Once a piece of $X$ or $S$ is modelled as a finite **simplicial complex** (exactly what the Vietoris–Rips construction produces), computing them is nothing but **linear algebra**: ranks, kernels, the Dimensionssatz. Perfect for Abitur-level tools.

Below: (0) the setup, (1) one fully worked example, (2) a genuinely different task for you to do by hand, (3) self-check questions whose answers are locked behind a conscious-cheating warning.
</div>

<div class="ps-card">
<div class="ps-card-title"><span class="dot"></span>0. Setup — the one formula you must internalize</div>

<p class="ps-lead">Order the vertices by index (orientation controls the signs). Chain groups over $\mathbb{R}$:</p>

<div class="ps-math">$C_n=\text{vector space of formal }\mathbb{R}\text{-combinations of the }n\text{-simplices}$</div>

<p class="ps-lead">The boundary map (delete each vertex in turn, with alternating sign):</p>

<div class="ps-math">$\partial[v_0,\dots,v_n]=\displaystyle\sum_{i=0}^{n}(-1)^i\,[v_0,\dots,\widehat{v_i},\dots,v_n]$</div>

<p class="ps-lead">Concretely:</p>

<div class="ps-math">$\underbrace{\partial[v_i,v_j]}_{\text{edge}}=\underbrace{v_j-v_i}_{\text{head}-\text{tail}}
\qquad
\underbrace{\partial[v_i,v_j,v_k]}_{\text{triangle}}=\underbrace{[v_j,v_k]}_{+}\underbrace{-[v_i,v_k]}_{-}\underbrace{+[v_i,v_j]}_{+}$</div>

<p class="ps-lead">The miracle that makes it work: $\partial\circ\partial=0$. Homology and Betti numbers:</p>

<div class="ps-math">$H_n=\dfrac{\overbrace{\ker\partial_n}^{\text{cycles}}}{\underbrace{\operatorname{im}\partial_{n+1}}_{\text{boundaries}}}
\qquad
\boxed{\,b_n=\underbrace{\dim C_n}_{\#\,n\text{-simplices}}-\underbrace{\operatorname{rank}\partial_n}_{}-\underbrace{\operatorname{rank}\partial_{n+1}}_{}\,}$</div>

<p class="ps-lead">from the Dimensionssatz $\dim\ker\partial_n=\dim C_n-\operatorname{rank}\partial_n$. Dictionary:
$b_0=\#$ clusters, $b_1=\#$ loops, $b_2=\#$ enclosed voids.</p>
</div>

<div class="ps-card">
<div class="ps-card-title"><span class="dot"></span>1. Worked example — the hollow triangle (a loop around a void)</div>

<p class="ps-lead">Three vertices $v_0,v_1,v_2$, all three edges present, but the middle <em>not</em> filled.</p>

<div class="ps-example">
Simplices: $\dim C_0=3$ (three vertices); $\dim C_1=3$ (edges $a=[v_0,v_1],\ b=[v_1,v_2],\ c=[v_0,v_2]$); $\dim C_2=0$ (no triangle).
</div>

<p class="ps-lead"><b>Step 1 — write $\partial_1$</b> using $\partial[v_i,v_j]=v_j-v_i$:</p>

<div class="ps-math">$\partial a=v_1-v_0,\quad \partial b=v_2-v_1,\quad \partial c=v_2-v_0$</div>

<div class="ps-math">$\partial_1=
\begin{array}{c|ccc}
 & a & b & c\\ \hline
v_0 & -1 & 0 & -1\\
v_1 & 1 & -1 & 0\\
...
<p class="ps-lead"><b>Step 2 — rank.</b> Column $c=$ column $a+$ column $b$ (check: $(v_1-v_0)+(v_2-v_1)=v_2-v_0$ ✓). So $a,b$ independent, $c$ dependent $\Rightarrow \operatorname{rank}\partial_1=2$.</p>

<p class="ps-lead"><b>Step 3 — trivial maps.</b> $\operatorname{rank}\partial_0=0$ (points have no boundary), $\operatorname{rank}\partial_2=0$ (no triangles).</p>

<p class="ps-lead"><b>Step 4 — master formula:</b></p>

<div class="ps-math">$b_0=\underbrace{3}_{C_0}-\underbrace{0}_{\partial_0}-\underbrace{2}_{\partial_1}=1
\qquad
b_1=\underbrace{3}_{C_1}-\underbrace{2}_{\partial_1}-\underbrace{0}_{\partial_2}=1
\qquad
b_2=0$</div>

<div class="ps-readout">(b0, b1, b2) = (1, 1, 0)  →  one cluster, one loop, no cavity: a circle.
Generating cycle: a + b − c   (check ∂ = 0 ✓)</div>
</div>

<div class="ps-card">
<div class="ps-card-title"><span class="dot"></span>2. Your task — the hollow tetrahedron ∂Δ³ (a shell wrapping a 3D void)</div>

<p class="ps-lead">The <em>surface</em> of a tetrahedron: four triangular faces, solid interior left empty. Genuinely harder than the example because now $C_2\neq 0$, so you must handle the <b>signed</b> triangle boundary map.</p>

<div class="ps-example">
Vertices $v_0,v_1,v_2,v_3$ → $\dim C_0=4$.<br>
Edges (increasing order) → $\dim C_1=6$: &nbsp; $e_{01},e_{02},e_{03},e_{12},e_{13},e_{23}$.<br>
Faces (omit one vertex each) → $\dim C_2=4$: &nbsp;
$F_0=[v_1,v_2,v_3],\ F_1=[v_0,v_2,v_3],\ F_2=[v_0,v_1,v_3],\ F_3=[v_0,v_1,v_2]$.<br>
No solid 3-cell → $\dim C_3=0$, so $\partial_3=0$.
</div>

<p class="ps-lead">Do these steps, writing everything out as in the example:</p>

<div class="md">
**(a)** Build the $4\times 6$ matrix of $\partial_1$ using $\partial e_{ij}=v_j-v_i$.

**(b)** Find $\operatorname{rank}\partial_1$ by reduction. (Shortcut to *check against, not skip*: for a connected graph, $\operatorname{rank}\partial_1=\#\text{vertices}-1$.)

**(c)** Build the $6\times 4$ matrix of $\partial_2$. Mind the alternating signs! Example:
</div>

<div class="ps-math">$\partial_2 F_0=\partial[v_1,v_2,v_3]=\underbrace{[v_2,v_3]}_{+}\underbrace{-[v_1,v_3]}_{-}\underbrace{+[v_1,v_2]}_{+}=e_{23}-e_{13}+e_{12}$</div>

<div class="md">
Work out $\partial_2F_1,\partial_2F_2,\partial_2F_3$ the same way and assemble the matrix.

**(d)** Find $\operatorname{rank}\partial_2$.

**(e)** Find $\dim\ker\partial_2=\dim C_2-\operatorname{rank}\partial_2$, and if nonzero, **solve** $\partial_2\alpha=0$ for $\alpha=(\text{coeff of }F_0,\dots,F_3)$ — this is the shell as a chain (the fundamental class).

**(f)** Compute $(b_0,b_1,b_2)$ from $b_n=\dim C_n-\operatorname{rank}\partial_n-\operatorname{rank}\partial_{n+1}$.
</div>
</div>

<div class="ps-card">
<div class="ps-card-title"><span class="dot"></span>3. Self-check — pick one letter per question</div>

<div class="ps-callout q">
<b>Q1.</b> $\operatorname{rank}\partial_1=$ &nbsp;(a) 2 &nbsp;(b) 3 &nbsp;(c) 4 &nbsp;(d) 6<br><br>
<b>Q2.</b> $\operatorname{rank}\partial_2=$ &nbsp;(a) 2 &nbsp;(b) 3 &nbsp;(c) 4 &nbsp;(d) 6<br><br>
<b>Q3.</b> $\dim\ker\partial_1=$ &nbsp;(a) 1 &nbsp;(b) 2 &nbsp;(c) 3 &nbsp;(d) 4<br><br>
<b>Q4.</b> Which chain is the generating 2-cycle (a basis of $\ker\partial_2$)?<br>
&nbsp;&nbsp;(a) $F_0+F_1+F_2+F_3$ &nbsp;(b) $F_0-F_1+F_2-F_3$<br>
&nbsp;&nbsp;(c) $F_0+F_1-F_2-F_3$ &nbsp;(d) $F_0-F_1-F_2+F_3$<br>
<span class="ps-lead">(Only one satisfies $\partial_2\alpha=0$. You must have the signed $\partial_2$ — plug each into the $e_{01}$-row to settle it.)</span><br><br>
<b>Q5.</b> $(b_0,b_1,b_2)=$ &nbsp;(a) $(1,1,0)$ &nbsp;(b) $(1,0,1)$ &nbsp;(c) $(1,0,0)$ &nbsp;(d) $(1,1,1)$<br><br>
<b>Q6.</b> $\partial\Delta^3$ is topologically $S^2$. In the foam dictionary your nonzero $b_2$ means:<br>
&nbsp;&nbsp;(a) two clusters &nbsp;(b) a loop/filament circuit<br>
&nbsp;&nbsp;(c) an enclosed void wrapped by a closed shell &nbsp;(d) a fractal boundary
</div>

<!-- ── conscious-cheating spoiler lock ── -->
<button class="ps-btn lock" id="ps-unlock"
  onclick="if(confirm('⚠️ CHEATING WARNING\n\nThis reveals the answer key.\n\nHave you ALREADY written out both boundary matrices, reduced them by hand, and solved ∂₂α = 0 on paper?\n\nClick CANCEL to keep working.\nClick OK only if you have genuinely finished.')){document.getElementById('ps-answers').style.display='block';this.style.display='none';}">
  &#128274; Reveal answers (fires a cheating warning)
</button>

<div class="ps-spoiler" id="ps-answers">
<div class="ps-callout a">
<b>Answer key — you consciously chose to look.</b>
<div class="ps-readout">Q1  (b) 3      rank ∂₁ = #vertices − 1 = 4 − 1 = 3, confirmed by reduction
Q2  (b) 3      the four face-boundaries span a rank-3 subspace of C₁
Q3  (c) 3      dim ker ∂₁ = dim C₁ − rank ∂₁ = 6 − 3 = 3
Q4  (d) F₀ − F₁ − F₂ + F₃     (this is the alternating-sign shell)
     e₀₁-row check: only F₂,F₃ contain e₀₁, with signs from
     ∂₂F₂ = e₁₃ − e₀₃ + e₀₁ and ∂₂F₃ = e₁₂ − e₀₂ + e₀₁,
     so coeff of e₀₁ in α = −(+1) + (+1) = 0  ✓ (all six edge-rows vanish)
Q5  (b) (1,0,1)
     b₀ = 4 − 0 − 3 = 1
     b₁ = 6 − 3 − 3 = 0
     b₂ = 4 − 3 − 0 = 1
Q6  (c) an enclosed void wrapped by a closed shell — exactly H₂(S²) = ℝ</div>
Send me your two boundary matrices, your reductions, your kernel solution for Q4, and your six letters, and I'll check every step and point out any slip.
</div>
</div>
</div>

<div class="md">
### Why this is cheat-resistant
- **Q1/Q3** force you to actually reduce the $4\times 6$ matrix (the connectedness shortcut only *confirms*).
- **Q2** requires the *signed* $\partial_2$; one wrong sign changes the rank.
- **Q4** offers four sign-patterns that look identical to a guesser; only the true one lies in the kernel.
- **Q5** chains off every earlier rank, so a single arithmetic slip propagates and shows.

Finished this? Ask for the **boss level**: the *solid* tetrahedron $\Delta^3$ (contractible, $(1,0,0)$) or a two-loop "theta" complex where $b_1=2$ — so you can feel the difference between filling a cell and leaving it empty.
</div>
...
- \citeauthor{he2023sideeffects}, \citealternativetitle{he2023sideeffects} (\citeyear{he2023sideeffects}) — proves that a network trained on low-dimensional data is pinned down on the data manifold while its variation in the normal (void) directions is fixed by initialization alone; the sample-count price of regularizing the voids ($N \sim \sigma^{-2}$) and the slow-convergence price of doing so.
- \citeauthor{jablonka2025geometry}, \citealternativetitle{jablonka2025geometry} (\citeyear{jablonka2025geometry}) — an accessible statement of the same geometry: learning is the fitting of smooth functions to low-dimensional structure, and every failure mode (shift, adversarial perturbation, extrapolation) is a query that leaves the manifold.
