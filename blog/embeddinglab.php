...
A critical property of embedding spaces is their **rotational invariance**. If you take an entire embedding space and rotate it, spinning every single vector by the same angle, nothing changes semantically. "King" is still near "Queen," "Cat" is still far from "Democracy." This is because meaning in these spaces is not encoded in absolute coordinates, but in the **relational geometry** between points: their distances, angles, and cluster structures.

This has a profound implication for **cross-lingual translation**. When a Transformer is trained on two languages, each language develops its own embedding space. Remarkably, these spaces tend to be **isomorphic**, they share the same internal geometric structure, just rotated and scaled relative to each other. The word for "king" in French and the word for "king" in English occupy analogous positions within their respective manifolds. Aligning two such spaces often requires nothing more than a linear transformation, a rotation matrix and a scaling factor, because the underlying topology of human concepts is, to a significant degree, language-invariant.
