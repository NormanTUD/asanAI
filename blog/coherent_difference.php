...
\text{Deep ReLU net (Origami)} & \text{Crease = Distinction; regions = Locality; continuity on the crease = Compatibility}
\end{array}
$$
</div>
...
(Apply the mark twice and you are back where you started) From this axiom, the entire Boolean calculus of propositions falls out \cite{spencerbrown1969form}. The point, for us, is not the algebra. The point is that Spencer-Brown placed *distinction itself* at the foundation of mathematics: the act that splits one world into two is the act from which everything else is built. In this book the most concrete instance of that first cut is the **ReLU crease** in the <a href="origami">Origami</a> chapter: one hyperplane is a single line drawn across the activation space — one distinction — and a deep network is a stack of such lines, whose arrangement is a *space* carved out entirely of cuts. You will meet it there as geometry you can draw.
...
- **Trichotomous**: for every $a, b$, *exactly* one of $a \mathrel{R} b$, $b \mathrel{R} a$, $a = b$ holds. Total says "at least one"; trichotomy says "exactly one". On $\mathbb{Z}$ with $<$, every pair splits cleanly into one of the three cases.
...
- **Partial equivalence relation** (PER): symmetric + transitive, *not* necessarily reflexive. The missing reflexivity is the point: equivalence classes form only inside some subset of $A$, the rest is untouched. A **partial function**, read as a relation, is a **functional** (right-unique) one — at most one $b$ for each $a$; a PER is a different, stronger object (symmetric + transitive) that records a partial *equivalence*, not a general partial map.
...
- **Universal relation**: $A \times A$. Every element relates to every element (including itself). The maximal relation under $\subseteq$, the identity of the algebra of relations under meet.
- **Empty relation**: $\varnothing$. Nothing relates to anything. The minimal relation under $\subseteq$, the identity under join.
- **Identity relation** (or *diagonal*): $\{(a, a) : a \in A\}$. Each element relates only to itself. Reflexive, symmetric, transitive, *and* coreflexive — the smallest equivalence relation, the prototype of equality. Universal, empty, and identity are the $1$, $0$, and diagonal of the relation lattice.
...
Open sets are exactly this: for each point $x$, the collection of all subsets that “contain $x$ with a little bit of room around it”. You don't measure the room. You just ask whether the room is there at all. A set is “open” if every point inside it has some room around it still inside the set. (Formally: $U$ is open iff for every $x \in U$ there exists an open $V$ with $x \in V \subseteq U$.)
...
This is the formal crystallization of the intuition. Sheaves (faisceaux) were introduced by Jean Leray in the late 1940s \cite{leray1948calcul} \cite{sheaf_mathematics} and developed into their general form by Alexander Grothendieck \cite[Alexander Grothendieck]{grothendieck_sheaf} (\citeyear{grothendieck_sheaf}) in the 1950s and 60s, precisely to handle questions that resisted the older “patch a space together by gluing charts” picture of \citetitle{manifold_wiki}. Where manifolds ask you to specify a single chart and a transition map, sheaves ask you to specify local data on every open set and let the global object fall out of the consistency conditions. The local-to-global direction is so productive that it is the entire backbone of algebraic geometry and much of modern logic.
...
In the early twentieth century, palaeontologists found fossils of the same species on continents now separated by oceans. *Cynognathus* and *Lystrosaurus* on Africa and South America; *Glossopteris* flora across Africa, South America, India, Antarctica, and Australia; *Mesosaurus* only in southern Africa and southern South America. Each continent was an open set $U_i$ carrying local data — the fossil record found there. The overlaps were the distributions: the same species appearing on two continents meant the local sections had to agree on the pairwise intersection.

The **Fixismus** — the prevailing geological orthodoxy — held that the continents had always been in their current positions. The sheaf problem was clear: the local data (fossils of the same terrestrial species on land masses separated by thousands of kilometres of ocean) did not glue. The overlap said "these two continents were once connected"; the geography said "they are not." The Fixists proposed a **presheaf solution**: land bridges that once linked the continents and later sank beneath the sea. This was an attempt to define transition maps that would make the local sections compatible — to *force* the presheaf to glue by postulating hidden infrastructure.

It did not work. The land bridges left no geological trace on the ocean floor, and the Permanenztheorie — the doctrine that continents never move — sat uneasily beside the very data it was supposed to explain. The presheaf was not a sheaf: the local sections could not be extended to a global section without inventing auxiliary data (the bridges) that themselves refused to cohere with the rest of the picture.

The resolution came from **Mobilismus**. Alfred Wegener proposed in 1912 that the continents had once formed a single landmass — Pangaea — and drifted apart. The fossil distributions were not anomalies to be explained away; they were *compatible local sections* on a single, larger open set that had since been split. The sheaf condition was not violated; the *site* had been misread. The open sets were not static continents in fixed positions; they were regions of a moving surface, and the overlaps had to be computed in the surface's own history, not in its current geometry.

Echolot measurements of the Mid-Atlantic Ridge in the 1920s confirmed a divergent plate boundary running north–south — exactly the structure Mobilismus predicted, and exactly the structure the land-bridge hypothesis could not accommodate. By the 1960s, plate tectonics had unified the evidence into a global section: a single coherent picture in which the local data from every continent's fossil record, rock formations, magnetic striping, and seismic activity glues uniquely. The sheaf condition held — once the right site was chosen.

The lesson, abstracted: **a presheaf that does not glue is not necessarily a failure of the data — it may be a failure of the site.** The local observations were fine. The overlapping distributions were fine. What was wrong was the background structure against which "overlap" and "compatibility" were being measured. Change the site — from a fixed geography to a moving one — and the same local sections that refused to glue now glue perfectly, producing a unique global section: the history of the Earth's surface.

This is the sheaf-theoretic structure of a paradigm shift. The old paradigm declares the presheaf unglueable and invents auxiliary hypotheses (land bridges) to force a global section. The new paradigm changes the site, and the global section falls out for free.
</div>
...
And the sheaf condition has teeth. Consider the unit circle $S^1$ in the plane. On any small arc (less than a full loop), you can define a continuous *angle function*: a smooth "clock reading" in radians that tells you where on the circle you are. But look at the *overlap* between two such arcs: because one full turn is indistinguishable from no turn, the two readings differ by a whole number of full turns — a constant $2\pi k$ on that overlap. They agree *exactly* there only when that integer happens to be $0$, and in general they do not.

Gluing them into one continuous angle function on the *entire* circle would require every pair of local readings to agree *exactly* on every overlap — and they do not. Walk all the way around, and your reading has crept up by $2\pi$ (one full rotation in radians); when you return to your starting point it must be both $0$ and $2\pi$ at once — a contradiction. The data is *locally* consistent but not compatible on the overlaps, and that is exactly why the global object does not exist.
...
It is also the one place the *principle of continuity* — *Natura non facit saltus*, "nature does not make jumps" \cite{naturanonfacitsaltus} — stops being a local fact and becomes a global obstruction. On any small arc the principle holds trivially: there is no jump to hide, so the local angle readings glue. The circle is the first space where the principle is *locally* satisfied at every point yet *globally* violated — the $2\pi$ jump is invisible on every patch, yet it accumulates around the hole into exactly the winding number that blocks the global section.
...
* **Grothendieck topos.** Defined, up to equivalence, as the category of sheaves on a *site* — a small category equipped with a notion of "covering families". Equivalently (by a theorem of Jean Giraud \cite{giraud1964descente}) a category $\mathcal{E}$ satisfying four explicit axioms: a small set of *generators*, all small colimits, sums that are *disjoint* (the fiber product of $X$ and $Y$ over their sum $X \sqcup Y$ is the initial object), and equivalence relations that are *effective* \cite{topos_wiki}. This is the topos most relevant to algebraic geometry.
...
* **Unique factorization in Boolean form.** In *every* topos, each morphism factors uniquely (up to isomorphism) as an epimorphism followed by a monomorphism, and its coimage and image coincide (the canonical map $\mathrm{coim}\, f \to \mathrm{im}\, f$ is an isomorphism). What fails in a non-Boolean topos is the *Boolean* treatment of the resulting subobjects that $\mathbf{Set}$ uses: a subobject need not have a complement, and excluded middle for subobjects can fail.
...
* **Barr's theorem.** Every Grothendieck topos admits a surjective geometric morphism from a topos satisfying the axiom of choice — which, by Diaconescu's theorem, is therefore Boolean (Barr 1974 \cite{barr1974toposes}). In particular, a geometric statement that holds in *all* Boolean toposes already holds in every Grothendieck topos, so results proved in a 'classical' setting often transfer to the general case.
...
8. **Is there a "fundamental theorem of toposes"?** Not in the sense of Galois theory. But Barr's theorem \cite{barr1974toposes} says every Grothendieck topos is covered by a Boolean topos, and Diaconescu's theorem gives the AC $\Rightarrow$ LEM direction; together with Giraud's theorem \cite{giraud1964descente} classifying Grothendieck topoi by their colimit structure, these are the closest things to a "fundamental theorem" the subject has. Whether a deeper unifying theorem is waiting to be found is open.
...
$$
\underset{\text{grammar}}{\text{coherent difference}}
\;\to\;
\underset{\text{world}}{\text{topos}}
\;\to\;
\underset{\text{geometry}}{\text{manifold}}
\;\to\;
\underset{\text{linear}}{\mathbb{R}^d}
\;\to\;
\underset{\text{lattice}}{\text{float}^{d}}
\;\to\;
\underset{\text{substrate}}{\text{silicon}}
$$
