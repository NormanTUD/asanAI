...
function toc() {
	...
	if (!tocDiv) {
		...
		tocDiv.id = 'toc';
		tocDiv.innerHTML = '';
		// functions.php emits #lesson-home as the first child of #contents.
		// Slip in behind it so the link reads as a breadcrumb ABOVE the TOC
		// instead of the TOC jumping in front of it. Falls back to prepend()
		// on index_full.php, which has no home link.
		var homeLink = document.getElementById('lesson-home');
		if (homeLink && homeLink.parentNode === contents) {
			contents.insertBefore(tocDiv, homeLink.nextSibling);
		} else {
