...
The idea of modeling language statistically predates computers themselves. In the 1940s, \citeauthor{shannon1948communication} applied information theory to English text, treating language as a stochastic process and showing that **n-gram models**, which predict the next word from the previous *n* words, could capture statistical regularities in language. His 1948 paper, \citetitle{shannon1948communication}, laid the mathematical foundation for all subsequent language modeling. Through the 1980s and 1990s, n-gram-based statistical language models dominated speech recognition and machine translation, championed by researchers like Frederick Jelinek at IBM. Meanwhile, **ELIZA** (1966), created by Joseph Weizenbaum, demonstrated early natural language interaction through simple pattern matching, but had no statistical understanding of language whatsoever [[1]](file://history.php).

The neural revolution in language modeling began with **Yoshua Bengio et al.'s** 2003 paper *A Neural Probabilistic Language Model*, which replaced sparse n-gram tables with dense, continuous **word embeddings** — learned vector representations capturing semantic similarity [[1]](file://history.php). This was supercharged in 2013 by **Word2Vec** (Tomáš Mikolov et al., Google), which efficiently trained embeddings on large corpora and revealed striking algebraic properties of language (e.g., *king − man + woman ≈ queen*). In 2018, **ELMo** (Peters et al., Allen Institute for AI) introduced **contextualized embeddings** that changed depending on surrounding context, finally addressing polysemy.

The real inflection point came with the **Transformer** architecture (Vaswani et al., 2017), which replaced sequential processing with parallelizable **self-attention** [[1]](file://history.php). Two competing branches emerged almost simultaneously. **BERT** (Jacob Devlin et al., Google, 2018) used the Transformer's *encoder* with **Masked Language Modeling** — hiding random words and predicting them from both directions — to produce deeply bidirectional representations. A single pre-trained BERT model, fine-tuned with minimal data, shattered nearly every NLP benchmark overnight and popularized the **pre-train then fine-tune** paradigm. In parallel, OpenAI pursued the *decoder-only* path: **GPT-1** (Radford et al., 2018) showed that autoregressive next-token prediction over a large Transformer could learn powerful representations [[1]](file://history.php). **GPT-2** (2019) demonstrated emergent zero-shot abilities, and **GPT-3** (2020), with 175 billion parameters, proved that sheer scale could produce remarkably flexible few-shot learners. Combined with **Reinforcement Learning from Human Feedback (RLHF)**, as described in the InstructGPT paper (Ouyang et al., 2022) [[1]](file://history.php), this lineage culminated in **ChatGPT** in November 2022 — the moment large language models crossed from research artifact into mainstream cultural phenomenon.

The trajectory from Shannon's n-grams to ChatGPT vindicates a consistent theme: each generation traded hand-crafted linguistic knowledge for greater scale and more general learning — raw computation and data, given the right architecture, eventually surpassing human-designed heuristics.
...
Then came another breakthrough, the \citealternativetitle{vaswani2017attention}. By utilizing a mechanism called **Self-Attention**, models could process entire sequences of data in parallel rather than word-by-word. This solved the "vanishing gradient" problem and allowed models to understand long-range context in text. The further text will lead you through every step you need to understand this Self-Attention-Mechanism on a basic level.
