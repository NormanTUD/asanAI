...
### Four ways to build a mind

Naming the field was only the beginning. The harder question was *what an intelligent machine should be measured against*, and it splits the space into four targets. \citeauthor{russell2021aima} lay it out along two axes — should the system *think* or merely *act*, and should it be judged against *human behavior* or against *rational optimality* \cite{russell2021aima}:

* **Acting humanly** — the behavior test: pass if the machine conducts itself like a person.
* **Thinking humanly** — the cognitive-model test: pass if it reasons the way people do.
* **Acting rationally** — the rational-agent test: pass if it does the best it can, by any means.
* **Thinking rationally** — the “laws of thought” test: pass if it reasons correctly.

This taxonomy is not a footnote; it is the hidden fault line of the first winter. The Toosi review is explicit that the 1960s field mostly chased **thinking humanly** — replicating the *way* humans solve a problem rather than analyzing the problem's actual complexity — and that this choice, more than any single failed system, set up the over-promise the funding cuts then punished \cite{toosi2021history}. The programs that follow live in the top row of the grid: the **Logic Theorist** and **GPS** imitated human problem-solving, and the Perceptron was built to *behave* like a learner. The LLMs of the course's later chapters are, by contrast, largely **acting rationally** — not asked to think like us, only to do the task well.
...
Toosi et al. frame the whole arc as a recipe for the next crash, because the same over-promise that built each summer is what the subsequent funding freeze punished \cite{toosi2021history}. Their review of where the field stands now shows how high the third summer has climbed, which is precisely what makes a fourth winter expensive: global venture investment in AI startups grew from roughly $1.3 billion in 2010 to more than $40 billion by 2020 (and about $70 billion in 2020 alone), the number of peer-reviewed AI articles per year multiplied roughly twelve-fold between 2000 and 2020, and NeurIPS attendance grew tenfold since 2012 \cite{toosi2021history}. When the stakes are that large, the difference between a field that matures and one that crashes is how well it disciplines its own hype.

Their proposed guardrails therefore read like a checklist for the present moment. Be sober about what counts as intelligence and avoid claims that machines have “become” human; resist extrapolating from toy problems to the general case on the assumption that faster hardware will absorb the difference; define the terms “AI”, “machine learning” and “deep learning” precisely, since their ambiguity has already become a cover for marketing; and treat the field's methodological lapses — confusing explanation with speculation, misattributing where performance gains come from, and misleading use of mathematics — as systemic risks rather than isolated slips \cite{toosi2021history}. Two of these deserve to outlive the rest. The **AI effect** is the field's moving target: a technique is only called “AI” while it is still unsolved, and the moment it is cracked it graduates into an ordinary engineering discipline, and the excitement moves to the next frontier \cite{toosi2021history}. And the **black box** is the standing objection to deep learning: the model often cannot say how it reached a decision, which is tolerable for a game but not for medicine, transport, or finance, where a wrong judgment has a human cost and regulation increasingly demands that it be retraced \cite{toosi2021history}. The interpretability chapters of this course are the direct answer to that demand.
</div>
