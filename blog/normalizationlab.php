...
LayerNorm has a beautiful geometric interpretation: after centering (subtracting the mean) and normalizing by the standard deviation, the output vector $\hat{x}$ lies on the surface of a **sphere of radius $\sqrt{d}$** in $d$-dimensional space. The learnable parameters $\gamma$ and $\beta$ then stretch and shift this sphere.
...
**Step 2 - Normalization:** Dividing by the standard deviation $\sigma$ scales each vector to have length $\sqrt{d}$. Every vector now lies on the surface of a $d$-dimensional hypersphere of radius $\sqrt{d}$. The direction of the vector, the relative pattern of activations, is preserved, but its magnitude is standardized.
...
The key insight: **LayerNorm separates direction from magnitude**. Only the *direction* of the activation vector carries information about the token's identity and context. The *magnitude* is discarded because it's unreliable, it can vary due to accumulated activations, layer depth, or input length. By projecting onto a sphere of radius $\sqrt{d}$, LayerNorm forces the model to encode all information in angular relationships alone, making learning more stable and less sensitive to the absolute scale of activations.
