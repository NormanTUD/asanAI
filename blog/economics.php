...
<div class="md">
A frontier LLM costs ~$100M to train, ~$1M/day to serve at scale, and ~$0.01 per typical user query. Who pays for this? How do companies make money? When does self-hosting beat APIs? When do small open models beat GPT-4o?

This chapter is the financial-engineering complement to the technical chapters above.
</div>
...
* **Compute**: ~85% of the bill for frontier training. Llama 3 405B used ~30 million H100-hours, ≈ $100M at $3/hour.
...
For a 70B model on an H100 with vLLM continuous batching: ~$0.0003 per 1K output tokens at retail GPU prices, ~$0.0001 at hyperscaler rates.
</div>
...
* **API**: per-token pricing, $2.50–$10 per 1M output tokens for GPT-4o.
* **Revenue**: estimated $5–10B annualized in 2025, growing 3× year-over-year.
* **Strategy**: closed frontier models + consumer brand.

### Anthropic (closed)

* **Claude API + Enterprise**. Estimated $5B annualized revenue in 2025.
...
* **DeepSeek-V3, R1**: open weights + cheap API ($0.27/M input tokens).
...
| 70B tokens/s throughput (16x H100) | $0.0004/1K | $0.0002/1K |
...
* 2025: DeepSeek V3 ~$0.001 / 1K tokens.
...
* **Architectural improvements**: FlashAttention, paged attention, speculative decoding.
* **Distillation**: 70B models approximating 400B quality.
...
* **EU**: AI Act + efficiency-first approach; "AI factories" with subsidized compute.
...
A 2024 Goldman Sachs report estimated 300M jobs exposed to AI automation; 7% of US employment could be displaced if generative AI scales rapidly. The actual outcome: **augmentation > replacement** for most knowledge work, with new roles emerging faster than old ones disappear.
