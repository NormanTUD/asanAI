...
\begin{frame}{Positional Encoding – Spiralförmige Nachbarschaft}
  \begin{columns}[T]
    \column{0.55\textwidth}
    \begin{itemize}
      \stich{Jedes Wort ist \textbf{kein isolierter Punkt} im Raum}
      \stich{Durch das Positional Encoding entsteht ein
             \textbf{spiralartiger Bereich} um jedes Wort}
      \stich{Dieser Bereich = die ``Gegend'' des Wortes,
             in der seine Positionsinformation kodiert ist}
      \stich{Benachbarte Positionen → überlappende Spiralen
             → \textbf{relative Abstände} bleiben erhalten}
      \stich{Hochdimensional: viele Frequenzen überlagert
             → einzigartiger Fingerabdruck pro Position}
    \end{itemize}

    \column{0.4\textwidth}
    \begin{center}
      \includegraphics[width=\linewidth, keepaspectratio]{posembed.png}\\[0.3em]
      {\scriptsize\color{gray}Visualisierung: Jedes Wort umgeben von einem
       spiralförmigen Bereich – die ``Gegend'' seiner Position im Embedding-Raum}
    \end{center}
  \end{columns}
  ...
  \begin{center}
    \colorbox{dunkel!10}{%
      \parbox{0.85\textwidth}{\centering\small
        \textbf{Intuition:} Ein Wort an Position $k$ wird nicht zu einem Punkt,
        sondern zu einer \textbf{Region} – definiert durch die Überlagerung
        verschieden schneller Rotationen (Sinus/Cosinus bei verschiedenen Frequenzen).
      }%
    }
  \end{center}
\end{frame}
