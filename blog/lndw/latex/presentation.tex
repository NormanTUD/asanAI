% ============================================================
%  KI-Präsentation – LaTeX Beamer
%  Kompilieren mit: pdflatex → biber → pdflatex (×2)
%  Alle Bilder sind als \placeholder markiert und leicht entfernbar.
% ============================================================
...
\begin{frame}{Dense Layer – Grundidee}
  \formelbox{f(\mathbf{x}) = A\,\mathbf{x} + \mathbf{b}}
  \begin{itemize}
  ...
  \placeholder[0.35\textwidth]{Bild: Dense Layer – Knoten voll verbunden}
\end{frame}
...
\begin{frame}{Wie approximiert ein Netz eine Funktion?}
  \begin{center}
    \begin{tikzpicture}[
      ...
      % ── Zielfunktion (glatte Kurve) ──
      \draw[thick, dunkel, domain=0:7, samples=100, opacity=0.35]
        plot (\x, {0.4*sin(60*\x) + 0.08*\x*\x - 0.15*\x + 1.2})
        node[right, opacity=1] {Zielfunktion $f(x)$};

      % ── Einzelne ReLU-Neuronen (verschoben & skaliert) ──
      %  Neuron i:  w_i * max(0, x - c_i)
      %  Jedes Stück wird einzeln gezeichnet und beschriftet

      % Neuron 1: startet bei x=0, Steigung +0.8
      \draw[neuron, akzent, dashed]
        (0,0) -- (1.5, 1.2)  -- (7, 1.2);
      \node[akzent, above] at (0.8, 0.7) {$h_1$};

      % Neuron 2: startet bei x=1.5, Steigung -1.0
      \draw[neuron, hell, dashed]
        (0,0) -- (1.5, 0) -- (3.5, -2) -- (7,-2);
      % Clip für Sichtbarkeit: nur den relevanten Teil zeigen
      \begin{scope}
        \clip (0,-1.5) rectangle (7.5,4.2);
        \draw[neuron, hell, dashed]
          (0,0) -- (1.5,0) -- (3.5, -2);
      \end{scope}
      \node[hell, below] at (2.5, -0.8) {$h_2$};

      % Neuron 3: startet bei x=3.5, Steigung +0.6
      \draw[neuron, highlight, dashed]
        (0,0) -- (3.5, 0) -- (5.5, 1.2) -- (7, 1.2);
      \node[highlight, above] at (4.5, 0.7) {$h_3$};

      % Neuron 4: startet bei x=5.5, Steigung -0.5
      \draw[neuron, violet, dashed]
        (0,0) -- (5.5, 0) -- (7, -0.75);
      \node[violet, below] at (6.3, -0.5) {$h_4$};

      % ── Summe = stückweise lineare Approximation ──
      % Manuell berechnet:
      %   [0, 1.5]:    Steigung +0.8, Start bei y=1.0 (Bias)
      %   [1.5, 3.5]:  +0.8 -1.0 = -0.2
      %   [3.5, 5.5]:  -0.2 +0.6 = +0.4
      %   [5.5, 7]:    +0.4 -0.5 = -0.1
      \draw[line width=2.5pt, akzent]
        (0, 1.0) -- (1.5, 2.2) -- (3.5, 1.8) -- (5.5, 2.6) -- (7, 2.45);

      % Knicke markieren
      \foreach \kx/\ky in {1.5/2.2, 3.5/1.8, 5.5/2.6} {
        \fill[akzent] (\kx, \ky) circle (3pt);
      }

      % Beschriftung Summe
      \node[akzent, right, font=\small\bfseries] at (7, 2.45)
        {$\hat{f}(x) = \sum_i h_i(x)$};

      % ── Legende unten ──
      \node[anchor=north west, text width=12cm, font=\scriptsize,
            inner sep=4pt, fill=white, draw=gray!40, rounded corners]
        at (0, -1.8) {%
          Jedes Neuron $h_i(x) = a_i\,\max(0,\; x - c_i)$ liefert
          einen \textbf{Knick} an Position $c_i$.\\[2pt]
          \textbf{4 Neuronen → 3 Knicke} → grobe Approximation.\\
          \textbf{100 Neuronen → 99 Knicke} → fast beliebig genau. \\
          \textbf{Negative Steigung} entsteht durch die zwei Dense-Schicht. die wieder negative Gewichte haben kann
        };
...
\begin{frame}{Versteckte Zusammenhänge}
  \begin{columns}[T]
    ...
    \medskip
    \color{akzent}\textbf{→ KI lernt: ``nachts = schlechter Ertrag''}\\
    \color{gray}\textit{Weiß aber nicht, dass Schnecken der Grund sind.}

    \column{0.4\textwidth}
    \placeholder[0.9\linewidth]{Bild: Garten + Schnecke}
  \end{columns}
\end{frame}
...
\begin{frame}{Verschachtelte Layer = Abstraktion}
  \begin{center}
    \begin{tikzpicture}[
        layer/.style={draw, thick, rounded corners, minimum width=2.5cm, 
                     minimum height=0.7cm, font=\small\bfseries},
        arr/.style={-{Stealth}, thick},
        node distance=0.3cm
      ]
      % Nodes von unten nach oben
      \node (in) at (0,0) {\footnotesize Bewässerungsdaten};
      
      % Layer 1: f(x) + Aktivierung
      \node[layer, fill=hell!20, above=of in]    (l1) {$f(\mathbf{x})$};
      \node[layer, fill=akzent!20, above=of l1]  (s1) {$\sigma$};
      
      % Layer 2: g(.) -> Direkter Output
      \node[layer, fill=hell!20, above=of s1]    (l2) {$g(\cdot)$};
      
      % Output Prognose
      \node[above=of l2] (out) {\footnotesize Ernteprognose (kg)};
  ...
  \centering
	\formelbox{\text{jetzt Bewässern ja/nein} = g\bigl(\sigma\bigl(f(\mathbf{[\text{Temperatur, aktuelle Uhrzeit, Windgeschwindigkeit}, \dots]})\bigr)\bigr)}
\end{frame}
...
\begin{frame}[plain]
  \begin{center}
    {\Large\textbf{Positional Encoding – Visualisierung}}\\[1em]
    \placeholder[0.7\textwidth]{Bild: hochdimensionale Spirale / PE-Heatmap
                                 (vorhanden – hier einfügen)}
  \end{center}
\end{frame}

% Füge das in deine presentation.tex ein, anstelle des Placeholders
% bei "Positional Encoding – Visualisierung"
...
\begin{frame}{Wie ein Sprachmodell Text generiert}
  \begin{center}
    \begin{tikzpicture}[
      ...
      \node[box, fill=highlight!15, right=of layers] (winkel) {Winkelvergleich};
...
\begin{frame}[plain]
  \begin{center}
    ...
    \vspace{1.5em}
    \placeholder[0.75\textwidth]{Vollbild: Faserbündel-Visualisierung
                                  mit Fragezeichen innen (vorhanden – hier einfügen)}
  \end{center}
\end{frame}
...
\begin{frame}{Zwei Interpretationen}
  \begin{columns}[T]
    ...
    \begin{center}
      \textbf{Interpretation A}\\[0.5em]
      Punkte werden im Raum \textbf{verschoben}
    \end{center}
    \begin{tikzpicture}[scale=0.7]
      \draw[step=1, gray!30, thin] (-0.5,-0.5) grid (3.5,3.5);
      \fill[hell] (1,1) circle (4pt);
      \fill[akzent] (2.5,2.8) circle (4pt);
      \draw[-{Stealth}, thick, dashed] (1,1) -- (2.5,2.8);
    \end{tikzpicture}

    \column{0.45\textwidth}
    \begin{center}
      \textbf{Interpretation B}\\[0.5em]
      Der \textbf{Raum selbst} wird verformt
    \end{center}
    \begin{tikzpicture}[scale=0.7]
      % Gekrümmtes Gitter – sichere Version ohne Overflow
      % Horizontale Linien
      \draw[gray!40, thin] (0.0, 0.10) -- (0.7, 0.05) -- (1.4, 0.15) -- (2.1, 0.0) -- (2.8, 0.12) -- (3.5, 0.05);
      \draw[gray!40, thin] (0.05, 0.75) -- (0.7, 0.82) -- (1.4, 0.68) -- (2.1, 0.78) -- (2.8, 0.65) -- (3.5, 0.72);
      \draw[gray!40, thin] (0.1, 1.45) -- (0.7, 1.52) -- (1.4, 1.38) -- (2.1, 1.50) -- (2.8, 1.42) -- (3.5, 1.48);
      \draw[gray!40, thin] (0.0, 2.15) -- (0.7, 2.22) -- (1.4, 2.10) -- (2.1, 2.25) -- (2.8, 2.12) -- (3.5, 2.18);
      \draw[gray!40, thin] (0.08, 2.85) -- (0.7, 2.78) -- (1.4, 2.92) -- (2.1, 2.82) -- (2.8, 2.90) -- (3.5, 2.85);
      \draw[gray!40, thin] (0.0, 3.55) -- (0.7, 3.48) -- (1.4, 3.58) -- (2.1, 3.45) -- (2.8, 3.52) -- (3.5, 3.50);
      % Vertikale Linien
      \draw[gray!40, thin] (0.05, 0.10) -- (0.0, 0.75) -- (0.1, 1.45) -- (0.0, 2.15) -- (0.08, 2.85) -- (0.0, 3.55);
      \draw[gray!40, thin] (0.7, 0.05) -- (0.72, 0.82) -- (0.68, 1.52) -- (0.7, 2.22) -- (0.73, 2.78) -- (0.7, 3.48);
      \draw[gray!40, thin] (1.4, 0.15) -- (1.38, 0.68) -- (1.42, 1.38) -- (1.4, 2.10) -- (1.38, 2.92) -- (1.4, 3.58);
      \draw[gray!40, thin] (2.1, 0.0) -- (2.12, 0.78) -- (2.08, 1.50) -- (2.1, 2.25) -- (2.12, 2.82) -- (2.1, 3.45);
      \draw[gray!40, thin] (2.8, 0.12) -- (2.78, 0.65) -- (2.82, 1.42) -- (2.8, 2.12) -- (2.78, 2.90) -- (2.8, 3.52);
      \draw[gray!40, thin] (3.5, 0.05) -- (3.48, 0.72) -- (3.52, 1.48) -- (3.5, 2.18) -- (3.48, 2.85) -- (3.5, 3.50);
      % Punkt
      \fill[hell] (1.4,1.38) circle (4pt);
      \node[font=\scriptsize, below] at (1.4,1.1) {gleicher Punkt};
      \node[font=\scriptsize\color{akzent}] at (2.8,3.9) {Raum gekrümmt};
    \end{tikzpicture}
  ...
  \medskip
  \begin{itemize}
    \stich{Augenscheinlich identisch – aber:}
    \stich{Raumkrümmung kann \textbf{Information speichern} in der Krümmung selbst}
    \stich{Und: in der Krümmung kann \textbf{gerechnet} werden}
  \end{itemize}
\end{frame}
...
\begin{frame}{Pigeonhole-Prinzip \& verteilte Speicherung}
  \begin{columns}[T]
    ...
    \column{0.45\textwidth}
    \placeholder[0.9\linewidth]{Bild: Pigeonhole-Prinzip}

    \vspace{0.5em}
    \placeholder[0.9\linewidth]{Bild: Hologramm –
                                 Smiley links, Hologramm-Muster rechts}
  \end{columns}
\end{frame}
...
\begin{frame}{Der Fluss – von Tokens zu Topologie}
  \begin{center}
    \begin{tikzpicture}[
      ...
      \node[box, fill=hell!15]      (s1) {1. Dense Layer};
      \node[box, fill=hell!15, below=of s1] (s2) {2. Positional Encoding};
      \node[box, fill=dunkel!15, text=white, below=of s2] (s3) {3. Attention + FFN};
      \node[box, fill=dunkel!15, text=white, below=of s3] (s3b) {3b. Residual Stream};
      \node[box, fill=akzent!15, below=of s3b] (s4) {4. Raumkrümmung};
      \node[box, fill=highlight!15, below=of s4] (s5) {5. Topologie};
      \node[box, fill=highlight!15, below=of s5] (s6) {6. Holografische Kodierung};
      \node[box, fill=highlight!20, below=of s6] (s7) {7. Garbentheorie};

      \foreach \a/\b in {s1/s2, s2/s3, s3/s3b, s3b/s4, s4/s5, s5/s6, s6/s7}
        \draw[arr] (\a) -- (\b);

      % Annotations rechts
      \node[right=1cm of s1, font=\scriptsize, text width=4cm]
        {Grundbaustein, universeller Approximator};
      \node[right=1cm of s3, font=\scriptsize, text width=4cm]
        {Beziehungen + Transformation};
      \node[right=1cm of s4, font=\scriptsize, text width=4cm]
        {Information in der Geometrie};
      \node[right=1cm of s7, font=\scriptsize, text width=4cm]
        {Lokale Daten → (Prä-)Garbe};
    \end{tikzpicture}
  \end{center}
\end{frame}
...
\begin{frame}{Offene Fragen}
  \begin{itemize}
    ...
    \stich{Wie weit reicht die \textbf{Prä-Garbe} – wo bricht sie zusammen?}
