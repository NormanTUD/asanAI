...
\begin{frame}{Verschachtelte Layer = Abstraktion}
  \begin{center}
    \begin{tikzpicture}[
      ...
      % Nodes von oben nach unten
      \node (in) at (0,0) {\footnotesize Bewässerungsdaten};
      
      % Layer 1: f(x) + Aktivierung
      \node[layer, fill=hell!20, below=of in]    (l1) {$f(\mathbf{x})$};
      \node[layer, fill=akzent!20, below=of l1]  (s1) {$\sigma$};
      
      % Layer 2: g(.) -> Direkter Output
      \node[layer, fill=hell!20, below=of s1]    (l2) {$g(\cdot)$};
      
      % Output Prognose
      \node[below=of l2] (out) {\footnotesize Ernteprognose (kg)};
