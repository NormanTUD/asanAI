...
% ══════════════════════════════════════════════════════════════
% Frame 1: Isosurfaces 1–6 (Vollbild, durchklickbar)
% ══════════════════════════════════════════════════════════════

\definecolor{isobg}{HTML}{121B2E}

{
\setbeamercolor{background canvas}{bg=isobg}
\foreach \n in {1,...,6} {%
  \begin{frame}[plain]
    \begin{tikzpicture}[remember picture, overlay]
      \node at (current page.center) {%
        \includegraphics[width=\paperwidth, height=\paperheight, keepaspectratio]{isosurfaces/\n.png}%
      };
    \end{tikzpicture}
  \end{frame}
}
...
% ══════════════════════════════════════════════════════════════
% Frame 2: Temperatur-Erklärung (Text auf dunklem Hintergrund)
% ══════════════════════════════════════════════════════════════
...
\begin{frame}[plain]
  \vfill
  \begin{center}
    {\Huge\color{white}\textbf{Temperatur}}\\[1.5em]
    {\Large\color{white!80}Was passiert, wenn $T$ steigt?}\\[1.5em]
    {\large\color{white!60}Die Isosurface wird \textbf{aufgebläht} –\\[0.4em]
    mehr Tokens erhalten ähnliche Wahrscheinlichkeit.\\[0.4em]
    Der Pfad der Wahl wird \textbf{diffuser}.}
  \end{center}
  \vfill
\end{frame}
}
...
% ══════════════════════════════════════════════════════════════
% Frame: Isosurface 4 vs. France High Temp (nebeneinander)
% ══════════════════════════════════════════════════════════════
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    % Bild links: Isosurface 4
    \node[anchor=east] at ([xshift=-0.3cm]current page.center) {%
      \includegraphics[height=0.85\paperheight, keepaspectratio]{isosurfaces/4.png}%
    };
    % Bild rechts: France High Temp
    \node[anchor=west] at ([xshift=0.3cm]current page.center) {%
      \includegraphics[height=0.85\paperheight, keepaspectratio]{isosurfaces/france_high_temp.png}%
    };
  \end{tikzpicture}
\end{frame}
}
...
\begin{frame}{Positional Encoding – Spiralförmige Nachbarschaft}
  \begin{columns}[T]
    \column{0.55\textwidth}
    \begin{itemize}
      \stich{Wörter werden nicht zu isolierten Punkten, sondern zu \textbf{Regionen} im Raum}
      \stich{Überlagerung verschieden schneller Rotationen → spiralartiger \textbf{Fingerabdruck} pro Position}
      \stich{Benachbarte Positionen → überlappende Spiralen → \textbf{relative Abstände} bleiben erhalten}
    \end{itemize}

    \column{0.4\textwidth}
    \begin{center}
      \includegraphics[width=\linewidth, keepaspectratio]{posembed.png}\\[0.3em]
      {\scriptsize\color{gray}Jedes Wort umgeben von einer spiralförmigen Region – seinem positionalen Fingerabdruck}
    \end{center}
  \end{columns}
\end{frame}
...
\begin{frame}{Panorama-Prinzip}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \stich{Wenn sie \emph{nicht} überlappen → \textbf{not verklebbar}}
    \end{itemize}
    ...
    \column{0.4\textwidth}
    \includegraphics[width=0.9\linewidth]{panorama_overlap.jpg}
  \end{columns}
\end{frame}
