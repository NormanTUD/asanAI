...
\begin{frame}{Der Residual Stream – globales Notizbuch}
  \begin{center}
    \begin{tikzpicture}[
      ...
      % Experten-Gruppen
      \foreach \x/\label in {1.5/L1, 4/L2, 6.5/L3, 9/\ldots, 10.5/L$_N$} {
        \node[expert] (e\x) at (\x, 1.2) {\label};
        \draw[arr] (\x, 0.15) -- (e\x.south);
        \draw[arr] (e\x.south) ++(0.3,0) -- ++(0,-0.95);
      }
  ...
  \begin{columns}[T]
    ...
    \begin{tikzpicture}[scale=0.7]
      ...
      \foreach \x in {0,0.7,...,3.5} {
        \draw[gray!40, thin] plot[smooth, domain=0:3.5, samples=20]
          ({\x + 0.15*sin(deg(\x)*80)*sin(deg(\x)*60)},
           {\x + 0.15*cos(deg(\x)*70)});
      }
      ...
      \fill[hell] (1,1) circle (4pt);
      \node[font=\scriptsize, below] at (1,0.8) {gleicher Punkt};
      \node[font=\scriptsize\color{akzent}, above right] at (2,2.5)
        {Raum gekrümmt};
    \end{tikzpicture}
  \end{columns}
