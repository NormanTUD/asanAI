...
\begin{frame}{Wie approximiert ein Netz eine Funktion?}
  \begin{center}
    \begin{tikzpicture}[
      ...
      % h₄: ReLU ab x=5.0, Steigung -0.8
      \draw[thick, violet]
        (-0.5, 0) -- (5.0, 0);
      \draw[thick, violet]
        (5.0, 0) -- (7, -1.6);
      \fill[violet] (5.0, 0) circle (3pt);
      \node[violet, below right, font=\small\bfseries] at (5.5, -0.5) {$h_4$};

      % Summe: bias + h₁ + h₂ + h₃ + h₄
      % x in [0,1.5]: 0.8 + 0.9x
      % x in [1.5,3.0]: 3.05 - 0.6x
      % x in [3.0,5.0]: -0.55 + 0.6x
      % x in [5.0,7]: -0.55 + 0.6x - 0.8(x-5.0) = 3.45 - 0.2x
      \draw[line width=2.5pt, akzent!70!black]
        (0, 0.8) -- (1.5, 2.15) -- (3.0, 1.25) -- (5.0, 2.45) -- (7, 2.05);
      \foreach \kx/\ky in {1.5/2.15, 3.0/1.25, 5.0/2.45} {
        \fill[akzent!70!black] (\kx, \ky) circle (3pt);
      }
      \node[akzent!70!black, right, font=\small\bfseries] at (7, 2.05) {$\hat{f}$};
    \end{tikzpicture}
  \end{center}
  ...
  \begin{center}
    \colorbox{dunkel!10}{%
      \parbox{0.78\textwidth}{\centering\small
        \textbf{Viertes Neuron} $h_4$ – bremst die Steigung am Ende.\\[4pt]
...
\begin{frame}{Wie approximiert ein Netz eine Funktion?}
  \begin{center}
    \begin{tikzpicture}[
      ...
      % Grobe Approximation (4 Neuronen, blass)
