...
\newcommand{\formelbox}[1]{%
  \begin{center}
    \colorbox{dunkel!10}{\parbox{0.8\textwidth}{\centering\Large$\displaystyle #1$}}
  \end{center}
}
...
\begin{frame}{Verschachtelte Layer = Abstraktion}
  \begin{center}
    \begin{tikzpicture}[
      ...
      \node[layer, fill=hell!20]   (l1) at (0,0)   {$f(\mathbf{x})$};
      \node[layer, fill=akzent!20] (s1) at (0,1.3)  {$\sigma$};
      \node[layer, fill=hell!20]   (l2) at (0,2.6)  {$g(\cdot)$};
      \node[layer, fill=akzent!20] (s2) at (0,3.9)  {$\sigma$};
      \node[layer, fill=hell!20]   (l3) at (0,5.2)  {$h(\cdot)$};
      \node (in)  at (0,-1.2) {Bewässerungsdaten};
      \node (out) at (0,6.4)  {Ernteprognose (kg)};
      \foreach \a/\b in {in/l1, l1/s1, s1/l2, l2/s2, s2/l3, l3/out}
        \draw[arr] (\a) -- (\b);
    \end{tikzpicture}
  \end{center}
  \formelbox{\text{Ausgabe} = h\bigl(\sigma\bigl(g\bigl(\sigma\bigl(f(\mathbf{x})\bigr)\bigr)\bigr)\bigr)}
\end{frame}
