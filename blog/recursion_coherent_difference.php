...
Gödel's incompleteness theorem is the recursion-theoretic twin. Any consistent formal system strong enough to express arithmetic contains a sentence $G$ whose content is equivalent to "$G$ is not provable in this system". The construction uses two recursive ingredients: *Gödel numbering*, which assigns each formula a natural number via primitive recursive functions, making syntax expressible inside arithmetic; and the *Diagonalization Lemma* (also called the Fixed-Point Lemma), which takes any property $\varphi(x)$ and produces a sentence $\lambda$ such that the system proves $\lambda \leftrightarrow \varphi(\ulcorner\lambda\urcorner)$. The lemma itself is a recursive construction — it builds a self-referencing sentence by a fixed-point trick that is structurally identical to the Y combinator. Applied to the property "is not provable", it yields $G$. The result is a sentence that is true (if the system is consistent) but unprovable — a fixed point of "not provable in this system" that the system cannot reach. The parallel with the sheaf picture is structural: a model $G$ that is coherent on its current cover but whose truth at contact points outside the cover is undecidable from within. The out-of-cover query pathology of \citetitle{coherent_world_models} — the fifth pathology, where the query lies outside $c_{\mathrm{acc}}$ and no contact point exists — is the epistemic analogue of Gödel's $G$: a claim the model cannot test, not because it is false, but because the cover does not reach far enough.
</div>
...
In Homotopy Type Theory, the hierarchy becomes the *universe tower* $\mathcal{U}_0 : \mathcal{U}_1 : \mathcal{U}_2 : \cdots$, and the univalence axiom adds a recursive twist: the identity type of a universe *is* the equivalence type. To say two types are equal in $\mathcal{U}$ is to say they are equivalent — and to say two equivalences are equal is to say they are homotopic — and so on. The identity relation itself is recursive, folding back on itself at every level. This is the categorical expression of Nagarjuna's claim that *whatever is dependently originated, that we declare to be emptiness*: identity has no intrinsic content; it is constituted entirely by the recursive structure of its own higher manifestations.

The natural numbers object $\mathbb{N}$ in a topos is the canonical example of a recursive type. It is defined by an *initial algebra* for the successor functor: the universal object with a point $0$ and a function $s : \mathbb{N} \to \mathbb{N}$. The recursion principle says: given any object $X$, a point $x \in X$, and a function $f : X \to X$, there exists a unique map $\mathbb{N} \to X$ sending $0$ to $x$ and commuting with $s$ and $f$. This is the *categorical statement of mathematical induction*: to define a function on $\mathbb{N}$, specify its value at 0 and specify how to get from $n$ to $n+1$. Induction *is* the recursion principle of the natural numbers, and the natural numbers *are* the free algebra generated by recursion on one constructor.
...
&\text{The natural numbers are what recursion builds.}
\end{aligned}}
$$
</div>
