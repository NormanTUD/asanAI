...
Gödel's incompleteness theorem is the recursion-theoretic twin. Any consistent formal system strong enough to express arithmetic contains a sentence $G$ that says "$G$ is not provable in this system". The construction of $G$ is recursive: it uses the Gödel numbering to represent the provability predicate inside the system, then diagonalizes. The result is a sentence that is true (if the system is consistent) but unprovable — a fixed point of "not provable in this system" that the system cannot reach. The parallel with the sheaf picture is structural: a model $G$ that is coherent on its current cover but whose truth at contact points outside the cover is undecidable from within. The out-of-cover query pathology of \citetitle{coherent_world_models} — the fifth pathology, where the query lies outside $c_{\mathrm{acc}}$ and no contact point exists — is the epistemic analogue of Gödel's $G$: a claim the model cannot test, not because it is false, but because the cover does not reach far enough.
</div>
