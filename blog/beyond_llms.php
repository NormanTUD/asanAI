...
SVMs held the state of the art on many classification tasks through the 1990s and 2000s, and remain the right tool when the dataset is small, the features are high-dimensional, and a sparse, kernel-based decision boundary is appropriate. The **kernel machines** viewpoint — that any algorithm expressible in inner products can be "kernelised" by replacing those inner products with a positive-definite kernel — extends to principal component analysis, ridge regression, canonical correlation analysis, and many more \cite{scholkopf2002learning}. A modern revival has come from the **neural tangent kernel** \cite{jacot2018ntk}, which shows that infinitely-wide neural networks are mathematically equivalent to kernel machines with a specific kernel, partially vindicating the 1990s intuition. The convergence is a theorem, not a heuristic: in the limit of infinite width, the gradient-descent training dynamics of the network converge almost surely to gradient flow in function space with a fixed, deterministic kernel — the NTK — and the trained function is the standard kernel-regression solution \cite{petersen2024mathdl}.
</div>
