...
with $\sigma$ typically being GeLU. \cite[Geva et al., 2020]{keyvalmem} made the observation that turned this into a research program: the columns of $W_1$ can be read as **keys** (input patterns the neuron "looks for"), and the corresponding rows of $W_2$ as **values** (vectors to write into the residual stream when that key matches). The activation $\sigma(x W_1)$ is the *soft* match score against every key.
...
Concretely, if $\gamma(w) \in \mathbb{R}^{d}$ denotes the unembedding-column vector for word $w$ (so that the logit for $w$ is $\gamma(w) \cdot x$), the hypothesis predicts
...
The answer is **superposition** \cite[Elhage et al., 2022]{elhage2022superposition}: the model represents more features than it has dimensions, by encoding features as nearly-orthogonal directions in activation space.
...
The systematic way to *recover* monosemantic features from a superposed residual stream is the **sparse autoencoder** \cite[Cunningham et al., 2023]{cunningham2023sparse} \cite[Bricken et al., 2023]{bricken2023monosemanticity}. A sparse autoencoder is a small network trained to decompose the residual stream into a much larger dictionary of sparse features, such that each feature fires for a single human-interpretable concept. The original SAE work by \cite[Bricken et al., 2023]{bricken2023monosemanticity} trained SAEs on a small real transformer (GPT-2 small), not a single-layer toy; \cite[Templeton et al., 2024]{templeton2024scaling} then scaled the same idea to Claude 3 Sonnet and recovered up to 34 million features. A parallel line of work by \cite[Elhage et al., 2022]{elhage2022solu} showed that an architectural change — replacing the GeLU activation in MLP neurons with a custom "SoLU" activation — makes individual neurons substantially *more* monosemantic even without SAEs, by reducing the incentive to use superposition in the first place. This is an active research area; the current best SAEs recover tens of millions of interpretable features from a frontier model, and they have been used to find features that correlate with specific factual associations, specific syntactic structures, specific sentiment, and specific safety-relevant concepts.
</div>
...
- **Attention** \cite[Vaswani et al., 2017]{vaswani2017attention} \cite[Elhage et al., 2021]{elhage2021mathematical} routes information between positions: in our prompt, the critical job is making the final position attend back to the position holding "France".
- **MLP** \cite[Geva et al., 2020]{keyvalmem} performs a soft key-value lookup: somewhere in the weight matrices $W_1, W_2$ is a key that activates on "country with capital-relation context" and a value that writes the "capital-of-X" direction \cite[Park et al., 2024]{park2024linear} into the residual stream.
...
This picture has not come from any single experiment; it has been confirmed in pieces, each line of work tying a different mechanism to a specific, testable claim. ROME \cite[Meng et al., 2022]{meng2022locating} showed that a single rank-one edit to one mid-layer MLP block can rewrite one fact — the Eiffel Tower's city, say — while leaving its neighbours intact, and the successor method MEMIT \cite[Meng et al., 2022]{meng2022memit} scaled the same locality result to thousands of simultaneous edits. Knowledge-neuron attribution \cite[Dai et al., 2022]{dai2022knowledgeneurons} found that suppressing or amplifying a small cluster of MLP neurons flips a specific prediction on and off. Function-vector steering \cite[Todd et al., 2024]{todd2024functionvectors} extracted the single residual-stream direction that encodes an in-context task and transplanted it into unrelated prompts. Induction-head analysis \cite[Olsson et al., 2022]{olsson2022induction} identified the attention pattern that lets later tokens read earlier ones, which is what powers in-context learning. The logit lens \cite[nostalgebraist, 2020]{nostalgebraist2020logitlens} and its refinement, the tuned lens \cite[Belrose et al., 2023]{belrose2023tunedlens}, let you decode the residual stream at every layer and watch the answer form. Superposition theory \cite[Elhage et al., 2022]{elhage2022superposition} explains why none of these features lives in a single neuron — the model packs far more features than it has dimensions — and sparse autoencoders \cite[Cunningham et al., 2023]{cunningham2023sparse} \cite[Bricken et al., 2023]{bricken2023monosemanticity} recover them as interpretable directions, scaled to a frontier production model by \cite[Templeton et al., 2024]{templeton2024scaling}; the SoLU activation \cite[Elhage et al., 2022]{elhage2022solu} attacks the same problem from the training side by reducing the incentive to superpose. Anthropic's report "Mapping the mind of a large language model" \cite[Anthropic, 2023]{anthropic2023mapping} was the early accessible summary; the full machinery grew out of the Transformer Circuits Thread series, from the residual-stream framework \cite[Elhage et al., 2021]{elhage2021mathematical} and induction heads \cite[Olsson et al., 2022]{olsson2022induction} to superposition \cite[Elhage et al., 2022]{elhage2022superposition}, dictionary learning \cite[Bricken et al., 2023]{bricken2023monosemanticity}, scaling monosemanticity \cite[Templeton et al., 2024]{templeton2024scaling}, and circuit tracing in a production model \cite[Ameisen et al., 2025]{ameisen2025circuit} \cite[Lindsey et al., 2025]{lindsey2025biology} — a series that has driven most of this story since 2021. The picture is also consistent with the Hopfield-network interpretation of attention \cite[Ramsauer et al., 2020]{ramsauer2020hopfield}, building on the classical associative-memory networks of \cite[Hopfield, 1982]{hopfield1982}. It is not yet, however, the whole picture for frontier-scale models or multi-hop reasoning.
